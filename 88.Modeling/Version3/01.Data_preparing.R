...
outdir <- paste0("./analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/06.Modeling/Version3")
...
Sigs_Valid <- Sigs[, .N, .(AA, file_id)][, .SD[which.min(N)], AA]
Sigs_Valid <- merge(Sigs_Valid[, 1:2], Sigs, by = c("AA", "file_id"))

Sigs <- Sigs[!ID %in% Sigs_Valid$ID]
...
Sigs_Valid_FM <- as.data.frame(FMs[Sigs_Valid$ID0, grepl("X", colnames(FMs)) | colnames(FMs) %in% c("DeltaMean", "StageSD", "CurrentSD", "Segments", "SignalCurrentPercent", "DwellTime", "Blockade"), with = F])

save(Sigs_Valid, Sigs_Valid_FM, Sigs_Test, Sigs_Test_FM, Sigs_Train_Upsample, Sigs_Train_Upsample_FM, file = paste0(outdir, "/L1/Modeling_Data.RData"))
