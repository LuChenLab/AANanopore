...
outdir <- paste0("./analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/06.Modeling/Version1")
...
maxn <- pmin(Sigs_Train[, .N, AA][, max(N)], 100)
...
Sigs_Train_Upsample_FM <- as.data.frame(FMs[Sigs_Train_Upsample$ID0, grepl("X", colnames(FMs)) | colnames(FMs) %in% c("DeltaMean", "StageSD", "CurrentSD", "Segments", "SignalCurrentPercent", "SignalCurrentWidth", "DwellTime", "Blockade"), with = F])
Sigs_Train_Upsample_FM$Class <- factor(Sigs_Train_Upsample$AA)

Sigs_Test <- Sigs[State == "Sington" & !ID %in% Sigs_Train_Upsample[, ID]]
Sigs_Test_FM <- as.data.frame(FMs[Sigs_Test$ID0, grepl("X", colnames(FMs)) | colnames(FMs) %in% c("DeltaMean", "StageSD", "CurrentSD", "Segments", "SignalCurrentPercent", "SignalCurrentWidth", "DwellTime", "Blockade"), with = F])
