...
FYSL.bgsignals <- do.call(rbind, lapply(bgfile, fread))
FYSL.bgsignals[, ID := paste0("Noise_", ID)]
FYSL.bgsignals <- data.table(FYSL.bgsignals, D1 = FYSL.bgsignals[, get_density(x = Blockade, y = log10(DwellTime))])
```

```{r }
FYSL.selected.signals <- fread(file = "./analysis/83.Hydrolyzation/03.SelectedSignals/FYSL.selected.signals.2.txt", sep = "\t")
FYSL.selected.signals <- data.table(FYSL.selected.signals, D1 = FYSL.selected.signals[, get_density(x = Blockade, y = log10(DwellTime))])
```

```{r}
FYSL.all.signals <- do.call(rbind, lapply(meta[amino_acid == "FYSL", sig_file], fread))
FYSL.all.signals <- FYSL.all.signals[A %in% FYSL.selected.signals[, A]]
FYSL.all.signals <- data.table(FYSL.all.signals, D1 = FYSL.all.signals[, get_density(x = Blockade, y = log10(DwellTime))])
```

```{r}
ggplot() + 
  geom_point(data = FYSL.bgsignals[sample(.N, .N)], aes(x = Blockade, y = DwellTime, colour = A, alpha = D1), size = 0.2) + 
  geom_rect(aes(xmin = FYSL.bgsignals[, min(Blockade)], xmax = 0.3, ymin = 0.35, ymax = FYSL.bgsignals[, max(DwellTime)]), fill = NA, colour = "red") + 
  scale_y_log10() + 
...
```{r}
ggplot() + 
  geom_point(data = FYSL.all.signals[sample(.N, .N)], aes(x = Blockade, y = DwellTime, colour = A, alpha = D1), size = 0.2) + 
  geom_rect(aes(xmin = FYSL.all.signals[, min(Blockade)], xmax = 0.3, ymin = 0.35, ymax = FYSL.all.signals[, max(DwellTime)]), fill = NA, colour = "red") + 
  scale_y_log10() + 
  labs(y = "Dwell time (ms)") + 
  ggthemes::theme_calc(base_size = 15) + 
  theme(panel.grid = element_blank(), 
...
```{r}
ggplot() + 
  geom_point(data = FYSL.selected.signals[sample(.N, .N)], aes(x = Blockade, y = DwellTime, colour = A, alpha = D1), size = 0.2) + 
  scale_y_log10() + 
  labs(y = "Dwell time (ms)") + 
  ggthemes::theme_calc(base_size = 15) + 
  theme(panel.grid = element_blank(), 
...
```{r}
subtab <- Sig_FeatureMatrixs[FYSL.selected.signals$ID]
...
```{r}
FYSL_1 <- merge(FYSL.selected.signals, pred1, by = "ID")
```

```{r}
fwrite(FYSL_1, "./analysis/83.Hydrolyzation/04.SignalsPrediction/FYSL_prediction.txt", sep = "\t", quote = FALSE)
```

```{r}
ggplot() + 
  geom_point(data = FYSL_1[sample(.N, .N)], aes(x = Blockade, y = DwellTime, colour = Pred2, alpha = D1), size = 1) + 
  scale_y_log10() + 
...
AADensityPlot1(x = FYSL_1[Prob1 > 0.75 & Delta1 > 0.5], target = c("Leu", "Ser", "Phe", "Tyr")) -> p5
p5
```
...
AADensityPlot2(x = FYSL_1[Prob1 > 0.75 & Delta1 > 0.5][Prob2 > 0.75 & Delta2 > 0.5, ], target = c("Leu", "Ser", "Phe", "Tyr")) -> p7
p7
```

```{r eval=FALSE}
FYSL_1[, .(.N, length(unique(Pred1))), .(A, round(StartTime))][order(N, decreasing = T)]
```

```{r}
FYSL_abf <- readRDS("./analysis/81.ABFProcessing/ABF/ABF_21o19001.Rds")
```

```{r fig.height=2, fig.width=12, echo=FALSE}
FYSL_abf_i <- FYSL_abf[round(Time) == 1395]
L0 <- MainRidge(x = FYSL_abf_i[, pA], bw = 0.5, peaks = 1, CI = 0.95, plot = F)[[1]]
aasumm1 <- SigSumm[A %in% c("Leu", "Ser", "Phe", "Tyr")]
...
ggplot() + 
  geom_rect(data = aasumm1, aes(xmin = FYSL_abf_i[, min(Time)], xmax = FYSL_abf_i[, max(Time)], ymin = ymin, ymax = ymax, fill = A), alpha = 0.3) + 
  geom_step(data = FYSL_abf_i, mapping = aes(x = Time, y = pA), size = 0.1) + 
  scale_y_continuous(n.breaks = 4, limits = c(aasumm1[, min(ymin)] - 5, FYSL_abf_i[, max(pA)]), 
...
```{r}
FYSL_1_Freq <- FYSL_1[, .N, Pred2]
FYSL_1_Freq[, Pred2 := factor(Pred2, levels = FYSL_1_Freq[order(N, decreasing = T), Pred2])]
ggplot(FYSL_1_Freq, aes(x = Pred2, y = N, fill = Pred2)) + 
  geom_col() + 
  scale_fill_manual(breaks = names(aa_cols), values = aa_cols) + 
