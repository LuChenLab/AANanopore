...
```{r}
sig0 <- do.call(rbind, lapply(meta[amino_acid == "FHL", sig_file], fread))
```
...
```{r}
sig_file <- c("21o22013.1", "21o22014.1", "21o24001.1", "21o24002.1")
...
saveRDS(distance_list, file = "./analysis/83.Hydrolyzation/02.SignalDistance/FHL.signal.dist.list.Rds")
```
...
ggplot(sig0[ID %in% sigs], aes(x = Blockade, y = DwellTime, colour = ID %in% sigs)) + 
  ...
  scale_y_log10() + 
  scale_colour_manual(values = "#E41A1C") + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[strsplit("FHL", "")[[1]]], Blockade]) + 
  theme_classic() + 
  theme(legend.position = "none") + 
...
```{r fig.width=20, fig.height=4}
sigs <- KnnSelecting(signals = sig0[A == sig_file[3]], dists = distance_list[[3]], k = 8, noise = 0)
```
...
```{r fig.width=20, fig.height=4}
sigs <- KnnSelecting(signals = sig0[A == sig_file[4]], dists = distance_list[[4]], k = 7, noise = 0)
```
...
```{r}
select_sigs <- sig0[ID %in% c(sigs1, sigs2, sigs3, sigs4)]
```
...
```{r eval=FALSE}
fwrite(select_sigs, file = "./analysis/83.Hydrolyzation/03.SelectedSignals/FHL.selected.signals.2.txt", sep = "\t", quote = FALSE)
```
...
subtab <- data.frame(subtab[, -1], row.names = subtab[[1]])
pred1 <- data.table(ID = row.names(subtab), 
...
H1 <- as.character(predict(model3, subtab[pred1[Pred1 == "His", ID], ]))
pred1[Pred1 == "His", Pred1 := H1]
H2 <- as.character(predict(model3, subtab[pred1[Pred2 == "His", ID], ]))
pred1[Pred2 == "His", Pred2 := H2]
```

```{r}
FHL_1 <- merge(select_sigs, pred1, by = "ID")
FHL_1 <- FHL_1[Blockade < 0.3 & DwellTime > 0.3]
```

```{r}
ggplot(FHL_1, aes(x = Blockade, y = DwellTime, colour = Pred1)) + 
  geom_point() + 
  scale_y_log10()

ggplot(FHL_1, aes(x = Blockade, y = DwellTime, colour = Pred2)) + 
  geom_point() + 
  scale_y_log10()
```

```{r}
FHL_1 <- do.call(rbind, lapply(split(FHL_1, FHL_1$Pred1), function(x) data.table(x, D1 = x[, get_density(x = Blockade, y = log10(DwellTime))])))
```

```{r}
ggplot(FHL_1, aes(x = Blockade, y = DwellTime, colour = get_density(x = Blockade, y = log10(DwellTime), n = 1000))) + 
  geom_point(size = 1) + 
...
```{r}
ggplot(FHL_1[Prob1 > 0.75 & Delta1 > 0.5], aes(x = Blockade, y = DwellTime, colour = Pred1)) + 
...
```{r}
FHL_1[, .N, Pred2][order(N, decreasing = T)]
```

```{r}
ggplot(FHL_1[Prob1 > 0.75 & Delta1 > 0.5], aes(x = SignalCurrentPercent, colour = Pred1)) + 
...
AADensityPlot1(x = FHL_1[Prob1 > 0.75 & Delta1 > 0.5], target = c("Leu", "His1", "His2", "Phe"))
```
...
AADensityPlot2(x = FHL_1[Prob1 > 0.75 & Delta1 > 0.5][Prob2 > 0.75 & Delta2 > 0.5, ], target = c("Leu", "His1", "His2", "Phe"))
```