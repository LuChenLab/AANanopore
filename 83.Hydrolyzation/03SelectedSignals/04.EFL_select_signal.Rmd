...
```{r}
sig0 <- do.call(rbind, lapply(meta[amino_acid == "LEF", sig_file], fread))
```
...
```{r}
sig_file <- c("21o22001.1", "21o22003.1", "21o22004.1", "230314002.1")
big_file <- c("21o22000.1", "21o22002.1", "230314001.1", "21807003.2")
...
```{r fig.width=25, fig.height=4}
ggplot(sig0[Blockade < 0.35], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = 0.4) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("LEF", ""))], Blockade], col = 2) + 
  scale_y_log10() + 
  facet_wrap(~ A, nrow = 1)
...
saveRDS(distance_list, file = "./analysis/83.Hydrolyzation/02.SignalDistance/LEF.signal.dist.list.Rds")
```
...
```{r}
sigs <- RemoveOutlier(tree = res.hc, n = 14, Keep = 0.8)
ggplot(sig0[ID %in% sigs], aes(x = Blockade, y = DwellTime, colour = ID %in% sigs)) + 
  geom_point(size = .5) + 
  scale_y_log10() + 
  scale_colour_manual(values = "#E41A1C") + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[strsplit("LEF", "")[[1]]], Blockade]) + 
  theme_classic() + 
  theme(legend.position = "none") + 
...
```{r}
sigs <- RemoveOutlier(tree = res.hc, n = 15, Keep = 0.75)
ggplot(sig0[ID %in% sigs], aes(x = Blockade, y = DwellTime, colour = ID %in% sigs)) + 
  geom_point(size = .5) + 
  scale_y_log10() + 
  scale_colour_manual(values = "#E41A1C") + 
...
```{r eval=FALSE}
fwrite(select_sigs, file = "./analysis/83.Hydrolyzation/03.SelectedSignals/LEF.selected.signals.2.txt", sep = "\t", quote = FALSE)
```
...
```{r}
LEF_1 <- merge(select_sigs, pred1, by = "ID")
```

```{r}
ggplot(LEF_1, aes(x = Blockade, y = DwellTime, colour = Pred1)) + 
  geom_point() + 
  scale_y_log10()

ggplot(LEF_1, aes(x = Blockade, y = DwellTime, colour = Pred2)) + 
  geom_point() + 
  scale_y_log10()
```

```{r}
LEF_1 <- do.call(rbind, lapply(split(LEF_1, LEF_1$Pred1), function(x) data.table(x, D1 = x[, get_density(x = Blockade, y = log10(DwellTime))])))
```

```{r}
ggplot(LEF_1, aes(x = Blockade, y = DwellTime, colour = get_density(x = Blockade, y = log10(DwellTime), n = 1000))) + 
  geom_point(size = 1) + 
...
```{r}
ggplot(LEF_1[Prob1 > 0.75 & Delta1 > 0.5], aes(x = Blockade, y = DwellTime, colour = Pred1)) + 
...
```{r}
ggplot(LEF_1[Prob1 > 0.75 & Delta1 > 0.5], aes(x = SignalCurrentPercent, colour = Pred1)) + 
...
AADensityPlot1(x = LEF_1[Prob1 > 0.75 & Delta1 > 0.5], target = c("Leu", "Glu", "Phe"))
```
...
AADensityPlot2(x = LEF_1[Prob1 > 0.75 & Delta1 > 0.5][Prob2 > 0.75 & Delta2 > 0.5, ], target = c("Leu", "Glu", "Phe"))
```