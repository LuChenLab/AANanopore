...
```{r}
sig0 <- do.call(rbind, lapply(meta[amino_acid == "FYSLH", sig_file], fread))
```
...
```{r}
sig0 <- do.call(rbind, lapply(meta[amino_acid == "EAFNL", sig_file], fread))
```

```{r fig.width=25, fig.height=4}
ggplot(sig0[Blockade < 0.3], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = 0.4) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("EAFNL", ""))], Blockade], col = 2) + 
  scale_y_log10() + 
  facet_wrap(~ A, nrow = 1)
```
...
```{r}
sig0 <- do.call(rbind, lapply(meta[is.na(amino_acid) | amino_acid == "CPA", sig_file], fread))
```
...
```{r}
bgfile <- meta[is.na(amino_acid) | amino_acid == "CPA", sig_file]
...
```{r}
mclapply(c("FYSLH", "FYSL", "LEF", "FHL", "EAFNL"), function(a) {
  print(a)
  outfile <- file.path("./analysis/83.Hydrolyzation/02.SignalDistance", paste0(a, ".signal.dist.Rds"))
  if(file.exists(outfile)) return(NULL)
  
  sig0 <- do.call(rbind, lapply(meta[amino_acid == a, sig_file], fread))[Blockade < 0.3 & DwellTime > 0.3]
  sig0[, ID := paste0(a, "_", ID)]
  
  B_file <- unique(meta[amino_acid == a, file_name])
  B_file <- paste0("./analysis/81.ABFProcessing/FeatureMatrix/FeatureMatrix_", B_file, ".txt")
  Sig_FeatureMatrix <- do.call(rbind, lapply(B_file, fread))
  Sig_FeatureMatrix[, ID := paste0(a, "_", ID)]
...
```{r}
mclapply(meta[amino_acid %in% c("FYSLH", "FYSL", "LEF", "FHL", "EAFNL"), sig_file], function(a) {
  print(a)
  outfile <- file.path("./analysis/83.Hydrolyzation/02.SignalDistance", gsub(".MainL0.txt", ".signal.dist.Rds", basename(a)))
  if(file.exists(outfile)) return(NULL)
  sig0 <- fread(a)[Blockade < 0.3 & DwellTime > 0.3]
