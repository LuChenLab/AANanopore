...
```{r}
meta0 <- as.data.table(openxlsx::read.xlsx("data/ChenShanchuan/20230715/数据记录20230715.xlsx", sheet = 2))
colnames(meta0) <- c("file_name", "start_time", "end_time", "amino_acid", "purpose", "level", "sample", "base_line", "note", "temp")
meta0 <- meta0[purpose == "阿尔兹海默症突变位点"]
meta0 <- meta0[!is.na(amino_acid)]
meta0 <- meta0[amino_acid != "AP"]
meta0 <- meta0[, .(file_name, start_time, end_time, product = amino_acid)]
```

```{r}
# 直接放弃 20230330_0007, 20230331 _0003, 20230331 _0004, 20230403_0008, 20230403_0009, 20230403_0010, 20230403_0012, 20230403_0013, 20230619_0005, 20230619_0007, 20230713_0007
# 可以一试 20230330 _0006, 20230330 _0007, 20230608_0027, 20230713_0006, 20230714_0006, 20230714_0007
# 感觉还行 20230714_0008
# 令人期待
meta0[file_name %in% c("20230330 _0006", "20230330 _0007", "20230608_0027", "20230713_0006", "20230714_0006", "20230714_0007", "20230714_0008")]
```


```{r}
meta <- meta0[file_name == "20230330 _0007"]
...
BaseMeanFiltering(SigFile = sg_files[1], starttime = meta$start_time, endtime = meta$end_time, target = meta$product, 
                  outdir = "./analysis/62.MachineLearning/03.ADHydrolyzation/20230330_0007", 
...
```{r}
files <- list.files("./analysis/62.MachineLearning/03.ADHydrolyzation/20230330_0007", "MainL0.txt", full.names = T, recursive = T)
...
saveRDS(Sig_Summa, file = "./analysis/62.MachineLearning/03.ADHydrolyzation/20230330_0007/Sig_Summa.Rds")
```

```{r}
Sig_Summa <- readRDS("./analysis/62.MachineLearning/03.ADHydrolyzation/20230330_0007/Sig_Summa.Rds")
```
...
saveRDS(euclidean, file = paste0("./analysis/62.MachineLearning/03.ADHydrolyzation/20230330_0007/Signals_euclidean_distance.Rds"))
```
...
Run1_dist <- readRDS("./analysis/62.MachineLearning/03.ADHydrolyzation/20230330_0007/Signals_euclidean_distance.Rds")
...
```{r}
subtab <- Sig_FeatureMatrix[ID %in% select_sigs_Run1 & ID %in% sgsignals[DwellTime > 0.5 & Blockade < 0.3, ID]]
...
select_sigs_RemoveOutlier_Run1 <- RemoveOutlier(tree = res.hc, n = 4, Keep = 0.95)
...
fwrite(Selected_sigs, "./analysis/62.MachineLearning/03.ADHydrolyzation/20230330_0007/Selected_sigs.txt", sep = "\t", quote = F)
```
...
sgsignals_curt <- merge(sigs, SigCur, by = "ID")
```
...
subtab <- Sig_FeatureMatrix[ID %in% sigs[, ID]]
Fit1 <- readRDS("./analysis/62.MachineLearning/01.ModelTraining/ModelAFGKLV/model.Rds")
...
sigs_pred <- merge(sigs, merge(SigCur[, .(ID, Within, WithinRatio, DiffRatio)], pred[, .(ID, Prob, DiffProb, Pred)], by = "ID"), by = "ID")
sigs_pred <- merge(sigs_pred, Sig_Summa, by = "ID")
```

```{r}
ggplot(sigs_pred[Pred == Within & ratio > 0.8 & WithinRatio > 0.1]) + 
...
fwrite(Selected_sigs2, "./analysis/62.MachineLearning/03.ADHydrolyzation/20230330_0007/Selected_sigs_summary.txt", sep = "\t", quote = F)
```


```{r}
ggplot(data = Selected_sigs2) + 
  geom_text(aes(x = Blockade, y = DwellTime, colour = Within, label = Within)) +
  # geom_rect(data = SigSumm[A %in% AA_Run1, ], aes(xmin = BlockadeMean - BlockadeSD, xmax = BlockadeMean + BlockadeSD, ymin = 0, ymax = 110, fill = A), alpha = 0.3) +
  scale_y_log10() + 
  scale_fill_manual(breaks = names(aa_cols), values = aa_cols, guide = guide_legend(title = element_blank())) + 
