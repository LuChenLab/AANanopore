...
meta <- meta[file_name == "20230418_0009"]
meta
```

```{r}
AA_Run1 <- AMINO_ACID_CODE[strsplit("LWQSTIFNDE", "")[[1]]]
```
...
BaseMeanFiltering(SigFile = sg_files[1], starttime = 0.1, endtime = 21.4, target = "LWQSTIFNDE", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/20230418_0009", 
...
```{r}
files <- list.files("./analysis/62.MachineLearning/02.MixtureAA/20230418_0009", "MainL0.txt", full.names = T, recursive = T)
...
saveRDS(Sig_Summa, file = "./analysis/62.MachineLearning/02.MixtureAA/20230418_0009/Sig_Summa.Rds")
```
...
saveRDS(euclidean, file = paste0("./analysis/62.MachineLearning/02.MixtureAA/20230418_0009/Signals_euclidean_distance.Rds"))
```
...
Run1_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/20230418_0009/Signals_euclidean_distance.Rds")
...
p1 + scale_x_continuous(limits = c(0.1, 0.26))
```

```{r}
Selected_sigs <- sgsignals[ID %in% select_sigs_RemoveOutlier_Run1][Blockade > 0.1]
fwrite(Selected_sigs, "./analysis/62.MachineLearning/02.MixtureAA/20230418_0009/Selected_sigs.txt", sep = "\t", quote = F)
```
...
ggplot(sgsignals_curt[ID %in% Sig_Summa[ratio > 0.9, ID]], aes(x = Blockade, y = DwellTime, colour = Within)) + 
  geom_text(aes(label = Within)) + 
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.125, 0.27)) + 
  theme_classic() + 
  theme(legend.position = "none")
```
...
Fit1 <- readRDS("./analysis/62.MachineLearning/01.ModelTraining/ModelDQELSTW/model.Rds")
...
fwrite(Selected_sigs2, "./analysis/62.MachineLearning/02.MixtureAA/20230418_0009/Selected_sigs_summary.txt", sep = "\t", quote = F)
```
