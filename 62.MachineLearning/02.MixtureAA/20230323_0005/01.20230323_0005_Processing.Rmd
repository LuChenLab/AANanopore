...
meta <- meta[file_name == "20230323_0005"]
meta
```
...
BaseMeanFiltering(SigFile = sg_files[1], starttime = 8.8, endtime = 24.5, target = "LWQST", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/20230323_0005", 
...
```{r}
files <- list.files("./analysis/62.MachineLearning/02.MixtureAA/20230323_0005", "MainL0.txt", full.names = T, recursive = T)
...
saveRDS(Sig_Summa, file = "./analysis/62.MachineLearning/02.MixtureAA/20230323_0005/Sig_Summa.Rds")
```
...
saveRDS(euclidean, file = paste0("./analysis/62.MachineLearning/02.MixtureAA/20230323_0005/Signals_euclidean_distance.Rds"))
```
...
Run1_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/20230323_0005/Signals_euclidean_distance.Rds")
...
fwrite(Selected_sigs, "./analysis/62.MachineLearning/02.MixtureAA/20230323_0005/Selected_sigs.txt", sep = "\t", quote = F)
```
...
fwrite(Selected_sigs2, "./analysis/62.MachineLearning/02.MixtureAA/20230323_0005/Selected_sigs_summary.txt", sep = "\t", quote = F)
```
...
```{r fig.height=5, fig.width=24}
time1 <- 663
abf_run1_i <- abf1[Time >= time1 - .5 & Time <= time1 + 2.5, ]
L0 <- MainRidge(x = abf_run1_i[, pA], bw = 0.5, peaks = 1, CI = 0.95, plot = F)[[1]]
aasumm <- SigSumm[A %in% AA_Run1]
aasumm[, ymin := L0 * (1 - BlockadeMean - BlockadeSD)]
aasumm[, ymax := L0 * (1 - BlockadeMean + BlockadeSD)]
