...
```{r}
meta0 <- as.data.table(openxlsx::read.xlsx("data/ChenShanchuan/20230824/副本数据记录20230824.xlsx", sheet = 2))
colnames(meta0) <- c("file_name", "start_time", "end_time", "amino_acid", "purpose", "level", "sample", "base_line", "note", "temp")
meta <- meta0[file_name %in% gsub(".abf", "", list.files("./data/ChenShanchuan/20230824", "abf"))]
meta <- meta[7:12]
meta
```
...
bg_files <- grep(pattern = "20230824_0016", files, value = T)
sg_files <- files[!grepl(pattern = "20230824_0016", files)]

outdir <- file.path("./analysis/62.MachineLearning/02.MixtureAA/CSC20230824/Rep2")

BaseMeanFiltering(SigFile = bg_files, starttime = 0, endtime = 5.5, outdir = outdir, target = "Background")

BaseMeanFiltering(SigFile = sg_files[1], starttime = 0, endtime = 12, target = "Run1", 
                  outdir = outdir, 
                  product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade]))

BaseMeanFiltering(SigFile = sg_files[1], starttime = 12.5, endtime = 24.0, target = "Run2", 
                  outdir = outdir, 
                  product = as.numeric(AABlockade[AA %in% AA_Run2, Blockade]))

BaseMeanFiltering(SigFile = sg_files[2], starttime = 0, endtime = 12, target = "Run3", 
                  outdir = outdir, 
                  product = as.numeric(AABlockade[AA %in% AA_Run3, Blockade])) # 可以重选一次试试

BaseMeanFiltering(SigFile = sg_files[2], starttime = 12.5, endtime = 24, target = "Run4", 
                  outdir = outdir, 
                  product = as.numeric(AABlockade[AA %in% AA_Run4, Blockade])) # 可以重选一次试试

BaseMeanFiltering(SigFile = sg_files[3], starttime = 0, endtime = 15, target = "Run5",
                  outdir = outdir,
...
Run1_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/CSC20230824/Rep2/Run1_Signals_euclidean_distance.Rds")
...
```{r fig.width=24, fig.height=6}
Run2_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/CSC20230824/Rep2/Run2_Signals_euclidean_distance.Rds")
...
```{r fig.width=24, fig.height=6}
Run3_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/CSC20230824/Rep2/Run3_Signals_euclidean_distance.Rds")
...
```{r fig.width=24, fig.height=6}
Run4_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/CSC20230824/Rep2/Run4_Signals_euclidean_distance.Rds")
...
```{r fig.width=24, fig.height=6}
Run5_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/CSC20230824/Rep2/Run5_Signals_euclidean_distance.Rds")
select_sigs_Run5 <- KnnSelecting(signals = sgsignals[A == "Run5"], dists = Run5_dist, k = 5, noise = 0)
```
...
ggplot(sgsignals[A %in% "Run5" & ID %in% select_sigs_RemoveOutlier_Run5], aes(x = Blockade, y = DwellTime, colour = A)) + 
  geom_point(size = .5) + 
  geom_vline(xintercept = AABlockade[AA %in% AA_Run5, Blockade]) + 
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.1, 0.3)) + 
...
```{r}
paste0(sort(names(AMINO_ACID_CODE[AMINO_ACID_CODE %in% AA_Run4])), collapse = "")
```
...
Fit1 <- readRDS("./analysis/62.MachineLearning/01.ModelTraining/Version2/ModelACEGIMQSTW/model.Rds")
...
ggplot(sgsignals_pred[A == "Run5" & SignalCurrentPercent > 75 & Prob > 0.5], aes(x = Blockade, y = DwellTime, colour = Pred)) +
  ...
  geom_text(aes(label = Pred), size = 2) +
  scale_y_log10() +
  scale_x_continuous(breaks = AABlockade[AA %in% AA_Run5, Blockade],
                     labels = AABlockade[AA %in% AA_Run5, AA], limits = c(0.1, 0.3)) +
  labs(title = "Run5") +
  theme_classic(base_size = 15) +
  theme(legend.position = "none") -> p5

p1 + p2 + p3 + p4 + plot_layout(ncol = 5)
```
