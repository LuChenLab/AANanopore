...
```{r}
meta <- as.data.table(openxlsx::read.xlsx("data/ChenShanchuan/20230804/数据记录（0803）.xlsx"))
colnames(meta) <- c("file_name", "starttime", "endtime", "product", "goals", "sample", "baseline", "note")
meta <- meta[6:11, ]
```
...
```{r}
AA_Run1 <- c("Ser", "Leu", "CbC")
AA_Run2 <- c("Ser", "Leu", "CbC", "Thr", "Gln", "Trp")
AA_Run3 <- c("Ser", "Leu", "CbC", "Thr", "Gln", "Trp", "Ala", "Ile", "Pro")
AA_Run4 <- c("Ser", "Leu", "CbC", "Thr", "Gln", "Trp", "Ala", "Ile", "Pro", "Lys", "His")
```
...
bg_files <- grep(pattern = "20230803_0009", files, value = T)
sg_files <- files[!grepl(pattern = "20230803_0009", files)]

BaseMeanFiltering(SigFile = bg_files, starttime = 3.8, endtime = 7.56, outdir = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2", target = "Background")
BaseMeanFiltering(SigFile = sg_files[1], starttime = 0, endtime = 15.30, target = "Run1", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2", 
                  product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade]))

BaseMeanFiltering(SigFile = sg_files[2], starttime = 0, endtime = 10.50, target = "Run2", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2", 
                  product = as.numeric(AABlockade[AA %in% AA_Run2, Blockade]))

BaseMeanFiltering(SigFile = sg_files[3], starttime = 0, endtime = 12.70, target = "Run3", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2", 
                  product = as.numeric(AABlockade[AA %in% AA_Run3, Blockade]))

BaseMeanFiltering(SigFile = sg_files[4], starttime = 0, endtime = 11.08, target = "Run4", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2", 
                  product = as.numeric(AABlockade[AA %in% AA_Run4, Blockade]))

BaseMeanFiltering(SigFile = sg_files[5], starttime = 0, endtime = 11.30, target = "Run4", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2", 
                  product = as.numeric(AABlockade[AA %in% AA_Run4, Blockade]))
```
...
```{r}
files <- list.files("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2", "MainL0.txt", full.names = T, recursive = T)
...
saveRDS(Sig_Summa, file = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2/Sig_Summa.Rds")
```
...
lapply(paste0("Run", 1:4), function(x) {
  ...
  saveRDS(FeatureMatrix, file = paste0("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2/", x, "_Signals_FeatureMatrix.Rds"))
})

mclapply(paste0("Run", 1:4), function(x) {
  FeatureMatrix <- readRDS(paste0("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2/", x, "_Signals_FeatureMatrix.Rds"))
  spearmanCor = cor(t(FeatureMatrix), method = "spearman")
  spearmanCor <- as.dist(1 - spearmanCor)
  saveRDS(spearmanCor, file = paste0("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2/", x, "_Signals_spearman_correlation.Rds"))
}, mc.cores = 4)

mclapply(paste0("Run", 1:4), function(x) {
  FeatureMatrix <- readRDS(paste0("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2/", x, "_Signals_FeatureMatrix.Rds"))
  euclidean = stats::dist(FeatureMatrix, method = "euclidean")
  saveRDS(euclidean, file = paste0("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2/", x, "_Signals_euclidean_distance.Rds"))
}, mc.cores = 4)
```
...
Run1_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2/Run1_Signals_euclidean_distance.Rds")
select_sigs_Run1 <- KnnSelecting(signals = sgsignals[A == "Run1"], dists = Run1_dist, k = 3, noise = 0)
```

## Run2

```{r fig.width=24, fig.height=6}
Run2_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2/Run2_Signals_euclidean_distance.Rds")
...
```{r fig.width=24, fig.height=6}
Run3_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2/Run3_Signals_euclidean_distance.Rds")
...
```{r fig.width=24, fig.height=6}
Run4_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep2/Run4_Signals_euclidean_distance.Rds")
...
ggplot(sgsignals_pred[A == "Run1" & Prob > 0.5 & ID %in% Sig_Summa[ratio > 0.75, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Pred)) + 
  geom_vline(xintercept = AABlockade[AA %in% c("Thr", "Gln", "Trp"), Blockade]) + 
  geom_text(aes(label = Pred)) + 
  scale_y_log10() + 
  scale_x_continuous(breaks = AABlockade[AA %in% c("Thr", "Gln", "Trp"), Blockade], 
                     labels = AABlockade[AA %in% c("Thr", "Gln", "Trp"), AA], limits = c(0.1, 0.3)) + 
  labs(title = "Run1") + 
  theme_classic(base_size = 15) + 
  theme(legend.position = "none") -> p1

ggplot(sgsignals_pred[A == "Run2" & Prob > 0.5 & ID %in% Sig_Summa[ratio > 0.75, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Pred)) + 
  geom_vline(xintercept = AABlockade[AA %in% c("Thr", "Gln", "Trp", "Ser", "Leu", "CbC"), Blockade]) + 
  geom_text(aes(label = Pred)) + 
  scale_y_log10() + 
  scale_x_continuous(breaks = AABlockade[AA %in% c("Thr", "Gln", "Trp", "Ser", "Leu", "CbC"), Blockade], 
                     labels = AABlockade[AA %in% c("Thr", "Gln", "Trp", "Ser", "Leu", "CbC"), AA], limits = c(0.1, 0.3)) + 
  labs(title = "Run2") + 
  theme_classic(base_size = 15) + 
  theme(legend.position = "none") -> p2

ggplot(sgsignals_pred[A == "Run3" & Prob > 0.5 & ID %in% Sig_Summa[ratio > 0.75, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Pred)) + 
  geom_vline(xintercept = AABlockade[AA %in% c("Thr", "Gln", "Trp", "Ser", "Leu", "Ala", "Ile", "Pro", "CbC"), Blockade]) + 
  geom_text(aes(label = Pred)) + 
  scale_y_log10() + 
  scale_x_continuous(breaks = AABlockade[AA %in% c("Thr", "Gln", "Trp", "Ser", "Leu", "Ala", "Ile", "Pro", "CbC"), Blockade], 
                     labels = AABlockade[AA %in% c("Thr", "Gln", "Trp", "Ser", "Leu", "Ala", "Ile", "Pro", "CbC"), AA], limits = c(0.1, 0.3)) + 
  labs(title = "Run3") + 
  theme_classic(base_size = 15) + 
  theme(legend.position = "none") -> p3

ggplot(sgsignals_pred[A == "Run4" & Prob > 0.5 & ID %in% Sig_Summa[ratio > 0.75, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Pred)) + 
  geom_vline(xintercept = AABlockade[AA %in% c("Thr", "Gln", "Trp", "Ser", "Leu", "Ala", "Ile", "Pro", "Lys", "His", "CbC"), Blockade]) + 
  geom_text(aes(label = Pred)) + 
  scale_y_log10() + 
  scale_x_continuous(breaks = AABlockade[AA %in% c("Thr", "Gln", "Trp", "Ser", "Leu", "Ala", "Ile", "Pro", "Lys", "His", "CbC"), Blockade], 
                     labels = AABlockade[AA %in% c("Thr", "Gln", "Trp", "Ser", "Leu", "Ala", "Ile", "Pro", "Lys", "His", "CbC"), AA], limits = c(0.1, 0.3)) + 
  labs(title = "Run4") + 
  theme_classic(base_size = 15) + 
...
abf1 <- readRDS(paste0("./analysis/21.ABFProcessing/03.MixtureAA/ABF/ABF_", "20230803_0005", ".Rds"))
...
aa_run1_i <- AABlockade[AA %in% c("Thr", "Gln", "Trp"), ]
...
ggplot() + 
  ...
  # scale_y_continuous(n.breaks = 4, limits = c(abf_run1_i[, max(pA) * 0.5], abf_run1_i[, max(pA)])) + 
...
```{r fig.height=5, fig.width=24}
abf2 <- readRDS(paste0("./analysis/21.ABFProcessing/03.MixtureAA/ABF/ABF_", "20230803_0006", ".Rds"))
...
aa_run2_i <- AABlockade[AA %in% c("Thr", "Gln", "Trp", "Ser", "Leu", "CbC"), ]
...
```{r fig.height=5, fig.width=24}
abf3 <- readRDS(paste0("./analysis/21.ABFProcessing/03.MixtureAA/ABF/ABF_", "20230803_0007", ".Rds"))
...
aa_run3_i <- AABlockade[AA %in% c("Thr", "Gln", "Trp", "Ser", "Leu", "Ala", "Ile", "Pro", "CbC"), ]
aa_run3_i[, y := L0 * (1 - Blockade)]

ggplot() + 
  geom_hline(yintercept = aa_run3_i[, y], colour = c(RColorBrewer::brewer.pal(n = 6, name = "Dark2"), RColorBrewer::brewer.pal(n = 3, name = "Set1"))) +
...
```{r fig.height=5, fig.width=24}
abf4 <- readRDS(paste0("./analysis/21.ABFProcessing/03.MixtureAA/ABF/ABF_", "20230803_0008", ".Rds"))
...
aa_run4_i <- AABlockade[AA %in% c("Thr", "Gln", "Trp", "Ser", "Leu", "Ala", "Ile", "Pro", "Lys", "His", "CbC"), ]
aa_run4_i[, y := L0 * (1 - Blockade)]

ggplot() + 
  geom_hline(yintercept = aa_run4_i[, y], colour = c(RColorBrewer::brewer.pal(n = 8, name = "Dark2"), RColorBrewer::brewer.pal(n = 4, name = "Set1"))) +
