...
```{r}
meta <- as.data.table(openxlsx::read.xlsx("data/ChenShanchuan/20230803/数据记录（0803）.xlsx"))
...
```{r}
AA_Run1 <- c("Thr", "Gln", "Trp")
AA_Run2 <- c("Thr", "Gln", "Trp", "Ser", "Leu", "CbC")
AA_Run3 <- c("Thr", "Gln", "Trp", "Ser", "Leu", "Ala", "Ile", "Pro", "CbC")
AA_Run4 <- c("Thr", "Gln", "Trp", "Ser", "Leu", "Ala", "Ile", "Pro", "Lys", "His", "CbC")
```

```{r}
files <- list.files("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal", full.names = TRUE)
files <- grep(pattern = "202308", files, value = T)
files <- files[gsub("RawSignal_", "", gsub(".txt", "", basename(files))) %in% meta$file_name]

bg_files <- grep(pattern = "20230803_0004", files, value = T)
sg_files <- files[!grepl(pattern = "20230803_0004", files)]

BaseMeanFiltering(SigFile = bg_files, starttime = 3.8, endtime = 6.7, outdir = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1", target = "Background")
BaseMeanFiltering(SigFile = sg_files[1], starttime = 0, endtime = 20, target = "Run1", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1", 
                  product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade]))

BaseMeanFiltering(SigFile = sg_files[2], starttime = 0, endtime = 12.12, target = "Run2", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1", 
                  product = as.numeric(AABlockade[AA %in% AA_Run2, Blockade]))

BaseMeanFiltering(SigFile = sg_files[3], starttime = 0, endtime = 13.44, target = "Run3", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1", 
                  product = as.numeric(AABlockade[AA %in% AA_Run3, Blockade]))

BaseMeanFiltering(SigFile = sg_files[4], starttime = 0, endtime = 17, target = "Run4", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1", 
...
```{r}
files <- list.files("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1", "MainL0.txt", full.names = T, recursive = T)
...
saveRDS(Sig_Summa, file = "./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1/Sig_Summa.Rds")
```
...
lapply(paste0("Run", 1:4), function(x) {
  ...
  saveRDS(FeatureMatrix, file = paste0("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1/", x, "_Signals_FeatureMatrix.Rds"))
})

mclapply(paste0("Run", 1:4), function(x) {
  FeatureMatrix <- readRDS(paste0("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1/", x, "_Signals_FeatureMatrix.Rds"))
  spearmanCor = cor(t(FeatureMatrix), method = "spearman")
  spearmanCor <- as.dist(1 - spearmanCor)
  saveRDS(spearmanCor, file = paste0("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1/", x, "_Signals_spearman_correlation.Rds"))
}, mc.cores = 4)

mclapply(paste0("Run", 1:4), function(x) {
  FeatureMatrix <- readRDS(paste0("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1/", x, "_Signals_FeatureMatrix.Rds"))
  euclidean = stats::dist(FeatureMatrix, method = "euclidean")
  saveRDS(euclidean, file = paste0("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1/", x, "_Signals_euclidean_distance.Rds"))
}, mc.cores = 4)
```
...
Run1_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1/Run1_Signals_euclidean_distance.Rds")
select_sigs_Run1 <- KnnSelecting(signals = sgsignals[A == "Run1"], dists = Run1_dist, k = 4, noise = 0)
```

## Run2

```{r fig.width=24, fig.height=6}
Run2_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1/Run2_Signals_euclidean_distance.Rds")
...
```{r fig.width=24, fig.height=6}
Run3_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1/Run3_Signals_euclidean_distance.Rds")
...
```{r fig.width=24, fig.height=6}
Run4_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/WZC20230804/Rep1/Run4_Signals_euclidean_distance.Rds")
...
select_sigs_RemoveOutlier_Run1 <- RemoveOutlier(tree = res.hc, n = 40, Keep = 0.8)
