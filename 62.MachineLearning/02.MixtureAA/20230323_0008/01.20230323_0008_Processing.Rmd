...
meta <- meta[file_name == "20230323_0008"]
meta
```
...
BaseMeanFiltering(SigFile = sg_files[1], starttime = 2.3, endtime = 23.1, target = "LWQST", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/20230323_0008", 
...
```{r}
files <- list.files("./analysis/62.MachineLearning/02.MixtureAA/20230323_0008", "MainL0.txt", full.names = T, recursive = T)
...
saveRDS(Sig_Summa, file = "./analysis/62.MachineLearning/02.MixtureAA/20230323_0008/Sig_Summa.Rds")
```
...
saveRDS(euclidean, file = paste0("./analysis/62.MachineLearning/02.MixtureAA/20230323_0008/Signals_euclidean_distance.Rds"))
```
...
Run1_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/20230323_0008/Signals_euclidean_distance.Rds")
...
select_sigs_RemoveOutlier_Run1 <- RemoveOutlier(tree = res.hc, n = 100, Keep = 0.8)
...
fwrite(Selected_sigs, "./analysis/62.MachineLearning/02.MixtureAA/20230323_0008/Selected_sigs.txt", sep = "\t", quote = F)
```
...
fwrite(Selected_sigs2, "./analysis/62.MachineLearning/02.MixtureAA/20230323_0008/Selected_sigs_summary.txt", sep = "\t", quote = F)
```
...
```{r fig.height=5, fig.width=24}
time1 <- 247
abf_run1_i <- abf1[Time >= time1 & Time <= time1 + 1, ]
L0 <- MainRidge(x = abf_run1_i[, pA], bw = 0.5, peaks = 1, CI = 0.95, plot = F)[[1]]
aasumm <- SigSumm[A %in% AA_Run1]
aasumm[, ymin := L0 * (1 - BlockadeMean - BlockadeSD)]
aasumm[, ymax := L0 * (1 - BlockadeMean + BlockadeSD)]
