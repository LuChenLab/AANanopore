...
```{r fig.width=10, fig.height=4}
select_sigs_RemoveOutlier_Run2 <- RemoveOutlier(tree = res.hc, n = 10, Keep = 0.9)
...
```{r fig.width=10, fig.height=4}
select_sigs_RemoveOutlier_Run3 <- RemoveOutlier(tree = res.hc, n = 10, Keep = 0.9)
...
```{r fig.width=10, fig.height=4}
select_sigs_RemoveOutlier_Run4 <- RemoveOutlier(tree = res.hc, n = 10, Keep = 0.9)
...
Sig_Current <- lapply(B_file, function(x) {
  readRDS(list.files(path = "./analysis/21.ABFProcessing/03.MixtureAA/SignalCurrent", x, full.names = TRUE))[ID %in% select_sigs]
})
...
ggplot(sgsignals_pred[A == "Run2" & Prob > 0.5 & ID %in% Sig_Summa[ratio > 0.9, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Pred)) + 
...
ggplot(sgsignals_pred[A == "Run3" & Prob > 0.5 & ID %in% Sig_Summa[ratio > 0.5, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Pred)) + 
  geom_vline(xintercept = AABlockade[AA %in% c("Ser", "Leu", "Glu", "Thr", "Gln", "Trp"), Blockade]) + 
  geom_text(aes(label = Pred)) + 
  scale_y_log10() + 
  scale_x_continuous(breaks = AABlockade[AA %in% c("Ser", "Leu", "Glu", "Thr", "Gln", "Trp"), Blockade], 
