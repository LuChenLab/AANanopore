...
```{r}
meta0 <- as.data.table(openxlsx::read.xlsx("data/ChenShanchuan/20230822/副本数据记录20230822.xlsx", sheet = 2))
colnames(meta0) <- c("file_name", "start_time", "end_time", "amino_acid", "purpose", "level", "sample", "base_line", "note", "temp")
meta <- meta0[file_name %in% gsub(".abf", "", list.files("./data/ChenShanchuan/20230822", "abf"))]
```

```{r}
AABlockade <- lapply(list.files("./analysis/61.SignalSelecting/01.StandardAA/02.SelectedSignals", "_V5.Rds", full.names = T), function(x) readRDS(x)$Summary)
AABlockade <- do.call(rbind, AABlockade)[State == "State1"]
```

```{r}
AA_Run1 <- c("Gln", "CbC", "Thr", "Ala")
AA_Run2 <- c("Gln", "CbC", "Thr", "Ala", "Ile", "Phe", "Leu")
AA_Run3 <- c("Gln", "CbC", "Thr", "Ala", "Ile", "Phe", "Leu", "Ser", "Trp")
AA_Run4 <- c("Gln", "CbC", "Thr", "Ala", "Ile", "Phe", "Leu", "Ser", "Trp", "Glu")
```
...
bg_files <- grep(pattern = "20230822_0003", files, value = T)
sg_files <- files[!grepl(pattern = "20230822_0003", files)]

BaseMeanFiltering(SigFile = bg_files, starttime = 0, endtime = 5.5, outdir = "./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1", target = "Background")

BaseMeanFiltering(SigFile = sg_files[1], starttime = 0, endtime = 23, target = "Run1", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1", 
                  product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade]))

BaseMeanFiltering(SigFile = sg_files[2], starttime = 0, endtime = 11.5, target = "Run2", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1", 
                  product = as.numeric(AABlockade[AA %in% AA_Run2, Blockade]))

BaseMeanFiltering(SigFile = sg_files[3], starttime = 0, endtime = 11.5, target = "Run3", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1", 
                  product = as.numeric(AABlockade[AA %in% AA_Run3, Blockade]))

# BaseMeanFiltering(SigFile = sg_files[3], starttime = 12, endtime = 24.8, target = "Run4",
#                   outdir = "./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1",
#                   product = as.numeric(AABlockade[AA %in% AA_Run4[10], Blockade])) # 没信号
```

```{r}
BaseMeanFiltering(SigFile = "./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_20230822_0006_2.txt", 
                  starttime = 12, endtime = 25, target = "Run4", 
                  outdir = "./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1", 
                  product = as.numeric(AABlockade[AA %in% AA_Run4, Blockade]))
```

# Remain sigle main peak signals

```{r}
files <- list.files("./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1", "MainL0.txt", full.names = T, recursive = T)
...
B_file <- paste0("./analysis/21.ABFProcessing/03.MixtureAA/SignalCurrent/SignalCurrent_", B_file, ".Rds")
...
saveRDS(Sig_Summa, file = "./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1/Sig_Summa.Rds")
```
...
abline(v = AABlockade[AA %in% AA_Run3, 1 - Blockade], col = 2)
```
...
sgsignals <- sigs[A != "Background", ]
B_file <- sgsignals[, unique(File)]
B_file <- paste0("./analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_", B_file, ".txt")
...
lapply(paste0("Run", 1:4), function(x) {
  ...
  saveRDS(FeatureMatrix, file = paste0("./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1/", x, "_Signals_FeatureMatrix.Rds"))
})

mclapply(paste0("Run", 1:4), function(x) {
  FeatureMatrix <- readRDS(paste0("./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1/", x, "_Signals_FeatureMatrix.Rds"))
  euclidean = stats::dist(FeatureMatrix, method = "euclidean")
  saveRDS(euclidean, file = paste0("./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1/", x, "_Signals_euclidean_distance.Rds"))
}, mc.cores = 4)
```
...
Run1_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1/Run1_Signals_euclidean_distance.Rds")
...
```{r fig.width=24, fig.height=6}
Run2_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1/Run2_Signals_euclidean_distance.Rds")
select_sigs_Run2 <- KnnSelecting(signals = sgsignals[A == "Run2"], dists = Run2_dist, k = 5, noise = 0)
```

## Run3

```{r fig.width=24, fig.height=6}
Run3_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1/Run3_Signals_euclidean_distance.Rds")
select_sigs_Run3 <- KnnSelecting(signals = sgsignals[A == "Run3"], dists = Run3_dist, k = 5, noise = 0)
```

## Run4

```{r fig.width=24, fig.height=6}
Run4_dist <- readRDS("./analysis/62.MachineLearning/02.MixtureAA/CSC20230822/Rep1/Run4_Signals_euclidean_distance.Rds")
select_sigs_Run4 <- KnnSelecting(signals = sgsignals[A == "Run4"], dists = Run4_dist, k = 5, noise = 0)
```
...
select_sigs_RemoveOutlier_Run1 <- RemoveOutlier(tree = res.hc, n = 5, Keep = 0.99)
ggplot(sgsignals[ID %in% select_sigs_RemoveOutlier_Run1], aes(x = Blockade, y = DwellTime, colour = A)) + 
  geom_point(size = .5) + 
  geom_vline(xintercept = AABlockade[AA %in% AA_Run1, Blockade]) + 
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.1, 0.3)) + 
...
```{r fig.width=10, fig.height=4}
select_sigs_RemoveOutlier_Run2 <- RemoveOutlier(tree = res.hc, n = 5, Keep = 0.99)
ggplot(sgsignals[ID %in% select_sigs_RemoveOutlier_Run2], aes(x = Blockade, y = DwellTime, colour = A)) + 
  geom_point(size = .5) + 
  geom_vline(xintercept = AABlockade[AA %in% AA_Run2, Blockade]) + 
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.1, 0.3)) + 
...
```{r fig.width=10, fig.height=4}
select_sigs_RemoveOutlier_Run3 <- RemoveOutlier(tree = res.hc, n = 5, Keep = 0.99)
ggplot(sgsignals[ID %in% select_sigs_RemoveOutlier_Run3], aes(x = Blockade, y = DwellTime, colour = A)) + 
  geom_point(size = .5) + 
  geom_vline(xintercept = AABlockade[AA %in% AA_Run3, Blockade]) + 
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.1, 0.3)) + 
...
```{r fig.width=10, fig.height=4}
select_sigs_RemoveOutlier_Run4 <- RemoveOutlier(tree = res.hc, n = 3, Keep = 0.99)
...
Fit1 <- readRDS("./analysis/62.MachineLearning/01.ModelTraining/Version2/ModelACEFILQSTW/model.Rds")
...
ggplot(sgsignals_pred[A == "Run1" & Prob > 0.5 & ID %in% Sig_Summa[ratio > 0.75, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Pred)) + 
  ...
  scale_y_log10() + 
  scale_x_continuous(breaks = AABlockade[AA %in% AA_Run1, Blockade], 
                     labels = AABlockade[AA %in% AA_Run1, AA], limits = c(0.1, 0.3)) + 
  labs(title = "Run1") + 
  theme_classic(base_size = 15) + 
  theme(legend.position = "none") -> p1
...
ggplot(sgsignals_pred[A == "Run2" & Prob > 0.5 & ID %in% Sig_Summa[ratio > 0.75, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Pred)) + 
  ...
  geom_text(aes(label = Pred)) + 
  scale_y_log10() + 
  scale_x_continuous(breaks = AABlockade[AA %in% AA_Run2, Blockade], 
                     labels = AABlockade[AA %in% AA_Run2, AA], limits = c(0.1, 0.3)) + 
  labs(title = "Run2") + 
  theme_classic(base_size = 15) + 
...
ggplot(sgsignals_pred[A == "Run3" & Prob > 0.5 & ID %in% Sig_Summa[ratio > 0.75, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Pred)) + 
  ...
  geom_text(aes(label = Pred)) + 
  scale_y_log10() + 
  scale_x_continuous(breaks = AABlockade[AA %in% AA_Run3, Blockade], 
                     labels = AABlockade[AA %in% AA_Run3, AA], limits = c(0.1, 0.3)) + 
  labs(title = "Run3") + 
  theme_classic(base_size = 15) + 
  theme(legend.position = "none") -> p3

ggplot(sgsignals_pred[A == "Run4" & Prob > 0.5 & ID %in% Sig_Summa[ratio > 0.75, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Pred)) +
  geom_vline(xintercept = AABlockade[AA %in% AA_Run4, Blockade]) +
  geom_text(aes(label = Pred)) +
  scale_y_log10() +
  scale_x_continuous(breaks = AABlockade[AA %in% AA_Run4, Blockade],
                     labels = AABlockade[AA %in% AA_Run4, AA], limits = c(0.1, 0.3)) +
  labs(title = "Run4") +
  theme_classic(base_size = 15) +
...
```{r}
aasumm <- SigSumm[, .(A, start = 1 - BlockadeMean - BlockadeSD, end = 1 - BlockadeMean + BlockadeSD)]
SigCur <- lapply(sigs[, unique(File)], function(x) {
  readRDS(paste0("./analysis/21.ABFProcessing/03.MixtureAA/SignalCurrent/SignalCurrent_", x, ".Rds"))
})
SigCur <- do.call(rbind, SigCur)[ID %in% select_sigs]
...
sgsignals_curt <- merge(sgsignals, SigCur[, .(ID, Within, WithinRatio, DiffRatio)], by = "ID")
```

```{r fig.width=24, fig.height=5}
ggplot(sgsignals_curt[A == "Run1" & ID %in% Sig_Summa[ratio > 0.75, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Within)) + 
  geom_vline(xintercept = AABlockade[AA %in% AA_Run1, Blockade]) + 
...
ggplot(sgsignals_curt[A == "Run2" & ID %in% Sig_Summa[ratio > 0.75, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Within)) + 
  geom_vline(xintercept = AABlockade[AA %in% AA_Run2, Blockade]) + 
  geom_text(aes(label = Within)) + 
  scale_y_log10() + 
  scale_x_continuous(breaks = AABlockade[AA %in% AA_Run2, Blockade], 
...
ggplot(sgsignals_curt[A == "Run3" & ID %in% Sig_Summa[ratio > 0.75, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Within)) + 
  geom_vline(xintercept = AABlockade[AA %in% AA_Run3, Blockade]) + 
  geom_text(aes(label = Within)) + 
  scale_y_log10() + 
  scale_x_continuous(breaks = AABlockade[AA %in% AA_Run3, Blockade], 
...
ggplot(sgsignals_curt[A == "Run4" & ID %in% Sig_Summa[ratio > 0.75, unique(ID)]], aes(x = Blockade, y = DwellTime, colour = Within)) +
  geom_vline(xintercept = AABlockade[AA %in% AA_Run4, Blockade]) +
  geom_text(aes(label = Within)) +
  scale_y_log10() +
  scale_x_continuous(breaks = AABlockade[AA %in% AA_Run4, Blockade],
...
abf1 <- readRDS(paste0("./analysis/21.ABFProcessing/03.MixtureAA/ABF/ABF_", "20230822_0004", ".Rds"))
...
abf2 <- readRDS(paste0("./analysis/21.ABFProcessing/03.MixtureAA/ABF/ABF_", "20230822_0005", ".Rds"))
time2 <- sgsignals_pred[A == "Run2", .N, round(StartTime)][order(N, decreasing = T)][1, round]
abf_run2_i <- abf2[Time >= time2 - 0.5 & Time <= time2 + 0.5, ]
L0 <- MainRidge(x = abf_run2_i[, pA], bw = 0.5, peaks = 1, CI = 0.95, plot = F)[[1]]
aasumm2 <- SigSumm[A %in% AA_Run2]
...
```{r fig.height=10, fig.width=24}
abf3 <- readRDS(paste0("./analysis/21.ABFProcessing/03.MixtureAA/ABF/ABF_", "20230822_0006", ".Rds"))
...
abf4 <- readRDS(paste0("./analysis/21.ABFProcessing/03.MixtureAA/ABF/ABF_", "20230822_0006_2", ".Rds"))
time4 <- sgsignals_pred[A == "Run4", .N, round(StartTime)][order(N, decreasing = T)][2, round]
abf_run4_i <- abf4[Time >= time4 - 0.5 & Time <= time4 + 0.5, ]
L0 <- MainRidge(x = abf_run4_i[, pA], bw = 0.5, peaks = 1, CI = 0.95, plot = F)[[1]]
aasumm4 <- SigSumm[A %in% AA_Run4]
...
ggplot(retime[Pred %in% AA_Run4]) +
  stat_ecdf(aes(Bini, colour = Pred)) + 
  scale_colour_manual(breaks = names(aa_cols), values = aa_cols, guide = guide_legend(ncol = 1)) + 
  # geom_rect(data = ran_gr, aes(xmin = start, xmax = end, ymin = 0, ymax = 1, fill = Run), alpha = 0.3) + 
  labs(x = "Relative time", y = "Cumulative distribution probablity") + 
