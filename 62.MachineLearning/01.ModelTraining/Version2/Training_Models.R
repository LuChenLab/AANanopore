...
ms <- list(c("Gln", "CbC", "Thr", "Ala", "Ile", "Phe", "Leu", "Ser", "Trp", "Glu"), 
           c("Gly", "Ser", "Ala", "Thr", "Gln", "Met", "Ile", "Trp", "Glu", "CbC"), 
           c("Gly", "Ser", "Ala", "Thr", "Gln", "Met", "Ile", "Trp", "Arg", "CbC"), 
           c("Gly", "Ser", "Ala", "Thr", "Gln", "Met", "Ile", "Trp", "Arg", "Glu", "CbC"), 
           c("Asn", "Gln", "Ile", "Leu", "Phe", "Ser", "Thr", "Trp"), 
           c("Asp", "Gln", "Glu", "Leu", "Ser", "Thr", "Trp"), 
           c("Gln", "Leu", "Ser", "Thr", "Trp"), 
           c("Leu", "Ser", "Tyr", "Phe"), 
           c("Leu", "His", "Phe"), 
           c("Leu", "Glu", "Phe"))

lapply(ms, function(AAs) {
  print(AAs)
  aas <- names(AMINO_ACID_CODE[AMINO_ACID_CODE %in% AAs])
  if(is.element("CbC", AAs)) aas <- c(aas, "C")
  outdir <- paste0("./analysis/62.MachineLearning/01.ModelTraining/Version2/Model", paste0(sort(aas), collapse = ""))
  if(!dir.exists(outdir)) dir.create(outdir)
  
  if(!file.exists(file.path(outdir, "modeling_data.RData"))) {
    ...
    names(Sigs) <- gsub("_State_Signals_V5.Rds", "", basename(files))
    ...
    set.seed(123)
    trainIndex <- createDataPartition(Sigs$AA, 
                                      p = .8, 
                                      list = FALSE, 
                                      times = 1)
...
lapply(ms, function(AAs) {
  ...
  outdir <- paste0("./analysis/62.MachineLearning/01.ModelTraining/Version2/Model", paste0(sort(aas), collapse = ""))
  if(!dir.exists(outdir)) dir.create(outdir)
  if(!file.exists(file.path(outdir, "model.Rds"))) {
    load(file.path(outdir, "modeling_data.RData"))
    set.seed(825)
    Fit1 <- train(Class ~ ., data = Sigs_Train_Upsample_FM, 
                  ...
                  metric = "Accuracy", 
                  allowParallel = TRUE)
    saveRDS(Fit1, file.path(outdir, "model.Rds"))
  }
})
