...
```{r}
meta0 <- as.data.table(openxlsx::read.xlsx("./data/实验信息表1101.xlsx"))
colnames(meta0) <- c("file_name", "date", "amino_acid", "concentration", "start_time", "end_time", "type", "baseline_mean")
meta0[, start_time := as.numeric(start_time)]
meta0[, end_time := as.numeric(end_time)]
meta0[concentration == 0, product := "noise"]
meta0[amino_acid == "α-Bag Cell Peptide (1-9) APRLRFYSL", product := "FYSL"]
meta0[amino_acid == "ACTH (18-39), human, RPVKVYPNGAEDESAEAFPLEF", product := "LEF"]
meta0[amino_acid == "Angiotensin I, human, DRVYIHPFHL", product := "FHL"]
meta0 <- meta0[19:31, ]
meta0
```

```{r}
meta <- meta0[product != "noise"]
meta
```

```{r}
(AA_Run1 <- AMINO_ACID_CODE[strsplit(meta$product[1], "")[[1]]])
```
...
stopifnot(identical(gsub(".txt", "", gsub("RawSignal_", "", basename(sg_files))), meta$file_name))

# BaseMeanFiltering(SigFile = sg_files[1], starttime = meta$start_time[1], endtime = meta$end_time[1], target = meta$product[1],
#                   outdir = "./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFHL",
#                   product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade]))

BaseMeanFiltering(SigFile = sg_files[2], starttime = meta$start_time[2], endtime = meta$end_time[2], target = meta$product[2], 
                  outdir = "./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFHL", 
                  product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade]))

BaseMeanFiltering(SigFile = sg_files[3], starttime = meta$start_time[3], endtime = meta$end_time[3], target = meta$product[3], 
                  outdir = "./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFHL", 
                  product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade]))

BaseMeanFiltering(SigFile = sg_files[4], starttime = meta$start_time[4], endtime = meta$end_time[4], target = meta$product[5],
                  outdir = "./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFHL",
                  product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade]))

BaseMeanFiltering(SigFile = sg_files[5], starttime = meta$start_time[5], endtime = meta$end_time[5], target = meta$product[5],
                  outdir = "./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFHL",
                  product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade]))

BaseMeanFiltering(SigFile = sg_files[6], starttime = meta$start_time[6], endtime = meta$end_time[6], target = meta$product[6], 
                  outdir = "./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFYSL", 
                  product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade])) # 很好

BaseMeanFiltering(SigFile = sg_files[7], starttime = meta$start_time[7], endtime = meta$end_time[7], target = meta$product[7], 
                  outdir = "./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFYSL", 
                  product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade]))

BaseMeanFiltering(SigFile = sg_files[8], starttime = meta$start_time[8], endtime = meta$end_time[8], target = meta$product[8], 
                  outdir = "./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFYSL", 
                  product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade])) # 很好

BaseMeanFiltering(SigFile = sg_files[9], starttime = meta$start_time[9], endtime = meta$end_time[9], target = meta$product[9], 
                  outdir = "./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFYSL", 
                  product = as.numeric(AABlockade[AA %in% AA_Run1, Blockade]))
```

```{r}
files <- list.files("./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFHL", "MainL0.txt", full.names = T, recursive = T)
sigs <- lapply(files, fread)
sigs <- do.call(rbind, sigs)
sigs$File <- stringr::str_remove_all(sigs$ID, "_([[:digit:]]+)$")
sigs <- sigs[Blockade < 0.3 & DwellTime > 0.3]
```
...
ggplot(data = sigs[Blockade < 0.3]) + 
  geom_line(aes(x = Blockade, colour = File), stat = "density", bw = 0.001) + 
  geom_rect(data = SigSumm[A %in% AA_Run1, ], aes(xmin = BlockadeMean - BlockadeSD, xmax = BlockadeMean + BlockadeSD, ymin = 0, ymax = 16, fill = A), alpha = 0.3) +
  scale_fill_manual(breaks = names(aa_cols), values = aa_cols, guide = guide_legend(title = element_blank())) + 
  facet_wrap(~File, ncol = 1, strip.position = "right")
```
...
saveRDS(Sig_Summa, file = "./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFHL/Sig_Summa.Rds")
```

```{r}
Sig_Summa <- readRDS("./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFHL/Sig_Summa.Rds")
```
...
if(nrow(bgsignals) > 0) {
  ...
  Big_FeatureMatrix <- list.files(path = "./analysis/21.ABFProcessing/04.PolypeptideSequencing/PolypeptideFHL/FeatureMatrix", B_file, full.names = TRUE)
...
saveRDS(euclidean, file = paste0("./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFHL/Signals_euclidean_distance.Rds"))
```
...
Run1_dist <- readRDS("./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFHL/Signals_euclidean_distance.Rds")
select_sigs_Run1 <- KnnSelecting(signals = sgsignals, dists = Run1_dist, k = 10, noise = 0)
```
...
```{r}
Selected_sigs <- sgsignals[ID %in% select_sigs_RemoveOutlier_Run1][Blockade > 0.15]
fwrite(Selected_sigs, "./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFHL/Selected_sigs.txt", sep = "\t", quote = F)
```

```{r fig.width=20, fig.height=7}
ggplot(Selected_sigs) + 
  geom_point(aes(x = Blockade, y = DwellTime), size = .5, colour = "grey") + 
  geom_rect(data = SigSumm[A %in% AA_Run1, ], aes(xmin = BlockadeMean - BlockadeSD, xmax = BlockadeMean + BlockadeSD, ymin = 0, ymax = 110, fill = A), alpha = 0.3) +
  scale_y_log10() + 
  scale_fill_manual(breaks = names(aa_cols), values = aa_cols, guide = guide_legend(title = element_blank())) + 
...
Fit1 <- readRDS("./analysis/62.MachineLearning/01.ModelTraining/ModelFHL/model.Rds")
...
fwrite(Selected_sigs2, "./analysis/62.MachineLearning/04.PolypeptideSequencing/PolypeptideFHL/Selected_sigs_summary.txt", sep = "\t", quote = F)
```


```{r}
ggplot(data = Selected_sigs2) + 
  geom_text(aes(x = Blockade, y = DwellTime, colour = Within, label = Within), size = 1) +
  geom_rect(data = SigSumm[A %in% AA_Run1, ], aes(xmin = BlockadeMean - BlockadeSD, xmax = BlockadeMean + BlockadeSD, ymin = 0, ymax = 110, fill = A), alpha = 0.3) +
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.1, 0.23)) + 
  scale_fill_manual(breaks = names(aa_cols), values = aa_cols, guide = guide_legend(title = element_blank())) + 
