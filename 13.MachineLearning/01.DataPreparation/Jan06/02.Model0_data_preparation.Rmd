...
```{r}
AA_Sig <- readRDS("./analysis/13.MachineLearning/01.DataPreparation/Jan06/Feature_Matrix_of_AA_Signal.Rds")
```
...
```{r}
Blank_Sig <- readRDS("./analysis/13.MachineLearning/01.DataPreparation/Jan06/Feature_Matrix_of_Blank_Signal.Rds")
```
...
Train[, ID := NULL]
Train <- data.frame(Train[, 2:509], Class = factor(Train[[1]], levels = c(sort(Mat[, unique(amino_acid)]), "Noise")))

saveRDS(Train, "./analysis/13.MachineLearning/01.DataPreparation/Jan06/Model0/L1_Train.Rds")
saveRDS(Test, "./analysis/13.MachineLearning/01.DataPreparation/Jan06/Model0/L1_Test.Rds")
saveRDS(Valid, "./analysis/13.MachineLearning/01.DataPreparation/Jan06/Model0/L1_Valid.Rds")
```
...
saveRDS(Train, "./analysis/13.MachineLearning/01.DataPreparation/Jan06/Model0/L2_Train.Rds")
saveRDS(Test, "./analysis/13.MachineLearning/01.DataPreparation/Jan06/Model0/L2_Test.Rds")
saveRDS(Valid, "./analysis/13.MachineLearning/01.DataPreparation/Jan06/Model0/L2_Valid.Rds")
```
...
```{r}
Mat <- rbind(L1_Mat, L2_Mat)
Vali_Set <- Mat[, .N, .(amino_acid, file_name)][N > 20, .SD[which.min(N)], amino_acid]
Vali_Set <- merge(Vali_Set[, .(amino_acid, file_name)], Mat[, .(amino_acid, ID, file_name)], by = c("amino_acid", "file_name"))

Valid <- AA_Sig[ID %in% Vali_Set$ID]
Mat <- AA_Sig[!ID %in% Vali_Set$ID]

set.seed(3456)
Train <- Mat[, .SD[sample(.N, .N * 0.8), ], amino_acid]
set.seed(3456)
Train <- rbind(Train[amino_acid %in% Train[, .N, amino_acid][N >= 2000, amino_acid], .SD[sample(.N, 2000), ], amino_acid], 
               Train[amino_acid %in% Train[, .N, amino_acid][N < 2000, amino_acid], .SD[sample(.N, 2000, replace = T), ], amino_acid])
Test <- Mat[!ID %in% Train$ID]


set.seed(3456)
Sig0 <- Blank_Sig[, .SD[sample(.N, 2000), ]]
Sig0[, amino_acid := "Noise"]
Sig0 <- Sig0[, .SD[sample(.N, 2000), ], amino_acid]

Train <- rbind(Train, Sig0)
Train[, ID := NULL]
Train <- data.frame(Train[, 2:509], Class = factor(Train[[1]], levels = c(sort(Mat[, unique(amino_acid)]), "Noise")))

saveRDS(Train, "./analysis/13.MachineLearning/01.DataPreparation/Jan06/Model0/L12_Train.Rds")
saveRDS(Test, "./analysis/13.MachineLearning/01.DataPreparation/Jan06/Model0/L12_Test.Rds")
saveRDS(Valid, "./analysis/13.MachineLearning/01.DataPreparation/Jan06/Model0/L12_Valid.Rds")
```
