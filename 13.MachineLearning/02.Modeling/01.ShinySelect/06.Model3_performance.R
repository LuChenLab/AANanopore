...
RFC <- readRDS("./analysis/13.MachineLearning/01.DataPreparation/01.ShinySelect/Model3/RF_Fit.Rds")
...
fm <- fread("./analysis/11.SignalIdentification/FeatureMatrix/FeatureMatrix_20230331 _0003.txt")
fm <- data.table(ID = fm[, ID], M1Pred = predict(RFC, fm), M1Prob = apply(predict(RFC, fm, type = "prob"), 1, max))
dataset <- fread("./analysis/11.SignalIdentification/Signal/RawSignal_20230331 _0003.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "LVFGK")
