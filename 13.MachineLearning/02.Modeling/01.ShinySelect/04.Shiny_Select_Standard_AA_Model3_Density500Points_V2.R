...
Selected_Sig <- Selected_Sig[!A %in% c("Cys", "Gly", "Lys")]
...
files <- list.files("./analysis/12.SignalFiltering/AASignal/Shiny3/Gly", "RawSignal_", full.names = T)
GlySignals <- do.call(rbind, lapply(files, fread))

files <- list.files("./analysis/12.SignalFiltering/AASignal/Shiny3/Lys", "RawSignal_", full.names = T)
LysSignals <- do.call(rbind, lapply(files, fread))

Selected_Sig <- rbind(Selected_Sig, CysSignals, GlySignals, LysSignals)
Selected_Sig[, .N, A]
...
ValSignals <- Selected_Sig[A == "Val"]
ValSignals$File <- mapply(function(x) x[1], strsplit(ValSignals$ID, "_"))
ggplot(ValSignals, aes(x = Blockade, colour = File)) +
  geom_line(stat = "density")
ggplot(ValSignals, aes(x = Blockade)) +
  geom_line(stat = "density")
...
Train <- setDF(Feature_Mat[Selected_Sig_TU[, ID], 1:600])
plot(colSums(Train))
Train <- Train[, paste0("X", sprintf("%03d", 201:500))]
Train$Class <- Selected_Sig_TU[, A]

saveRDS(Train, "./analysis/13.MachineLearning/01.DataPreparation/01.ShinySelect/Model3/Train2.Rds")
...
Train <- readRDS("./analysis/13.MachineLearning/01.DataPreparation/01.ShinySelect/Model3/Train2.Rds")
set.seed(825)
RFC <- train(Class ~ ., data = Train, 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/13.MachineLearning/01.DataPreparation/01.ShinySelect/Model3/RF_Fit2.Rds")
