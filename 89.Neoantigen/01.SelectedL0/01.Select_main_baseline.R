...
outdir <- file.path("analysis/89.Neoantigen/01.SelectedL0")
...
(i <- i + 1)
AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit(meta[i, amino_acid], ""))], ][order(Blockade)]
...
meta[, start_time := as.numeric(start_time)]
meta[, end_time := as.numeric(end_time)]
meta[grepl("blank", amino_acid), amino_acid := "blank"]
meta[, amino_acid := gsub(" ", "", amino_acid)]
outdir <- file.path("analysis/89.Neoantigen/01.SelectedL0")

meta <- meta[!file_id %in% gsub(".MainL0.txt", "", list.files("analysis/89.Neoantigen/01.SelectedL0"))]
# i = 1
BaseMeanFiltering(SigFile = meta[i, sg_files], starttime = meta[i, start_time], endtime = meta[i, end_time], target = meta[i, file_id], outdir = outdir, 
                  product = as.numeric(AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit(meta[i, amino_acid], ""))], Blockade]), 
...
meta <- as.data.table(openxlsx::read.xlsx("data/ChenShanchuan/20240107/20240107 整理数据(20230831)-3.xlsx", sheet = 1))
colnames(meta) <- c("file_name", "start_time", "end_time", "amino_acid", "experiment", "sample", "baseline", "note", "X")
setkey(meta, file_name, start_time)
meta <- meta[file_name %in% gsub(".abf", "", list.files("./data/ChenShanchuan/20240107", "abf"))]
setkey(meta, file_name, start_time)
meta <- meta[, .SD[, .(amino_acid, start_time, end_time, file_id = paste0(file_name, ".", seq_len(.N)))], file_name]
meta[, sg_files := paste0("./analysis/81.ABFProcessing/RawSignal/RawSignal_", file_name, ".txt")]
meta[, file.exists(sg_files)]
...
ggplot(fread(meta[i, sg_files]), aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = .1) + 
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.1, 0.3)) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit(meta[i, amino_acid], ""))], Blockade])
