...
meta <- meta[amino_acid == "VGAL"]
meta[, sig_file := paste0("./analysis/89.Neoantigen/01.SelectedL0/", file_id, ".MainL0.txt")]
meta
```

```{r fig.width=24, fig.height=6}
dataset <- do.call(rbind, lapply(meta[amino_acid == "VGAL", sig_file], fread))[Blockade < 0.35 & DwellTime > 0.3]
B_file <- paste0("./analysis/81.ABFProcessing/FeatureMatrix/FeatureMatrix_", unique(meta[amino_acid == "VGAL", file_name]), ".txt")
...
```{r fig.width=24, fig.height=6}
alldists <- readRDS(file.path("./analysis/89.Neoantigen/02.SignalDistance/VGAL_euclidean_distance_knn.Rds"))
sigs <- KnnSelecting(signals = dataset, dists = alldists, k = 10, noise = 0, index = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("VGAL", ""))], Blockade])
```
...
```{r}
fwrite(dataset[ID %in% sigs], file.path("./analysis/89.Neoantigen/03.SelectedSignals/VGAL_euclidean_distance_knn_selected.signals.txt"), sep = "\t", quote = F)
```
