...
meta1 <- meta[file.exists(sg_files), ]
...
meta[, sg_files := paste0("./analysis/81.ABFProcessing/RawSignal/RawSignal_", file_name, ".txt")]
meta[, file.exists(sg_files)]
meta2 <- meta[file.exists(sg_files), ]
...
meta[, sg_files := paste0("./analysis/81.ABFProcessing/RawSignal/RawSignal_", file_name, ".txt")]
meta[, file.exists(sg_files)]
meta3 <- meta[file.exists(sg_files), ]
...
meta <- meta[grepl("L[SF]GV", amino_acid)]
meta <- meta[!grepl("blank", amino_acid)]
meta <- meta[!grepl("-EF", amino_acid)]
meta[, amino_acid := gsub(" ", "", amino_acid)]
meta$amino_acid <- mapply(function(x) tail(x, 1), strsplit(meta$amino_acid, ","))
meta <- meta[file.exists(paste0("analysis/89.Neoantigen/01.SelectedL0/", file_id, ".MainL0.txt"))]
setkey(meta, amino_acid)
```

```{r}
meta <- as.data.table(openxlsx::read.xlsx("./analysis/89.Neoantigen/02.SignalDistance/meta.xlsx", sheet = 1))
meta <- meta[amino_acid == "LFGV"]
...
```{r fig.width=24, fig.height=6}
alldists <- readRDS(file.path("./analysis/89.Neoantigen/02.SignalDistance/LFGV_euclidean_distance_knn.Rds"))
sigs <- KnnSelecting(signals = dataset, dists = alldists, k = 10, noise = 0, index = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("LFGV", ""))], Blockade])
```
...
```{r}
fwrite(dataset[ID %in% sigs & Blockade < 0.25], file.path("./analysis/89.Neoantigen/03.SelectedSignals/LFGV_euclidean_distance_knn_selected.signals.txt"), sep = "\t", quote = F)
```
