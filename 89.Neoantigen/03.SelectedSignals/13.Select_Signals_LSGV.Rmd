...
```{r fig.width=24, fig.height=6}
alldists <- readRDS(file.path("./analysis/89.Neoantigen/02.SignalDistance/LSGV_euclidean_distance_knn.Rds"))
sigs <- KnnSelecting(signals = dataset, dists = alldists, k = 10, noise = 0, index = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("LSGV", ""))], Blockade])
```
...
ggplot(dataset[ID %in% sigs], aes(x = Blockade, y = DwellTime)) + 
  ...
  geom_vline(xintercept = c(0.1, 0.21))
```

```{r}
sigs <- intersect(sigs, dataset[Blockade < 0.21, ID])
```
...
```{r}
fwrite(dataset[ID %in% sigs], file.path("./analysis/89.Neoantigen/03.SelectedSignals/LSGV_euclidean_distance_knn_selected.signals.txt"), sep = "\t", quote = F)
```

# One by one

## Username_2022-03-11_003.1
...
sigs <- KnnSelecting(signals = dataset[A == "Username_2022-03-11_003.1"], dists = alldists, k = 10, noise = 1, index = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("LSGV", ""))], Blockade])
```

```{r}
sigs <- intersect(sigs, dataset[Blockade < 0.23, ID])
```
...
```{r}
fwrite(dataset[ID %in% sigs], file.path("./analysis/89.Neoantigen/03.SelectedSignals/LSGV_Username_2022-03-11_003.1_euclidean_distance_knn_selected.signals.txt"), sep = "\t", quote = F)
```
...
```{r fig.width=24, fig.height=6}
alldists <- readRDS(file.path("./analysis/89.Neoantigen/02.SignalDistance/22402007.1.signal.dist.Rds"))
sigs <- KnnSelecting(signals = dataset[A == "22402007.1"], dists = alldists, k = 10, noise = 1, index = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("LSGV", ""))], Blockade])
```
...
```{r}
fwrite(dataset[ID %in% sigs], file.path("./analysis/89.Neoantigen/03.SelectedSignals/LSGV_22402004.1_euclidean_distance_knn_selected.signals.txt"), sep = "\t", quote = F)
```
