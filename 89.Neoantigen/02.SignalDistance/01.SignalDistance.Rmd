...
sig0 <- do.call(rbind, lapply(meta[amino_acid == "VGAL", sig_file], fread))
ggplot(sig0[Blockade < 0.3 & A == "22223009.1"], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = 0.4) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("VGAL", ""))], Blockade], col = 2) + 
  scale_x_continuous(breaks = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("VGAL", ""))], Blockade], 
                     labels = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("VGAL", ""))], AA]) + 
  scale_y_log10() + 
  theme_base() + theme(plot.background = element_blank()) -> p1
p1
```

```{r}
sig0 <- do.call(rbind, lapply(meta[amino_acid == "VGALD", sig_file], fread))
ggplot(sig0[Blockade < 0.3], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = 0.4) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("VGALD", ""))], Blockade], col = 2) + 
  scale_x_continuous(breaks = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("VGALD", ""))], Blockade], 
                     labels = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("VGALD", ""))], AA]) + 
  scale_y_log10() + 
  theme_base() + theme(plot.background = element_blank()) -> p2
p2
```
...
```{r}
sig0 <- do.call(rbind, lapply(meta[amino_acid == "LSGV", sig_file], fread))
```
...
```{r}
sig0 <- do.call(rbind, lapply(meta[amino_acid == "LFGV", sig_file], fread))
```
...
```{r}
sig0 <- do.call(rbind, lapply(meta[is.na(amino_acid), sig_file], fread))
```
...
ggplot(bgsignals[Blockade < 0.3], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = 0.4) + 
  scale_y_log10()
ggplot(Big_FeatureMatrix[Blockade < 0.3], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = 0.4) + 
  scale_y_log10()
```
...
```{r}
ggplot(bgsignals0[Blockade < 0.3], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = 0.4) + 
  scale_y_log10()
ggplot(Big_FeatureMatrixs[Blockade < 0.3], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = 0.4) + 
  scale_y_log10()
```

```{r}
mclapply(meta[amino_acid == "LSGV", sig_file], function(a) {
  print(a)
  outfile <- file.path("./analysis/89.Neoantigen/02.SignalDistance", gsub(".MainL0.txt", ".signal.dist.Rds", basename(a)))
...
```{r}
mclapply(meta[amino_acid == "LFGV", sig_file], function(a) {
  print(a)
  outfile <- file.path("./analysis/89.Neoantigen/02.SignalDistance", gsub(".MainL0.txt", ".signal.dist.Rds", basename(a)))
  if(file.exists(outfile)) return(NULL)
  sig0 <- fread(a)[Blockade < 0.3 & DwellTime > 0.35]
...
meta <- meta0[amino_acid == "VGALD" | is.na(amino_acid)]
```
...
```{r}
mclapply(meta[amino_acid == "VGALD", sig_file], function(a) {
  print(a)
  outfile <- file.path("./analysis/89.Neoantigen/02.SignalDistance", gsub(".MainL0.txt", ".signal.dist.Rds", basename(a)))
  if(file.exists(outfile)) return(NULL)
  sig0 <- fread(a)[Blockade < 0.3 & DwellTime > 0.35]
...
meta0[, sig_file := paste0("./analysis/89.Neoantigen/01.SelectedL0/", file_id, ".MainL0.txt")]
meta0 <- meta0[file.exists(sig_file)]
meta <- meta0[amino_acid == "LSGV" | is.na(amino_acid)]
```
