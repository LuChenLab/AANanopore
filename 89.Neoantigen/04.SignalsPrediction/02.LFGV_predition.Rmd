...
```{r }
LFGV.selected.signals <- do.call(rbind, lapply(list.files("./analysis/89.Neoantigen/03.SelectedSignals", "LFGV", full.names = TRUE), fread))
LFGV.selected.signals <- data.table(LFGV.selected.signals, D1 = LFGV.selected.signals[, get_density(x = Blockade, y = log10(DwellTime))])
```
...
```{r}
ggplot() + 
  geom_point(data = LFGV.selected.signals[sample(.N, .N)], aes(x = Blockade, y = DwellTime, colour = A, alpha = D1), size = 0.2) + 
...
```{r}
subtab <- Sig_FeatureMatrixs[LFGV.selected.signals$ID]
pred1 <- data.table(ID = subtab$ID, 
                    Pred1 = as.character(predict(model2, subtab)), 
                    Prob1 = apply(predict(model2, subtab, type = "prob"), 1, max), 
                    Delta1 = apply(predict(model2, subtab, type = "prob"), 1, function(x) -diff(head(sort(x, decreasing = T), 2))), 
...
```{r}
LFGV <- merge(LFGV.selected.signals, pred1, by = "ID")
```

```{r}
fwrite(LFGV, "./analysis/89.Neoantigen/04.SignalsPrediction/LFGV_prediction.txt", sep = "\t", quote = FALSE)
```

```{r}
ggplot() + 
  geom_point(data = LFGV[sample(.N, .N)], aes(x = Blockade, y = DwellTime, colour = Pred2, alpha = D1), size = 1) + 
  scale_y_log10() + 
...
AADensityPlot1(x = LFGV[Prob2 > 0.75 & Delta2 > 0.5 & Blockade < 0.23 & Blockade > 0.11], target = c("Gly", "Leu", "Ser", "Val", "Phe")) -> p4
p4 <- p4 + scale_x_continuous(limits = c(0.11, 0.23))
p4
```

```{r fig.width=6, fig.height=8}
LFGV_N <- LFGV[Prob1 > 0.75 & Delta1 > 0.5][Prob2 > 0.75 & Delta2 > 0.5, .N, .(Pred1, Pred2)][, .(Pred2, N, P = N / sum(N)), Pred1]
LFGV_N[, Pred1 := as.character(Pred1)]
LFGV_N[, Pred2 := as.character(Pred2)]
LFGV_N[, L := paste0(round(P * 100, 2), " %")]
# LFGV_N[Pred1 != Pred2, L := ""]

ggplot(LFGV_N, aes(x = 1, y = P, fill = Pred2)) + 
  geom_col() + 
...
```{r eval=FALSE}
LFGV[, .(.N, length(unique(Pred1))), .(A, round(StartTime))][order(V2, N, decreasing = T)]
```

```{r}
LFGV_abf <- readRDS("./analysis/81.ABFProcessing/ABF/ABF_Username_2022-03-14_005.Rds")
```

```{r fig.height=2, fig.width=12, echo=FALSE}
LFGV_abf_i <- LFGV_abf[round(Time) == 1283]
L0 <- MainRidge(x = LFGV_abf_i[, pA], bw = 0.5, peaks = 1, CI = 0.95, plot = F)[[1]]
aasumm1 <- SigSumm[A %in% c("Gly", "Leu", "Phe", "Val")]
...
ggplot() + 
  geom_rect(data = aasumm1, aes(xmin = LFGV_abf_i[, min(Time)], xmax = LFGV_abf_i[, max(Time)], ymin = ymin, ymax = ymax, fill = A), alpha = 0.3) + 
  geom_step(data = LFGV_abf_i, mapping = aes(x = Time, y = pA), size = 0.1) + 
  scale_y_continuous(n.breaks = 4, limits = c(aasumm1[, min(ymin)] - 5, LFGV_abf_i[, max(pA)]), 
...
```{r}
LFGV_Freq <- LFGV[Prob2 > 0.75 & Delta2 > 0.5 & Blockade > 0.11 & Blockade < 0.23, .N, Pred2]
LFGV_Freq[, Pred2 := factor(Pred2, levels = LFGV_Freq[order(N, decreasing = T), Pred2])]
ggplot(LFGV_Freq, aes(x = Pred2, y = N, fill = Pred2)) + 
  geom_col() + 
...
```{r}
ggplot() + 
  geom_line(data = LFGV.selected.signals, aes(x = Blockade), size = 0.2, stat = "density", adjust = .2, colour = "red") + 
  scale_x_continuous(limits = c(0.11, 0.23)) + 
  labs(tag = "a") + 
  theme_void() + 
...
ggsave("./analysis/89.Neoantigen/04.SignalsPrediction/LFGV_prediction.pdf", width = 12, height = 6)
```
