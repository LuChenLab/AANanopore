...
dataset[A == "22401011.1", experiment := "Rep1"]
dataset[A != "22401011.1", experiment := "Rep2"]
```

```{r}
ggplot(dataset[Delta1 > 0.9], aes(x = Blockade, y = DwellTime, colour = Pred1)) + 
  geom_point(size = .5) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("LFGV", ""))], Blockade]) + 
  scale_y_log10() + 
...
```{r}
dataset2 <- dataset[Delta1 > 0.9]
```

```{r}
subtab <- subset.data.frame(FeatureMatrix, row.names(FeatureMatrix) %in% dataset2$ID)
...
ggplot(dataset2[ID %in% sigs], aes(x = Blockade, y = DwellTime, colour = ID %in% sigs)) + 
  geom_point(size = .5) + 
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.1, 0.3)) + 
  scale_colour_manual(values = "#E41A1C") + 
...
```{r}
dataset3 <- dataset2[ID %in% sigs]
fwrite(dataset3, "./analysis/89.Neoantigen/04.SignalsPrediction/LFGV_prediction_V2.txt", sep = "\t")
```

```{r}
ggplot(dataset3, aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = .5) + 
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.1, 0.3)) + 
  scale_colour_manual(values = "#E41A1C") + 
