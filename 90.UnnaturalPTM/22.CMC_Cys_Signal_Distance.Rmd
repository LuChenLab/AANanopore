...
```{r fig.width=8, fig.height=4}
ggplot(Big_FeatureMatrixs, aes(x = Blockade, y = DwellTime * 1000)) + 
  geom_point(size = 0.4) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("C", ""))], Blockade], col = 2) + 
  geom_hline(yintercept = 0.35) + 
  scale_y_log10() + 
...
saveRDS(dist, file = "./analysis/90.UnnaturalPTM/02.SignalsDistance/CMC_Cys_spearman_distance_knn.Rds")

dist = stats::dist(FeatureMatrix, method = "euclidean")
dist <- dbscan::kNN(dist, k = 20)
saveRDS(dist, file = "./analysis/90.UnnaturalPTM/02.SignalsDistance/CMC_Cys_euclidean_distance_knn.Rds")
```
