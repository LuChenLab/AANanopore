...
meta <- as.data.table(openxlsx::read.xlsx("data/ChenShanchuan/20240108/20240108 整理数据(20230831)-2.xlsx", sheet = 1))
colnames(meta) <- c("file_name", "start_time", "end_time", "product", "experiment", "sample", "baseline", "note", "X")
setkey(meta, file_name, start_time)
all(gsub(".abf", "", list.files("./data/ChenShanchuan/20240108", "abf")) %in% meta$file_name)
setdiff(gsub(".abf", "", list.files("./data/ChenShanchuan/20240108", "abf")), meta$file_name)

meta <- meta[file_name %in% gsub(".abf", "", list.files("./data/ChenShanchuan/20240108", "abf"))]
...
meta <- meta[!grepl("20240107_", file_name)]

outdir <- file.path("analysis/90.UnnaturalPTM/01.SelectedL0")

meta <- meta[!file_id %in% gsub(".MainL0.txt", "", list.files("analysis/90.UnnaturalPTM/01.SelectedL0"))]
# i = 1
BaseMeanFiltering(SigFile = meta[i, sg_files], starttime = meta[i, start_time], endtime = meta[i, end_time], target = meta[i, file_id], outdir = outdir, 
                  product = c(as.numeric(AABlockade[AA %in% AMINO_ACID_CODE["K"], Blockade]), 0.23))
(i <- i + 1); meta[i]
