...
ggplot(sig0[Blockade < 0.5], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = 0.4) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("S", ""))], Blockade], col = 2) + 
...
```{r}
mclapply(meta[!is.na(amino_acid), sig_file], function(a) {
  print(a)
  ...
  dist <- dbscan::kNN(euclidean, k = 20)
  saveRDS(dist, file = outfile)
}, mc.cores = 4)
```
...
saveRDS(dist, file = "./analysis/90.UnnaturalPTM/02.SignalsDistance/SerPTM_spearman_distance_knn.Rds")

dist = stats::dist(FeatureMatrix, method = "euclidean")
dist <- dbscan::kNN(dist, k = 20)
saveRDS(dist, file = "./analysis/90.UnnaturalPTM/02.SignalsDistance/SerPTM_euclidean_distance_knn.Rds")
```
