...
```{r}
ggplot(dataset[Pred1 == "Cys"], aes(x = Blockade, y = DwellTime)) + 
  geom_point() + 
  scale_y_log10() + 
...
ggplot(dataset[Pred2 == "Cys" & Prob2 > 0.95 & Prob1 < 0.5], aes(x = Blockade, y = DwellTime)) + 
  geom_point() + 
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.1, 0.6)) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("C", ""))], Blockade], col = 2)
...
```{r}
ggplot(CMCC, aes(x = Blockade, y = DwellTime)) + 
  geom_point() + 
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.1, 0.6)) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("C", ""))], Blockade], col = 2)
...
i <- dataset[ID %in% Cys_ID & A == "20231030_0007.1"][order(DwellTime, decreasing = T), StartTime][1]
ggplot(abf[Time > i - 0.2 & Time < i + 0.2], aes(x = Time, y = pA)) + 
  geom_step() + 
...
```{r fig.width=18, fig.height=3}
i <- dataset[ID %in% Cys_ID & A == "20231030_0007.1"][order(DwellTime, decreasing = T), StartTime][3]
ggplot(abf[Time > i - 0.02 & Time < i + 0.2], aes(x = Time, y = pA)) + 
  geom_step() + 
  geom_vline(xintercept = i) +
  theme_base()
```
...
```{r fig.width=18, fig.height=3}
i <- dataset[ID %in% Cys_ID & A == "20231030_0007.1"][order(DwellTime, decreasing = T), StartTime][6]
ggplot(abf[Time > 476.2 - 0.16 & Time < 476.2 + 0.078], aes(x = Time, y = pA)) + 
  geom_step() + 
  theme_base()
```
...
```{r fig.width=18, fig.height=3}
ggplot(abf_i, aes(x = Time, y = pA / L0)) + 
  geom_step() + 
  scale_x_continuous(breaks = c(476.2 - 0.12, 476.2 - 0.12 + 0.1), labels = c(0, 0.1), expand = c(0.0001, 0.0001)) + 
  scale_y_continuous(limits = c(0.4, abf_i[, max(pA) / L0]), breaks = c(0.5, 0.75, 1)) + 
  geom_text_repel(data = RaeSig, aes(x = StartTime + (EndTime - StartTime)/2, y = SignalCurrent / L0 - .1, label = Lab), direction = "y") + 
  labs(x = "Time (s)", y = "Standardized current") + 
...
ggsave("./analysis/90.UnnaturalPTM/04.PredictedSignalPlot/CMC.pdf", width = 6, height = 9)
```
