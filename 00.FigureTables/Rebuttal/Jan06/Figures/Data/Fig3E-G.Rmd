...
Asp_All[file_id == "21125018.1" & StartTime < 250, .(SigTime = sum(DwellTime), N = .N), .(file_id)]

Asp_All_Sig_Freq1 <- merge(SigInterTime[File == "21125018.1" & StartTime < 250, .(GapTime1 = sum(Time1), GapTime2 = sum(Time2)), .(File)], 
                          Asp_All[file_id == "21125018.1" & StartTime < 250, .(SigTime = sum(DwellTime), N = .N), .(file_id)], by.x = "File", by.y = "file_id")
Asp_All_Sig_Freq1[, File := "21125018.0"]
...
Asp_All_Sig_Freq <- rbind(Asp_All_Sig_Freq, Asp_All_Sig_Freq1)
...
Asp_All_Sig_Freq <- merge(meta[, .(file_id, concentration)], Asp_All_Sig_Freq, by.y = "File", by.x = "file_id", all.y = T)
Asp_All_Sig_Freq[is.na(concentration), concentration := 4]
```

```{r}
id2rep <- as.data.table(openxlsx::read.xlsx("./data/MetaInfomation/File_id_to_replicates_rep.xlsx"))
Gly_All_Sig_Freq <- merge(Gly_All_Sig_Freq, id2rep, by.x = "file_id", by.y = "File_id")
Arg_All_Sig_Freq <- merge(Arg_All_Sig_Freq, id2rep, by.x = "file_id", by.y = "File_id")
Asp_All_Sig_Freq <- merge(Asp_All_Sig_Freq, id2rep, by.x = "file_id", by.y = "File_id", all.x = T)
Asp_All_Sig_Freq[file_id == "21125018.0", rep := 2]
Asp_All_Sig_Freq[file_id == "21125018.0", Fig := "3f"]
# 
Gly_All_Sig_Freq <- Gly_All_Sig_Freq[, .(Freq = mean(Freq)), .(concentration, rep)]
Arg_All_Sig_Freq <- Arg_All_Sig_Freq[, .(Freq = mean(Freq)), .(concentration, rep)]
Asp_All_Sig_Freq <- Asp_All_Sig_Freq[, .(Freq = mean(Freq)), .(concentration, rep)]
```

```{r}
Gly_All_Sig_Freq <- Gly_All_Sig_Freq[, .(Frequency = Freq, Experiment = paste0("Rep", seq_len(.N))), .(Concentration = concentration)]
Arg_All_Sig_Freq <- Arg_All_Sig_Freq[, .(Frequency = Freq, Experiment = paste0("Rep", seq_len(.N))), .(Concentration = concentration)]
Asp_All_Sig_Freq <- Asp_All_Sig_Freq[, .(Frequency = Freq, Experiment = paste0("Rep", seq_len(.N))), .(Concentration = concentration)]
```

```{r}
openxlsx::write.xlsx(list(Gly = Gly_All_Sig_Freq, Arg = Arg_All_Sig_Freq, Asp = Asp_All_Sig_Freq), "./analysis/00.FigureTables/Rebuttal/Jan06/Figures/Data/Fig3E-G.xlsx")
```
