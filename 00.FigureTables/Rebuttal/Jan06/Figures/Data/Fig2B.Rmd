...
sig0[, A := gsub("CbC", "CMC", A)]
sig0[, AA := gsub("CbC", "CMC", AA)]
```
...
```{r}
data <- aaBlockade[A != "CMC", .(Class, AminoAcid = A, BlockadeMean, BlockadeSD, Blockade = paste(round(BlockadeMean, 3), "±", round(BlockadeSD, 4)), Volume = V)]
data <- aaBlockade[A != "CMC", .(Class, AminoAcid = A, BlockadeMean, BlockadeSD, Volume = V)]
openxlsx::write.xlsx(data, "./analysis/00.FigureTables/Rebuttal/Jan06/Figures/Data/Fig2B.xlsx", overwrite = T)
```
