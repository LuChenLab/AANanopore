...
```{r}
SigSumm <- as.data.table(openxlsx::read.xlsx("./analysis/00.FigureTables/Rebuttal/Jan06/Figures/Data/Fig2A.xlsx"))
...
ggplot() +
  ...
  stat_function(fun = dnorm, n = 1024, args = list(mean = SigSumm[Class == "Charged", BlockadeMean][7], sd = SigSumm[Class == "Charged", BlockadeSD][7]), colour = aa_cols[SigSumm[Class == "Charged", A][7]]) + 
  scale_x_continuous(limits = c(0.1, 0.26)) + 
  geom_text_repel(data = SigSumm[Class == "Charged"][, .(A = A, x = BlockadeMean, y = dnorm(BlockadeMean, BlockadeMean, BlockadeSD))], aes(x, y, label = A, colour = A), size = 8) + 
  ...
  theme(panel.grid = element_blank(), 
        ...
        strip.background = element_rect(fill = RColorBrewer::brewer.pal(n = 3, "Dark2")[1])) -> p1
...
ggplot() +
  ...
  scale_x_continuous(limits = c(0.1, 0.26)) + 
  geom_text_repel(data = SigSumm[Class == "Nonpolar"][, .(A = A, x = BlockadeMean, y = dnorm(BlockadeMean, BlockadeMean, BlockadeSD))], aes(x, y, label = A, colour = A), size = 8) + 
  scale_colour_manual(breaks = names(aa_cols), values = aa_cols, guide = "none") + 
  ...
  theme(panel.grid = element_blank(), 
        axis.text = element_text(colour = "black"), 
        axis.title = element_text(colour = "black"), 
        strip.background = element_rect(fill = RColorBrewer::brewer.pal(n = 3, "Dark2")[3])) -> p2
...
ggplot() +
  ...
  scale_x_continuous(limits = c(0.1, 0.26)) + 
  geom_text_repel(data = SigSumm[Class == "Polar"][, .(A = A, x = BlockadeMean, y = dnorm(BlockadeMean, BlockadeMean, BlockadeSD))], aes(x, y, label = A, colour = A), size = 8) + 
  scale_colour_manual(breaks = names(aa_cols), values = aa_cols, guide = "none") + 
  ...
  theme(panel.grid = element_blank(), 
        axis.text = element_text(colour = "black"), 
        axis.title = element_text(colour = "black"), 
        strip.background = element_rect(fill = RColorBrewer::brewer.pal(n = 3, "Dark2")[2])) -> p3
```
...
```{r}
IntervalTime <- as.data.table(openxlsx::read.xlsx("./analysis/00.FigureTables/Rebuttal/Jan06/Figures/Data/Fig2CD.xlsx"))
...
ggplot(IntervalTime, aes(x = Property2, y = Frequency, colour = Property2)) + 
  ...
  scale_y_log10(limits = c(1e-1, 300)) + 
  stat_compare_means(aes(label = ..p.signif..), label.y = 2.1, comparisons = my_comp, size = 8) + 
...
```{r}
Sigs1 <- as.data.table(openxlsx::read.xlsx("./analysis/00.FigureTables/Rebuttal/Jan06/Figures/Data/Fig2E.xlsx", 1))
DwellTime <- as.data.table(openxlsx::read.xlsx("./analysis/00.FigureTables/Rebuttal/Jan06/Figures/Data/Fig2E.xlsx", 2))
od <- DwellTime[order(Class, half.life), AminoAcid]
Sigs1[, AminoAcid := factor(AminoAcid, levels = od)]
Sigs1[, Class := factor(Class, levels = c("Charged", "Nonpolar", "Polar"))]
```

```{r fig.height=5, fig.width=6.5}
ggplot() + 
  geom_violin(data = Sigs1, aes(x = AminoAcid, y = DwellTime, colour = Class)) +
  geom_pointrange(data = DwellTime, aes(x = AminoAcid, y = half.life, ymin = half.life - half.life_Std.Error, ymax = half.life + half.life_Std.Error), size = 0.5) + 
  scale_y_log10() +
  theme_bw(base_size = 22) + 
  scale_colour_manual(values = RColorBrewer::brewer.pal(n = 3, name = "Dark2")[c(1, 3, 2)]) + 
...
grDevices::cairo_pdf("./analysis/00.FigureTables/Rebuttal/Jan06/Figures/Figure/Fig2.pdf", width = 22, height = 13)
p1 + p2 + p3 + p4 + p5 + p6 + p7 + 
  plot_layout(design = design)
dev.off()
```
