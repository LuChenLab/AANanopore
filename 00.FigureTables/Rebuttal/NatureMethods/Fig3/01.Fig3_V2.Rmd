...
model1 <- readRDS("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/06.Modeling/Version1/L1/RF_model.Rds")
...
```{r eval=FALSE}
predicts0 <- mclapply(c("AdaBoost", "CART", "KNN", "NB", "NNet", "RF"), function(m) {
  model <- readRDS(paste0("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/06.Modeling/Version1/L1/", m, "_model.Rds"))
...
saveRDS(predicts0, file = "analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/06.Modeling/Version1/L1/predicts0.Rds")
```
...
ggplot(roc_mat, aes(x = 1 - Specificity, y = Sensitivity, colour = Method)) + 
  geom_path(size = 1) + 
  geom_text(data = AUCs, mapping = aes(x = 0.75, y = 6:1/10, label = round(AUC, 3))) +
  theme_bw(base_size = 15) + 
