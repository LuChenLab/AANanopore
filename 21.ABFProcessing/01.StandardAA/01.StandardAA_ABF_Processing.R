...
mclapply(1:nrow(meta), function(i) {
  print(i)
  if(file.exists(paste0("./analysis/21.ABFProcessing/01.StandardAA/RawSignal/RawSignal_", meta[i, file_name], ".txt"))) return()
  File <- meta[i, file_path]
  StartTime <- meta[i, start_time]
  EndTime <- meta[i, end_time]
  
  if(file.exists(paste0("./analysis/21.ABFProcessing/01.StandardAA/ABF/ABF_", meta[i, file_name], ".Rds"))) {
    abf <- readRDS(paste0("./analysis/21.ABFProcessing/01.StandardAA/ABF/ABF_", meta[i, file_name], ".Rds"))
  } else {
    ...
    saveRDS(abf, paste0("./analysis/21.ABFProcessing/01.StandardAA/ABF/ABF_", meta[i, file_name], ".Rds"))
  }
  ...
  fwrite(Sigs, file = paste0("./analysis/21.ABFProcessing/01.StandardAA/RawSignal/RawSignal_", meta[i, file_name], ".txt"), sep = "\t", row.names = F, quote = F)
  
  # Signal Current
  Current <- SignalCurrent(Sigs, abf = abf)
  saveRDS(Current, file = paste0("./analysis/21.ABFProcessing/01.StandardAA/SignalCurrent/SignalCurrent_", meta[i, file_name], ".Rds"))
  ...
  setkey(Mat, ID)
  Mat <- Mat[Sigs$ID, ]
  fwrite(Mat, paste0("./analysis/21.ABFProcessing/01.StandardAA/FeatureMatrix/FeatureMatrix_", meta[i, file_name], ".txt"), sep = "\t", quote = F)
}, mc.cores = 1)
