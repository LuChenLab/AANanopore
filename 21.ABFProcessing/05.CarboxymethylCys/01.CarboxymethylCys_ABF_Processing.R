...
meta <- meta1[实验目的 == "半胱氨酸突变"]
meta <- meta[, 1:4]
colnames(meta) <- c("file_name", "start_time", "end_time", "amino_acid")
...
mclapply(1:nrow(meta), function(i) {
  print(i)
  if(file.exists(paste0("./analysis/21.ABFProcessing/05.CarboxymethylCys/RawSignal/RawSignal_", meta[i, file_name], ".txt"))) return()
  File <- meta[i, file_path]
  StartTime <- meta[i, start_time]
  EndTime <- meta[i, end_time]
  
  if(file.exists(paste0("./analysis/21.ABFProcessing/05.CarboxymethylCys/ABF/ABF_", meta[i, file_name], ".Rds"))) {
    abf <- readRDS(paste0("./analysis/21.ABFProcessing/05.CarboxymethylCys/ABF/ABF_", meta[i, file_name], ".Rds"))
  } else {
    ...
    saveRDS(abf, paste0("./analysis/21.ABFProcessing/05.CarboxymethylCys/ABF/ABF_", meta[i, file_name], ".Rds"))
  }
  ...
  fwrite(Sigs, file = paste0("./analysis/21.ABFProcessing/05.CarboxymethylCys/RawSignal/RawSignal_", meta[i, file_name], ".txt"), sep = "\t", row.names = F, quote = F)
  
  # Signal Current
  Current <- SignalCurrent(Sigs, abf = abf)
  saveRDS(Current, file = paste0("./analysis/21.ABFProcessing/05.CarboxymethylCys/SignalCurrent/SignalCurrent_", meta[i, file_name], ".Rds"))
  ...
  setkey(Mat, ID)
  Mat <- Mat[Sigs$ID, ]
  fwrite(Mat, paste0("./analysis/21.ABFProcessing/05.CarboxymethylCys/FeatureMatrix/FeatureMatrix_", meta[i, file_name], ".txt"), sep = "\t", quote = F)
}, mc.cores = 2)


list.files("/mnt/raid61/Personal_data/tangchao/AANanopore/analysis/21.ABFProcessing/03.MixtureAA", "20230404_0005", recursive = T, full.names = T)
