...
meta2 <- as.data.table(openxlsx::read.xlsx("./data/ZhangMing_20220314/neoantigen水解20220314.xlsx"))
colnames(meta2) <- c("file_name", "date", "amino_acid", "concentration", "start_time", "end_time", "type")
meta2 <- meta2[concentration == 20]
meta2$amino_acid <- mapply(function(x) x[2], strsplit(meta2$amino_acid, ","))
meta2[, start_time := as.numeric(start_time)]
meta2[, end_time := as.numeric(end_time)]

meta3 <- as.data.table(openxlsx::read.xlsx("./data/ZhangMing_20211025/实验信息表1025.xlsx"))
colnames(meta3) <- c("file_name", "date", "amino_acid", "concentration", "start_time", "end_time", "type")
meta3[, start_time := as.numeric(start_time)]
meta3[, end_time := as.numeric(end_time)]
meta3 <- meta3[concentration > 0]

meta <- rbind(meta2, meta3)
meta <- meta[, .(start_time = min(start_time), end_time = max(end_time)), file_name]
meta$file_path <- mapply(meta[, file_name], FUN = function(x) list.files("./data", pattern = x, full.names = TRUE, recursive = TRUE))
