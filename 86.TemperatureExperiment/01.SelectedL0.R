...
meta <- fread("./data/MetaInfomation/Temperature_Experiment_Meta.txt")
meta[, sg_files := paste0("./analysis/81.ABFProcessing/RawSignal/RawSignal_", file_name, ".txt")]
meta <- meta[file.exists(sg_files)]
meta <- meta[!file_id %in% gsub(".MainL0.txt", "", list.files("analysis/86.TemperatureExperiment/SelectedL0"))]
meta[sample == "blank", amino_acid := NA]

outdir <- file.path("analysis/86.TemperatureExperiment/SelectedL0")
# i = 1
meta[i]; BaseMeanFiltering(SigFile = meta[i, sg_files], starttime = meta[i, start_time], endtime = meta[i, end_time], target = meta[i, file_id], outdir = outdir, product = as.numeric(AABlockade[AA %in% AMINO_ACID_CODE[meta[i, strsplit(amino_acid, "")][[1]]], Blockade]))
(i <- i + 1)
