...
meta[, sig_file := paste0("./analysis/84.ADpolypeptide/11.SelectedL0/", file_id, ".MainL0.txt")]
```
...
```{r }
LVFAK.selected.signals <- do.call(rbind, lapply(list.files("./analysis/84.ADpolypeptide/13.SelectedSignals", "LVFAK.selected.signals.txt", full.names = TRUE), fread))
...
```{r}
fwrite(LVFAK, "./analysis/84.ADpolypeptide/14.SignalsPrediction/LVFAK_prediction.txt", sep = "\t", quote = FALSE)
```
...
AADensityPlot1(x = LVFAK[Blockade < 0.24 & Blockade > 0.11], target = c("Phe", "Ala", "Val", "Leu", "Lys")) -> p4
p4 <- p4 + scale_x_continuous(limits = c(0.11, 0.24))
p4
```

```{r fig.width=6, fig.height=8}
LVFAK_N <- LVFAK[, .N, .(Pred1, Pred2)][, .(Pred2, N, P = N / sum(N)), Pred1]
LVFAK_N[, Pred1 := as.character(Pred1)]
LVFAK_N[, Pred2 := as.character(Pred2)]
LVFAK_N[, L := paste0(round(P * 100, 2), " %")]
# LVFAK_N[Pred1 != Pred2, L := ""]

ggplot(LVFAK_N, aes(x = 1, y = P, fill = Pred2)) + 
  geom_col() + 
...
```{r eval=FALSE}
LVFAK[, .(.N, length(unique(Pred1))), .(A, round(StartTime))][order(V2, N, decreasing = T)]
```

```{r}
LVFAK_abf <- readRDS("./analysis/81.ABFProcessing/ABF/ABF_20231020_0007.Rds")
```

```{r fig.height=2, fig.width=12, echo=FALSE}
LVFAK_abf_i <- LVFAK_abf[round(Time) == 1138]
L0 <- MainRidge(x = LVFAK_abf_i[, pA], bw = 0.5, peaks = 1, CI = 0.95, plot = F)[[1]]
aasumm1 <- SigSumm[A %in% c("Phe", "Ala", "Val", "Leu", "Lys")]
...
ggplot() + 
  geom_rect(data = aasumm1, aes(xmin = LVFAK_abf_i[, min(Time)], xmax = LVFAK_abf_i[, max(Time)], ymin = ymin, ymax = ymax, fill = A), alpha = 0.3) + 
  geom_step(data = LVFAK_abf_i, mapping = aes(x = Time, y = pA), size = 0.1) + 
  scale_y_continuous(n.breaks = 4, limits = c(aasumm1[, min(ymin)] - 5, LVFAK_abf_i[, max(pA)]), 
...
```{r}
LVFAK_Freq <- LVFAK[Blockade > 0.11 & Blockade < 0.24, .N, Pred2]
...
```{r}
ggplot() + 
  geom_line(data = LVFAK.selected.signals, aes(x = Blockade), size = 0.2, stat = "density", adjust = .2, colour = "red") + 
  scale_x_continuous(limits = c(0.11, 0.24)) + 
  labs(tag = "a") + 
  theme_void() + 
...
ggsave("./analysis/84.ADpolypeptide/14.SignalsPrediction/LVFAK_prediction.pdf", width = 12, height = 6)
```
...
AADensityPlot1(x = LVFAK[Blockade < 0.24 & Blockade > 0.11 & Pred2 != "Phe"], target = c("Phe", "Ala", "Val", "Leu", "Lys")) -> p4
p4 <- p4 + scale_x_continuous(limits = c(0.11, 0.24))
p4
```
