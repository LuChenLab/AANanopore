...
```{r}
sig0 <- do.call(rbind, lapply(meta[amino_acid == "LVPFA", sig_file], fread))
```

```{r fig.width=6, fig.height=4}
ggplot(sig0[Blockade < 0.3], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = 0.4) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("LVPFA", ""))], Blockade], col = 2) + 
  scale_y_log10() + 
  facet_wrap(~ A, nrow = 1)
```
...
mclapply(meta[amino_acid == "LVFAG", sig_file], function(a) {
  print(a)
  outfile <- file.path("./analysis/84.ADpolypeptide/02.SignalDistance", gsub(".MainL0.txt", ".signal.dist.Rds", basename(a)))
