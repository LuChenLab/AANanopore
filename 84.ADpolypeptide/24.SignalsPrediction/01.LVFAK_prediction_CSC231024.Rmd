...
```{r }
LVFAK.selected.signals <- do.call(rbind, lapply(list.files("./analysis/84.ADpolypeptide/23.SelectedSignals", "LVFAK.selected.signals_CSC231024.txt", full.names = TRUE), fread))
...
```{r}
LVFAK_abf <- readRDS("./analysis/81.ABFProcessing/ABF/ABF_20231024_0005.Rds")
```

```{r fig.height=2, fig.width=12, echo=FALSE}
LVFAK_abf_i <- LVFAK_abf[round(Time) == 811	]
...
```{r}
ggplot() + 
  geom_line(data = LVFAK.selected.signals, aes(x = Blockade), size = 0.2, stat = "density", adjust = .5, colour = "red") + 
  scale_x_continuous(limits = c(0.11, 0.24)) + 
  labs(tag = "a") + 
  theme_void() + 
...
ggsave("./analysis/84.ADpolypeptide/24.SignalsPrediction/LVFAK_prediction.pdf", width = 12, height = 6)
```
