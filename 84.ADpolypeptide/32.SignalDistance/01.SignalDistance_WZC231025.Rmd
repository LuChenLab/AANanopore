...
meta[, sig_file := paste0("./analysis/84.ADpolypeptide/31.SelectedL0/", file_id, ".MainL0.txt")]
```
...
```{r}
sig0 <- do.call(rbind, lapply(meta[amino_acid == "LVFA", sig_file], fread))
```

```{r fig.width=20, fig.height=8}
ggplot(sig0[Blockade < 0.3], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = 0.4) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("LVFA", ""))], Blockade], col = 2) + 
  scale_x_continuous(sec.axis = dup_axis(breaks = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("LVFA", ""))], Blockade], 
                                         labels = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("LVFA", ""))], AA])) + 
  geom_hline(yintercept = 0.35, colour = "red") + 
  scale_y_log10() + 
  facet_wrap(~ A, nrow = 2, strip.position = "right") + 
  theme_base()
```
...
```{r}
mclapply(meta[amino_acid == "LVFA", sig_file], function(a) {
  print(a)
  outfile <- file.path("./analysis/84.ADpolypeptide/32.SignalDistance", gsub(".MainL0.txt", ".signal.dist.Rds", basename(a)))
...
```{r}
sig0 <- lapply(meta[amino_acid == "LVFA", sig_file], fread)
sig0 <- do.call(rbind, sig0)[Blockade < 0.3 & DwellTime > 0.35]
B_file <- unique(meta[amino_acid == "LVFA", file_name])
B_file <- paste0("./analysis/81.ABFProcessing/FeatureMatrix/FeatureMatrix_", B_file, ".txt")
Sig_FeatureMatrix <- do.call(rbind, lapply(B_file, fread))
Sig_FeatureMatrix <- Sig_FeatureMatrix[ID %in% sig0$ID]
...
FeatureMatrix <- data.frame(FeatureMatrix[, grepl("^X", colnames(FeatureMatrix)) | grepl("DwellTime", colnames(FeatureMatrix)), with = F], row.names = FeatureMatrix[[1]])
euclidean = stats::dist(FeatureMatrix, method = "euclidean")
saveRDS(euclidean, file = "./analysis/84.ADpolypeptide/32.SignalDistance/LVFA_all_sigs_distance.Rds")
```
