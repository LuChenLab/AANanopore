...
```{r}
sig0 <- do.call(rbind, lapply(meta[amino_acid == "LVFAG", sig_file], fread))
```
...
```{r}
sig0 <- do.call(rbind, lapply(meta[amino_acid == "LVFAK", sig_file], fread))
```
...
ggplot(sig0[Blockade < 0.3], aes(x = Blockade, y = DwellTime)) + 
  ...
  scale_y_log10() + 
  geom_hline(yintercept = 0.35, col = 2) + 
  facet_wrap(~ A, nrow = 1) + 
  theme_base()
```
...
```{r}
mclapply(meta[amino_acid == "LVFAK", sig_file], function(a) {
  print(a)
  outfile <- file.path("./analysis/84.ADpolypeptide/12.SignalDistance", gsub(".MainL0.txt", ".signal.dist.Rds", basename(a)))
...
```{r}
mclapply(meta[amino_acid == "LVFAG", sig_file], function(a) {
  print(a)
  outfile <- file.path("./analysis/84.ADpolypeptide/12.SignalDistance", gsub(".MainL0.txt", ".signal.dist.Rds", basename(a)))
  if(file.exists(outfile)) return(NULL)
  sig0 <- fread(a)[Blockade < 0.3 & DwellTime > 0.35]
...
```{r}
sig0 <- lapply(meta[amino_acid == "LVFAK", sig_file], fread)
sig0 <- do.call(rbind, sig0)[Blockade < 0.3 & DwellTime > 0.35]
B_file <- unique(meta[amino_acid == "LVFAK", file_name])
B_file <- paste0("./analysis/81.ABFProcessing/FeatureMatrix/FeatureMatrix_", B_file, ".txt")
Sig_FeatureMatrix <- do.call(rbind, lapply(B_file, fread))
Sig_FeatureMatrix <- Sig_FeatureMatrix[ID %in% sig0$ID]
...
FeatureMatrix <- data.frame(FeatureMatrix[, grepl("^X", colnames(FeatureMatrix)) | grepl("DwellTime", colnames(FeatureMatrix)), with = F], row.names = FeatureMatrix[[1]])
euclidean = stats::dist(FeatureMatrix, method = "euclidean")
saveRDS(euclidean, file = "./analysis/84.ADpolypeptide/12.SignalDistance/LVFAK_all_sigs_distance.Rds")
```



```{r}
sig0 <- lapply(meta[amino_acid == "LVFAG", sig_file], fread)
sig0 <- do.call(rbind, sig0)[Blockade < 0.3 & DwellTime > 0.35]
B_file <- unique(meta[amino_acid == "LVFAG", file_name])
B_file <- paste0("./analysis/81.ABFProcessing/FeatureMatrix/FeatureMatrix_", B_file, ".txt")
Sig_FeatureMatrix <- do.call(rbind, lapply(B_file, fread))
Sig_FeatureMatrix <- Sig_FeatureMatrix[ID %in% sig0$ID]
...
FeatureMatrix <- data.frame(FeatureMatrix[, grepl("^X", colnames(FeatureMatrix)) | grepl("DwellTime", colnames(FeatureMatrix)), with = F], row.names = FeatureMatrix[[1]])
euclidean = stats::dist(FeatureMatrix, method = "euclidean")
saveRDS(euclidean, file = "./analysis/84.ADpolypeptide/12.SignalDistance/LVFAG_all_sigs_distance.Rds")
```
