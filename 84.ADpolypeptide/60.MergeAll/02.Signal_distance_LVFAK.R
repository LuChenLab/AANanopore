...
saveRDS(dist, file = "./analysis/84.ADpolypeptide/60.MergeAll/02.SignalDistance/LVFAK_spearman_distance_knn.Rds")

dist = stats::dist(FeatureMatrix, method = "euclidean")
dist <- dbscan::kNN(dist, k = 20)
saveRDS(dist, file = "./analysis/84.ADpolypeptide/60.MergeAll/02.SignalDistance/LVFAK_euclidean_distance_knn.Rds")

mclapply(sig0[, unique(A)], function(x) {
  outfile <- paste0("./analysis/84.ADpolypeptide/60.MergeAll/02.SignalDistance/LVFAK_", x)
