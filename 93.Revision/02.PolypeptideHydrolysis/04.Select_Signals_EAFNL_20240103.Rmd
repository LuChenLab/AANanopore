...
RawSig <- data.table(file_id = rep(meta[, file_id], mapply(nrow, RawSig)), do.call(rbind, RawSig))
RawSig <- merge(RawSig, meta[, .(file_id = file_id, experiment)])
...
Sigs1 <- merge(meta[, .(A = file_id, experiment, amino_acid)], Sigs1)
```

```{r fig.width=18, fig.height=8}
ggplot(Sigs1[Blockade < 0.3 & amino_acid == ""], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = .2) + 
  scale_y_log10() + 
  geom_hline(yintercept = 0.35) + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("EAFNL", ""))], Blockade]) + 
  scale_x_continuous(breaks = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("EAFNL", ""))], Blockade], labels = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("EAFNL", ""))], AA]) + 
...
```{r}
meta[, sg_files := paste0("./analysis/93.Revision/02.PolypeptideHydrolysis/03.SignalDistance/", file_id, "_euclidean_distance_knn.Rds")]
...
dist1 <- readRDS(meta[7, sg_files])
sig1 <- KnnSelecting(signals = Sigs1[A == meta[7, file_id]], dists = dist1, k = 10, noise = 0, index = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("EAFNL", ""))], Blockade])
```
