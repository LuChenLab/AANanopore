...
```{r}
meta <- meta[seq == "EAFNL" & Stage == "Polypeptide + Hydrolase"]
```
...
EAFNL <- EAFNL[ID %in% select_sigs$ID]
```

```{r fig.width=12, fig.height=4}
ggplot(EAFNL[order(Delta1)], aes(x = Blockade, y = DwellTime * 1000, colour = Delta1 > 0.99)) + 
  geom_point(size = 0.1) + 
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.1, 0.3)) + 
  geom_vline(xintercept = AABlockade[AA == "Ala", Blockade], col = 2) + 
...
ggplot(EAFNL[Delta1 > 0.99], aes(x = Blockade, y = DwellTime * 1000, colour = Pred1)) + 
  geom_point(size = .2) + 
  scale_y_log10() + 
  scale_x_continuous(breaks = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("EAFNL", ""))], Blockade], limits = c(0.1, 0.4), 
...
```{r fig.width=8, fig.height=4}
ggplot(EAFNL[Delta1 > 0.95], aes(x = BaseMean)) + 
  geom_histogram(binwidth = .5) + 
  scale_x_continuous(limits = c(50, 150)) + 
  facet_wrap( ~ file_id, scales = "free", nrow = 1)
```
...
```{r}
ECDFcumsum <- function(x) {
  lapply(EAFNL[, unique(file_id)], function(fi) {
    x <- EAFNL[file_id == fi]
...
tab <- ECDFcumsum(EAFNL[Delta1 > 0.95])
