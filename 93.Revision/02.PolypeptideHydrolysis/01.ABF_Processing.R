...
GetSignal <- function(abf, minblockade = 0.1, mindwelltime = 0, cores = 1) {
  fun1 <- function(x, y, minblockade = 0.1, mindwelltime = 0.5, cores = cores) {
    y2 <- Rle(y)
    ...
    dtime <- (x[end(res)] - x[start(res)]) * 1000
    res <- res[dtime > mindwelltime] # remove signal dwelltime too short
  ...
  S0 <- abf[, suppressWarnings(fun1(Time, Sm, minblockade = minblockade, mindwelltime = mindwelltime, cores = cores))]
...
files <- list.files("./data/ChenShanchuan/20231225", "abf", recursive = T, full.names = T)
names(files) <- gsub(".abf", "", basename(files))
...
files <- list.files("./data/ChenShanchuan/20231226", "abf", recursive = T, full.names = T)
names(files) <- gsub(".abf", "", basename(files))
files <- data.table(x = files, y = names(files))
files <- files[!y %in% gsub("FeatureMatrix_", "", gsub(".txt", "", list.files("./analysis/81.ABFProcessing/FeatureMatrix")))]

files <- list.files("./data/ChenShanchuan/20231227", "abf", recursive = T, full.names = T)
names(files) <- gsub(".abf", "", basename(files))
files <- data.table(x = files, y = names(files))
files <- files[!y %in% gsub("FeatureMatrix_", "", gsub(".txt", "", list.files("./analysis/81.ABFProcessing/FeatureMatrix")))]

files <- list.files("./data/ChenShanchuan/20231228", "abf", recursive = T, full.names = T)
names(files) <- gsub(".abf", "", basename(files))
files <- data.table(x = files, y = names(files))
files <- files[!y %in% gsub("FeatureMatrix_", "", gsub(".txt", "", list.files("./analysis/81.ABFProcessing/FeatureMatrix")))]

files <- list.files("./data/ChenShanchuan/20231229", "abf", recursive = T, full.names = T)
names(files) <- gsub(".abf", "", basename(files))
files <- data.table(x = files, y = names(files))
files <- files[!y %in% gsub("FeatureMatrix_", "", gsub(".txt", "", list.files("./analysis/81.ABFProcessing/FeatureMatrix")))]

files <- list.files("./data/ChenShanchuan/20240103", "abf", recursive = T, full.names = T)
names(files) <- gsub(".abf", "", basename(files))
files <- data.table(x = files, y = names(files))
files <- files[!y %in% gsub("FeatureMatrix_", "", gsub(".txt", "", list.files("./analysis/81.ABFProcessing/FeatureMatrix")))]

files <- list.files("./data/ChenShanchuan/20240104", "abf", recursive = T, full.names = T)
names(files) <- gsub(".abf", "", basename(files))
files <- data.table(x = files, y = names(files))
files <- files[!y %in% gsub("FeatureMatrix_", "", gsub(".txt", "", list.files("./analysis/81.ABFProcessing/FeatureMatrix")))]

files <- list.files("./data/ChenShanchuan/20240106", "abf", recursive = T, full.names = T)
names(files) <- gsub(".abf", "", basename(files))
files <- data.table(x = files, y = names(files))
files <- files[!y %in% gsub("FeatureMatrix_", "", gsub(".txt", "", list.files("./analysis/81.ABFProcessing/FeatureMatrix")))]

files <- list.files("./data/ChenShanchuan/20240107", "abf", recursive = T, full.names = T)
names(files) <- gsub(".abf", "", basename(files))
files <- data.table(x = files, y = names(files))
files <- files[!y %in% gsub("FeatureMatrix_", "", gsub(".txt", "", list.files("./analysis/81.ABFProcessing/FeatureMatrix")))]

files <- list.files("./data/ChenShanchuan/20240108", "abf", recursive = T, full.names = T)
names(files) <- gsub(".abf", "", basename(files))
files <- data.table(x = files, y = names(files))
files <- files[!y %in% gsub("FeatureMatrix_", "", gsub(".txt", "", list.files("./analysis/81.ABFProcessing/FeatureMatrix")))]
...
file.exists(file.path(outdir, "ABF", paste0("ABF_", files[, y], ".Rds")))
file.exists(file.path(outdir, "RawSignal", paste0("RawSignal_", files[, y], ".txt")))
file.exists(file.path(outdir, "SignalCurrent", paste0("SignalCurrent_", files[, y], ".Rds")))
file.exists(file.path(outdir, "FeatureMatrix", paste0("FeatureMatrix_", files[, y], ".txt")))
...
lapply(1:nrow(files), function(i) {
  ...
  if(file.exists(file.path(outdir, "RawSignal", paste0("RawSignal_", files[i, y], ".txt")))) {
    ...
    Sigs <- tryCatch(GetSignal(abf = abf, minblockade = 0.1, mindwelltime = 0.5, cores = 20), error = function(e) NULL)
