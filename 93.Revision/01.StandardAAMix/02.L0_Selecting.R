...
BaseMeanFiltering <- function(SigFile, starttime = NULL, endtime = NULL, outdir = NULL, target = NULL, product = NULL, label = NULL, MinDwellTime = NULL) {
  ...
  if(!is.null(MinDwellTime)) dataset <- dataset[DwellTime > MinDwellTime, ]
  shinyApp(
    ui = fluidPage(
...
outdir <- file.path("analysis/93.Revision/01.StandardAAMix/02.SelectedL0")

meta <- meta[!file_id %in% gsub(".MainL0.txt", "", list.files("analysis/93.Revision/01.StandardAAMix/02.SelectedL0"))]
# i = 1
meta[i]; BaseMeanFiltering(SigFile = meta[i, sg_files], target = meta[i, file_id], outdir = outdir, starttime = meta[i, start_time], endtime = meta[i, end_time], 
                           ...
                           label = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit(meta[i, amino_acid], ""))], AA], MinDwellTime = 0.35)
(i <- i + 1)
