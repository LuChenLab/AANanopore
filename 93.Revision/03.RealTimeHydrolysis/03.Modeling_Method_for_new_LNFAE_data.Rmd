...
```{r}
meta <- fread("./data/ChenShanchuan/20240103/meta_20240103.txt")
meta[amino_acid == "", amino_acid := NA]
meta$experiment <- paste0("Rep", as.numeric(as.factor(meta$experiment)))
meta[, sg_files := NULL]
...
fwrite(sig2, "./analysis/93.Revision/03.RealTimeHydrolysis/01.SignalPrediction/Predicted_sub_LNFAE_signals.txt", sep = "\t")
```

```{r}
sig_N <- sig2[Delta1 > 0.9 & Pred2 != "Noise", .N, Pred1]
sig_N[, A := plyr::mapvalues(Pred1, AMINO_ACID_CODE, names(AMINO_ACID_CODE))]
sig_N[, A := factor(A, levels = c("E", "A", "F", "N", "L"))]
ggplot(sig_N, aes(x = A, y = N, fill = Pred1)) + 
  geom_col() + 
  geom_text(aes(label = N, y = N + 100), size = 5) + 
...
```{r}
ggplot(sig2[Delta1 > 0.99 & Delta2 > 0.99 & Pred2 != "Noise"], aes(x = Blockade, y = DwellTime, colour = Pred1)) + 
  geom_point(size = 0.1) + 
  scale_y_log10() + 
  scale_x_continuous(breaks = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("EAFNL", ""))], Blockade], 
                     labels = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("EAFNL", ""))], A], limits = c(0.1, 0.3)) + 
...
```{r}
EAFNL <- sig2[Delta1 > 0.9 & Pred2 != "Noise"]
...
EAFNL[, Pred1 := plyr::mapvalues(Pred2, AMINO_ACID_CODE, names(AMINO_ACID_CODE))]
...
```{r}
ggplot() + 
  geom_col(data = Freq_N[seq == "LNFAE", .(N = sum(N)), .(Pred1, experiment)], mapping = aes(x = Pred1, y = N, fill = experiment), position = position_dodge()) + 
  geom_text(data = Freq_N[seq == "LNFAE", .(N = sum(N)), .(Pred1, experiment)],
            mapping = aes(x = Pred1, y = N + 100, label = N),
...
EAFNL[, AA := Pred1]
tab <- ECDFcumsum(EAFNL[seq == "LNFAE"])
...
```{r}
ggplot(EAFNL[seq == "LNFAE"], aes(x = AA, y = StartTime, colour = AA)) + 
  geom_violin() + 
  # geom_label_repel(data = tab[, .SD[which.max(Time)], .(AA, file_id)], aes(label = AA)) + 
  facet_wrap(~ file_id, scales = "free")
```
...
ggplot(tab[file_id %in% c("20240103_0019.1")], aes(x = Time, y = cumsum, colour = AminoAcid)) + 
  geom_smooth() + 
  geom_label_repel(data = tab[file_id %in% c("20240103_0019.1"), .SD[which.max(Time)], .(AA, file_id)], aes(label = AA)) + 
  scale_y_log10() + 
...
EAFNL <- sig2[Prob1 > 0.95 & Prob2 > 0.7 & Pred2 != "Noise" & !A %in% c("20240102_002.1")]
EAFNL[, file_id := as.character(A)]
EAFNL <- merge(EAFNL, meta[, .(file_id, seq, experiment)], by = "file_id")
...
ggplot(Freq_N[seq == "LNFAE"], aes(x = Pred1, y = N, fill = seq)) + 
  geom_col(position = position_dodge()) + 
  geom_text(aes(x = Pred1, y = N * 1.3, label = round(N)), position = position_dodge(width = 0.9), size = 6) + 
  scale_fill_wsj() + 
  facet_wrap(~ file_id, nrow = 2, scales = "free_y") + 
