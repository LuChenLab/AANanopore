...
meta <- meta[date != "1019"]
meta[-(1:5), .(file_name, date, amino_acid, `start_time.(min)`, `end_time.(min)`, baseline_mean)]
```

```{r}
File <- "./data/ZhangMing_20211025/1022-ACTHx3 Ang1+1/21o22014.abf"
StartTime <- 0
EndTime <- 24.9
...
```{r}
DenSm <- density(phe[, Sm], adjust = 0.8)
L0 <- L0Coor(DenSm, d = 111.6)
L0Min <- min(L0) * 0.9
```
...
```{r}
openxlsx::write.xlsx(BinBUB3, "./analysis/02.PolypeptideSequencing/20211025/Version1/AAs_Signal_21o22014.xlsx")
```
