...
meta <- meta[date == "1019" & amino_acid != "blank"]
...
mclapply(seq_len(nrow(meta)), function(f) {
  ...
  File <- paste0("./data/ZhangMing_20211019/", filename, ".abf")
  ...
  # AA Signal
  Sigs <- lapply(BUB, FUN = function(x) DensityPeaksSignal(Sig = x, adjust = 0.1, plot = FALSE))
...
```{r}
hist(SigTab[TimeRatio > 0.8 & DwellTime > 0.75 & SignalSD < 2, Blockade], breaks = 500)
```
