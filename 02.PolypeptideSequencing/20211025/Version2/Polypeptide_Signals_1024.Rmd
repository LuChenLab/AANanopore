...
meta <- meta[date == "1024" & amino_acid != "blank"]
...
mclapply(seq_len(nrow(meta)), function(f) {
  ...
  File <- paste0("./data/ZhangMing_20211025/1024-Ang3/", filename, ".abf")
...
files <- files[grepl("21o24", files)]
SigTabs <- lapply(files, function(x) as.data.table(read.xlsx(x)))
SigTab <- as.data.table(do.call(rbind, SigTabs))
```

```{r}
ggplot(SigTab[TimeRatio > 0.2 & DwellTime > 0.5 & SignalSD < 2, ], aes(x = Blockade, y = DwellTime)) + 
  geom_point()
```

```{r}
hist(SigTab[TimeRatio > 0.2 & DwellTime > 0.5 & SignalSD < 2, Blockade], breaks = 100)
```
