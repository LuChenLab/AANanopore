...
```{r}
meta <- as.data.table(openxlsx::read.xlsx("./data/meta_info_and_base_line_20210823.xlsx"))
meta[date %in% 822][10:13]
```

```{r}
File <- "./data/ZhangMing_20210822/21822010.abf"
StartTime <- 0
EndTime <- 3.5
...
```{r eval=FALSE}
phe <- rbind(phe[Time > 2 * 60 & Time < 6.4 * 60, ], 
             phe[Time > 7.3 * 60 & Time < 18 * 60, ], 
             phe[Time > 19.5 * 60 & Time < 24.9 * 60, ])
```
...
```{r}
DenSm <- density(phe[, Sm], adjust = 0.5)
L0 <- L0Coor(DenSm)
L0Min <- min(L0) * 0.9
```
...
```{r}
openxlsx::write.xlsx(BinBUB3, "./analysis/02.PolypeptideSequencing/Version1/AAs_Signal_21822010.xlsx")
```

```{r}
ggplot() + 
  geom_point(data = BinBUB3, mapping = aes(x = Blockade, y = DwellTime)) + 
  geom_rect(data = AAs, 
            mapping = aes(xmin = blockade - SD, 
                          xmax = blockade + SD, 
                          ymin = 0, ymax = 6, fill = amino.acid), alpha = 0.5) + 
  geom_text_repel(data = AAs, aes(x = blockade, y = 6, label = amino.acid)) + 
  xlim(x = c(0.1, 0.3)) +
