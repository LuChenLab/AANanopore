...
meta <- meta[amino_acid == "KLVVVGAGGV,VGAL"]
...
mclapply(seq_len(nrow(meta)), function(f) {
  ...
  File <- paste0("./data/ZhangMing_20220314/", filename)
  ...
  pdf(paste0("./analysis/02.PolypeptideSequencing/20220314/Version1/L0_Coord_", filename, ".pdf"))
  ...
  saveRDS(BUB, file = paste0("./analysis/02.PolypeptideSequencing/20220314/Version1/BUB", filename, ".Rds"))
  # AA Signal
  ...
  openxlsx::write.xlsx(Sigs, paste0("./analysis/02.PolypeptideSequencing/20220314/Version1/AA_Signals_", filename, ".xlsx"))
}, mc.cores = 1)
```

```{r}
files <- list.files("./analysis/02.PolypeptideSequencing/20220314/Version1", "xlsx", full.names = T)
SigTabs <- lapply(files, function(x) as.data.table(read.xlsx(x)))
SigTab <- as.data.table(do.call(rbind, SigTabs))
```

```{r fig.width=5, fig.height=4}
ggplot(SigTab[TimeRatio > 0.8 & DwellTime > 0.5 & SignalSD < 2, ], aes(x = Blockade, y = DwellTime)) + 
  geom_point() + 
...
```{r fig.width=5, fig.height=4}
ggplot(SigTab[TimeRatio > 0.8 & DwellTime > 0.5 & SignalSD < 2, ], aes(x = Blockade)) + 
...
```{r}
meta <- as.data.table(openxlsx::read.xlsx("./data/ZhangMing_20220314/neoantigen水解20220314.xlsx"))
meta <- meta[amino_acid == "KLVVVGADGV,VGALD"]
meta[, .(file_name, date, amino_acid, `start_time.(min)`, `end_time.(min)`, baseline_mean)]
```
...
mclapply(seq_len(nrow(meta)), function(f) {
  print(paste0(f, " of ", nrow(meta)))
  filename <- meta[f, file_name]
  File <- paste0("./data/ZhangMing_20220314/", filename, ".ABF")
  StartTime <- as.numeric(meta[f, `start_time.(min)`])
  EndTime <- as.numeric(meta[f, `end_time.(min)`])
  base_mean <- as.numeric(meta[f, baseline_mean])
...
```{r}
meta <- as.data.table(openxlsx::read.xlsx("./data/ZhangMing_20220314/neoantigen水解20220314.xlsx"))
meta <- meta[amino_acid == "VLLGVKLSGV,LSGV"]
meta[, .(file_name, date, amino_acid, `start_time.(min)`, `end_time.(min)`, baseline_mean)]
```
...
```{r}
meta <- as.data.table(openxlsx::read.xlsx("./data/ZhangMing_20220314/neoantigen水解20220314.xlsx"))
meta <- meta[amino_acid == "VLLGVKLFGV,LFGV"]
meta[, .(file_name, date, amino_acid, `start_time.(min)`, `end_time.(min)`, baseline_mean)]
```
