...
Sig0 <- lapply(meta[amino_acid == A, file_name], function(x) {
  x <- data.table(file_name = as.character(x), fread(paste0("./analysis/11.SignalIdentification/Jan07/RawSignal_", x, ".txt")))
...
```{r}
Sig1[[4]] <- BaselineSelect(si = Sig0[[4]], AABlockade = AABlockade, a = A, G = 3)
BaselinePlot(Sig1[[4]])
```

```{r}
Sig1[[5]] <- BaselineSelect(si = Sig0[[5]], AABlockade = AABlockade, a = A, G = 3)
BaselinePlot(Sig1[[5]])
```
...
ggsave(paste0("./analysis/12.SignalFiltering/Jan07/Filter1/", A, "_baseline_Select.pdf"), width = 12, height = 6)
```
...
```{r}
ggsave(paste0("./analysis/12.SignalFiltering/Jan07/Filter1/", A, "_Major_Baseline_Signal.pdf"), plot = p1 + theme(legend.position = "none"), width = 4, height = 3)
```
...
ggsave(paste0("./analysis/12.SignalFiltering/Jan07/Filter1/", A, "_Final_Signal.pdf"), width = 4, height = 3)
```

```{r}
fwrite(Sig2, paste0("./analysis/12.SignalFiltering/Jan07/Filter1/", A, "_Signal1.txt"), sep = "\t", quote = F, row.names = F)
fwrite(Sig3_L1, paste0("./analysis/12.SignalFiltering/Jan07/Filter1/", A, "_Signal1_Mainly_L1.txt"), sep = "\t", quote = F, row.names = F)
fwrite(Sig3_L2, paste0("./analysis/12.SignalFiltering/Jan07/Filter1/", A, "_Signal1_Mainly_L2.txt"), sep = "\t", quote = F, row.names = F)
```

```{r}
hist(Sig3_L1[, L2Ratio], breaks = 100)
ggplot(rbind(Sig3_L1[L2Ratio == 0], Sig3_L2), aes(y = DwellTime * 1000, x = Blockade, colour = file_name)) + 
  geom_jitter(size = .1, height = 0, width = 0) + 
  lims(x = c(.1, .6)) +
  scale_y_log10() + 
  theme_bw(base_size = 15) +
  # facet_wrap(~ Group, scales = "free_y") +
  theme(legend.position = "none") +
  labs(y = "Dwell time (ms)", x = "Blockade", title = paste0(A, ": L1: ", nrow(Sig3_L1[L2Ratio == 0]), ", L2: ", nrow(Sig3_L2)))
```
...
```{r message=F}
Sig1 <- mclapply(Sig0, function(x) BaselineSelect(si = x, AABlockade = AABlockade, a = A, G = 5), mc.cores = length(Sig0))
```
...
```{r}
Sig1[[3]] <- BaselineSelect(si = Sig0[[3]], AABlockade = AABlockade, a = A, G = 3)
BaselinePlot(Sig1[[3]])
```
...
```{r}
Sig1[[5]] <- BaselineSelect(si = Sig0[[5]], AABlockade = AABlockade, a = A, G = 2)
BaselinePlot(Sig1[[5]])
```
...
```{r}
Sig1[[3]] <- BaselineSelect(si = Sig0[[3]], AABlockade = AABlockade, a = A, G = 3, minG = 2)
BaselinePlot(Sig1[[3]])
```
