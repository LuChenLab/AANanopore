...
StandardAA <- function(SigFile, outdir = "./analysis/12.SignalFiltering/AASignal/Shiny3/Lys") {
...
files_info <- files_info[AA == "Lys"]

StandardAA(SigFile = files_info[6, file])


StandardAA <- function(SigFile, outdir = "./analysis/12.SignalFiltering/AASignal/Shiny3/Gly") {
  dataset <- fread(SigFile)
  shinyApp(
    ui = fluidPage(
      fluidRow(style = "padding-bottom: 20px;",
...
files_info <- data.table(file = files, AA = mapply(files, FUN = function(x) fread(x)[, unique(A)]))
files_info <- files_info[AA == "Gly"]

StandardAA(SigFile = files_info[5, file])
