...
```{r }
selected.signals <- do.call(rbind, lapply(list.files("./analysis/91.RealTimeHydrolysis/03.SelectedSignals", "euclidean", full.names = TRUE), fread))
...
```{r}
fwrite(EAFNL, "./analysis/91.RealTimeHydrolysis/04.SignalsPrediction/EAFNL_prediction.txt", sep = "\t", quote = FALSE)
```
...
```{r}
ggplot(EAFNL[A == "21822001.2"], aes(x = StartTime, colour = Pred1)) + 
  stat_ecdf()
ggplot(EAFNL[A == "21822002.1"], aes(x = StartTime, colour = Pred1)) + 
  stat_ecdf()
ggplot(EAFNL[A == "21822008.1"], aes(x = StartTime, colour = Pred1)) + 
  stat_ecdf() + 
