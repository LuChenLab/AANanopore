...
Sigs <- Sigs[AA %in% c(AMINO_ACID_CODE[c("A", "E", "F", "N", "L")])]
...
Sig_tu <- rbind(Sigs[AA == "Ala", .(AA, ID = ID0)], 
                ...
                Sigs[AA != "Ala", .SD[sample(.N, 2000, replace = F), .(ID = ID0)], AA], 
...
save(Sigs_Train_Upsample_FM, file = "./analysis/91.RealTimeHydrolysis/00.Models/01.EAFNL/Sigs_Train_Upsample_FM.RData")
...
Fit1 <- train(Class ~ ., 
              ...
              metric = "Accuracy", 
              allowParallel = TRUE)
saveRDS(Fit1, "./analysis/91.RealTimeHydrolysis/00.Models/01.EAFNL/Model_EAFNL_Noise.Rds")
