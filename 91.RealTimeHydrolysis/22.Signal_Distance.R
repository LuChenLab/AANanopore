...
bgfile <- meta[is.na(amino_acid), sig_file]
...
saveRDS(dist, file = "./analysis/91.RealTimeHydrolysis/22.SignalDistance/spearman_distance_knn.Rds")

dist = stats::dist(FeatureMatrix, method = "euclidean")
dist <- dbscan::kNN(dist, k = 20)
saveRDS(dist, file = "./analysis/91.RealTimeHydrolysis/22.SignalDistance/euclidean_distance_knn.Rds")


# mclapply(sig0[, unique(A)], function(x) {
#   outfile <- paste0("./analysis/91.RealTimeHydrolysis/12.SignalDistance/", x)
