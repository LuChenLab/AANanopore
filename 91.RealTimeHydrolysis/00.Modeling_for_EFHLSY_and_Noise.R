...
Sigs <- Sigs[AA %in% c(AMINO_ACID_CODE[c("E", "F", "H", "L", "S", "Y")])]
...
set.seed(123)
Sig_tu <- rbind(Sigs[AA == "His", .(AA, ID = ID0)], 
                Sigs[AA == "His", .SD[sample(.N, 2000 - 499, replace = T), .(ID = ID0)], AA], 
                Sigs[!AA %in% c("His"), .SD[sample(.N, 2000, replace = F), .(ID = ID0)], AA], 
...
save(Sigs_Train_Upsample_FM, file = "./analysis/91.RealTimeHydrolysis/00.Models/01.EFHLSY/Sigs_Train_Upsample_FM.RData")
...
Fit1 <- train(Class ~ ., 
              ...
              metric = "Accuracy", 
              allowParallel = TRUE)
saveRDS(Fit1, "./analysis/91.RealTimeHydrolysis/00.Models/01.EFHLSY/Model_EFHLSY_Noise.Rds")
