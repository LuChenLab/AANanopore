...
```{r fig.width=24, fig.height=6}
alldists <- readRDS(file.path("./analysis/91.RealTimeHydrolysis/12.SignalDistance/euclidean_distance_knn.Rds"))
sigs <- KnnSelecting(signals = dataset[Blockade < 0.3], dists = alldists, k = 10, noise = 0, index = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("FHL", ""))], Blockade])
```
...
```{r}
sigs <- RemoveOutlier(tree = res.hc, n = 10, Keep = 0.8)
...
```{r}
fwrite(dataset[ID %in% sigs], file.path("./analysis/91.RealTimeHydrolysis/13.SelectedSignals/euclidean_distance_knn_selected.signals.txt"), sep = "\t", quote = F)
```
