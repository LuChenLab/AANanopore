...
```{r fig.width=24, fig.height=6}
alldists <- readRDS(file.path("./analysis/91.RealTimeHydrolysis/22.SignalDistance/euclidean_distance_knn.Rds"))
sigs <- KnnSelecting(signals = dataset[Blockade < 0.3], dists = alldists, k = 10, noise = 1, index = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("FHL", ""))], Blockade])
```

```{r}
sigs <- setdiff(sigs, "20231031_0005_2610")
```

```{r}
subtab <- subset.data.frame(FeatureMatrix, row.names(FeatureMatrix) %in% sigs)
res.ds <- stats::dist(subtab, method = "euclidean")
# res.ds <- as.dist(1 - cor(t(subtab), method = "spearman"))
...
ggplot(dataset[ID %in% sigs], aes(x = Blockade, y = DwellTime, colour = ID %in% sigs)) + 
  geom_point(size = .5) + 
  scale_y_log10() + 
  geom_vline(xintercept = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("FHL", ""))], Blockade], colour = "red") + 
  scale_x_continuous(breaks = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("FHL", ""))], Blockade], 
                     labels = AABlockade[AA %in% AMINO_ACID_CODE[unlist(strsplit("FHL", ""))], plyr::mapvalues(AA, AMINO_ACID_CODE, names(AMINO_ACID_CODE))]) + 
  scale_colour_manual(values = "#E41A1C") + 
  theme_classic() + 
  theme(legend.position = "none") + 
...
```{r}
fwrite(dataset[ID %in% sigs], file.path("./analysis/91.RealTimeHydrolysis/23.SelectedSignals/euclidean_distance_knn_selected.signals.txt"), sep = "\t", quote = F)
```
