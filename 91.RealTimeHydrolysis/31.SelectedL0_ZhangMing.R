...
meta <- as.data.table(openxlsx::read.xlsx("data/231101实时水解/实时水解检测_实验信息表.xlsx", sheet = 1))
colnames(meta) <- c("file_name", "start_time", "end_time", "experiment", "batch", "note")
meta[, file_name := gsub(".abf", "", file_name)]
setkey(meta, file_name, start_time)

meta <- meta[!file_name %in% as.data.table(openxlsx::read.xlsx("./analysis/91.RealTimeHydrolysis/meta_ZhangMing.xlsx"))[[1]]]
...
meta[experiment == "铜离子", amino_acid := NA]
meta[file_id == "21807003.2", amino_acid := NA]
meta[file_id == "21808001.1", amino_acid := NA]

meta[experiment == "混匀", Stage := "Homogenized"]
meta[experiment == "铜离子", Stage := "Blank"]
meta[file_id %in% c("21807003.2", "21808001.1"), Stage := "Polypeptide"]
meta[experiment == "水解", Stage := "Polypeptide + Hydrolase"]

meta[file_name %in% c("21807003", "21807004", "21807005"), experiment := "EAFNL Rep1"]
meta[file_name %in% c("21808000", "21808001", "21808005", "21808006"), experiment := "EAFNL Rep2"]
meta <- meta[!file_name %in% "21827013"]

openxlsx::write.xlsx(meta, "./analysis/91.RealTimeHydrolysis/meta_ZhangMing2.xlsx")
