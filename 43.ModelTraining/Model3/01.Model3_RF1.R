...
files <- list.files("./analysis/41.ABFProcessing/02.Background/FeatureMatrix", pattern = "", full.names = TRUE, recursive = TRUE)
Feature_Mat0 <- lapply(files, fread)
Feature_Mat0 <- do.call(rbind, Feature_Mat0)
Feature_Mat0 <- na.omit(Feature_Mat0)

files <- list.files("./analysis/41.ABFProcessing/01.StandardAA/FeatureMatrix", pattern = "", full.names = TRUE, recursive = TRUE)
...
Train <- Train[, c(paste0("X", sprintf("%04d", 513:1024)), "StageSD", "CurrentSD", "Segments", "Blockade", "DwellTime")]
Train$Class <- Selected_Sig_Upsample[, AA]

Test <- setDF(Feature_Mat[Test_Set[, ID], ])
Test <- Test[, c(paste0("X", sprintf("%04d", 513:1024)), "StageSD", "CurrentSD", "Segments", "Blockade", "DwellTime")]
Test$Class <- Test_Set[, AA]


saveRDS(Train, "./analysis/43.ModelTraining/Model3/Train1.Rds")
saveRDS(Test, "./analysis/43.ModelTraining/Model3/Test1.Rds")



Train <- setDF(Feature_Mat[Selected_Sig_Upsample[, ID], ])
Train <- Train[, c(paste0("P", sprintf("%03d", 1:512)), "StageSD", "CurrentSD", "Segments", "Blockade", "DwellTime")]
Train$Class <- Selected_Sig_Upsample[, AA]

Test <- setDF(Feature_Mat[Test_Set[, ID], ])
Test <- Test[, c(paste0("P", sprintf("%03d", 1:512)), "StageSD", "CurrentSD", "Segments", "Blockade", "DwellTime")]
Test$Class <- Test_Set[, AA]


saveRDS(Train, "./analysis/43.ModelTraining/Model3/Train2.Rds")
saveRDS(Test, "./analysis/43.ModelTraining/Model3/Test2.Rds")



Train <- setDF(Feature_Mat[Selected_Sig_Upsample[, ID], ])
Train <- Train[, c(paste0("X", sprintf("%04d", 513:1024)), paste0("P", sprintf("%03d", 1:512)), "StageSD", "CurrentSD", "Segments", "Blockade", "DwellTime")]
Train$Class <- Selected_Sig_Upsample[, AA]

Test <- setDF(Feature_Mat[Test_Set[, ID], ])
Test <- Test[, c(paste0("X", sprintf("%04d", 513:1024)), paste0("P", sprintf("%03d", 1:512)), "StageSD", "CurrentSD", "Segments", "Blockade", "DwellTime")]
Test$Class <- Test_Set[, AA]


saveRDS(Train, "./analysis/43.ModelTraining/Model3/Train3.Rds")
saveRDS(Test, "./analysis/43.ModelTraining/Model3/Test3.Rds")
...
Train <- readRDS("./analysis/43.ModelTraining/Model3/Train1.Rds")
set.seed(825)
RFC <- train(Class ~ ., data = Train, 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/43.ModelTraining/Model3/RF1.Rds")


Train <- readRDS("./analysis/43.ModelTraining/Model3/Train2.Rds")
set.seed(825)
RFC <- train(Class ~ ., data = Train, 
             preProc = c("center", "scale", "YeoJohnson", "nzv"),
             method = "rf", 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/43.ModelTraining/Model3/RF2.Rds")
stopCluster(cl)


Train <- readRDS("./analysis/43.ModelTraining/Model3/Train3.Rds")
set.seed(825)
RFC <- train(Class ~ ., data = Train, 
             preProc = c("center", "scale", "YeoJohnson", "nzv"),
             method = "rf", 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/43.ModelTraining/Model3/RF3.Rds")
stopCluster(cl)
...
RFC <- readRDS("./analysis/43.ModelTraining/Model3/RF1.Rds")
...
RFC <- train(Class ~ ., data = Train, 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/43.ModelTraining/Model3/RF1_1.Rds")
...
RFC <- train(Class ~ ., data = Train, 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/43.ModelTraining/Model3/RF1_2.Rds")
...
RFC <- train(Class ~ ., data = Train, 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/43.ModelTraining/Model3/RF1_3.Rds")
...
RFC <- train(Class ~ ., data = Train, 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/43.ModelTraining/Model3/RF1_4.Rds")
...
RFC <- train(Class ~ ., data = Train, 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/43.ModelTraining/Model3/RF1_5.Rds")
