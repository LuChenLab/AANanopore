...
library(parallel)
library(caret)
load("./analysis/43.ModelTraining/bandwidth/TrainTestSet/TrainTestSet.RData")
...
lapply(c(0.001, 0.005, 0.01, 0.02, 0.05, 0.1), function(d) {
  print(d)
  ...
  Train$Class <- factor(Train_Set$AA, levels = c('M', 'L'))
  RFC <- train(Class ~ ., data = Train, 
               ...
               metric = "Accuracy", 
               allowParallel = TRUE)
  saveRDS(RFC, file = paste0("./analysis/43.ModelTraining/bandwidth/Model/RF", d*1000, ".Rds"))
})

lapply(c(1e-05, 1:9/10000), function(d) {
  print(d)
  FeaMat <- do.call(rbind, lapply(list.files(paste0("./analysis/43.ModelTraining/bandwidth/FeatureMatrix/bandwidth", d*1000), full.names = T), fread))
  setkey(FeaMat, ID)
  Train <- setDF(FeaMat[Train_Set[, ID]])
...
Res <- lapply(c(1e-05, 1:9/10000, 0.001, 0.005, 0.01, 0.02, 0.05, 0.1), function(d) {
  ...
  RFC <- readRDS(paste0("./analysis/43.ModelTraining/bandwidth/Model/RF", d*1000, ".Rds"))
  data.table(bandwidth = d, Obse = Test_Set[, AA], Pred = predict(RFC, Test), Prob = apply(predict(RFC, Test, type = 'prob'), 1, max))
})

ResPer_LM <- lapply(Res, function(x) {
  x[, Obse := factor(Obse, levels = c('L', 'M'))]
  x[, Pred := factor(Pred, levels = c('L', 'M'))]
...
ResPer_LM <- do.call(rbind, ResPer_LM)

ggplot(ResPer_LM[Variable == 'F1'], aes(x = factor(BandWidth), y = Value)) + 
  geom_point()
