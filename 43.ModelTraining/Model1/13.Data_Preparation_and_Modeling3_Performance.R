...
RFC1 <- readRDS("./analysis/43.ModelTraining/Model1/RF.Rds")
RFC2 <- readRDS("./analysis/43.ModelTraining/Model1/RF2.Rds")
RFC3 <- readRDS("./analysis/43.ModelTraining/Model1/RF3.Rds")
...
P1 <- PredFun(mat = fm, model = RFC1); colnames(P1) <- c("ID", "M1Pred", "M1Prob")
P2 <- PredFun(mat = fm, model = RFC2); colnames(P2) <- c("ID", "M2Pred", "M2Prob")
P3 <- PredFun(mat = fm, model = RFC3); colnames(P3) <- c("ID", "M3Pred", "M3Prob")
fm <- Reduce(merge, list(P1, P2, P3))
...
P1 <- PredFun(mat = fm, model = RFC1, p = .5); colnames(P1) <- c("ID", "M1Pred", "M1Prob")
P2 <- PredFun(mat = fm, model = RFC2, p = .5); colnames(P2) <- c("ID", "M2Pred", "M2Prob")
P3 <- PredFun(mat = fm, model = RFC3, p = .5); colnames(P3) <- c("ID", "M3Pred", "M3Prob")
fm <- Reduce(merge, list(P1, P2, P3))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_20230419_0002.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "DE")

ggplot(dataset[M3Pred %in% c('D', 'P', 'F')], aes(x = M3Pred, y = M3Prob)) + 
  geom_point() + 
  geom_boxplot()
...
sort(table(dataset[M3Pred != "Noise", round(StartTime)]))
ggplot(abf[Time > 391 - 1 & Time < 391 + 1], aes(x = Time, y = pA)) + 
  geom_step(colour = 'grey80') + 
  geom_text(data = dataset[StartTime > 391 - 1 & EndTime < 391 + 1], 
            aes(x = StartTime + DwellTime / 2, y = SignalCurrent - 6, label = M3Pred), colour = 'red') + 
...
fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_21o19009.txt")
P1 <- PredFun(mat = fm, model = RFC1); colnames(P1) <- c("ID", "M1Pred", "M1Prob")
P2 <- PredFun(mat = fm, model = RFC2); colnames(P2) <- c("ID", "M2Pred", "M2Prob")
P3 <- PredFun(mat = fm, model = RFC3); colnames(P3) <- c("ID", "M3Pred", "M3Prob")
fm <- Reduce(merge, list(P1, P2, P3))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_21o19009.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "FYSL")



fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_20230323_0014.txt")
P1 <- PredFun(mat = fm, model = RFC1, p = .5); colnames(P1) <- c("ID", "M1Pred", "M1Prob")
P2 <- PredFun(mat = fm, model = RFC2, p = .5); colnames(P2) <- c("ID", "M2Pred", "M2Prob")
P3 <- PredFun(mat = fm, model = RFC3, p = .5); colnames(P3) <- c("ID", "M3Pred", "M3Prob")
fm <- Reduce(merge, list(P1, P2, P3))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_20230323_0014.txt")
dataset <- merge(dataset, fm, by = "ID")
...
table(dataset[M3Pred != "Noise", round(StartTime)])
ggplot(abf[Time > 1200 - 1 & Time < 1200 + 1], aes(x = Time, y = pA)) + 
  geom_step(colour = 'grey80') + 
  geom_text(data = dataset[StartTime > 1200 - 1 & EndTime < 1200 + 1], 
            aes(x = StartTime + DwellTime / 2, y = SignalCurrent - 6, label = M3Pred), colour = 'red') + 
  theme_light(base_size = 15) + 
  theme(panel.grid = element_blank())
