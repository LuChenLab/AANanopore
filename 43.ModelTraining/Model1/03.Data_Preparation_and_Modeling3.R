...
RFC <- readRDS(file = "./analysis/43.ModelTraining/Model1/RF2.Rds")
...
ggplot(Sigs[Pred %in% c("I", "Y", "D") & Pred == AA, ], aes(x = Blockade, colour = AA)) + 
  geom_density()
...
Train3 <- unique(rbind(Sigs[Pred != "Noise" & AA == Pred & Prob > .95, .(ID, AA)], Sigs999[, .(ID, AA)], 
                 Sigs[Pred != "Noise" & AA == Pred & AA == "K" & Prob > .5, .(ID, AA)], 
                 Sigs[Pred != "Noise" & AA == Pred & AA == "Y" & Prob > .5, .(ID, AA)]))

saveRDS(Train3, "./analysis/43.ModelTraining/Model1/Train_Data_ID3.Rds")


set.seed(123)
Selected_Sig_Upsample <- Train3[, .SD[sample(.N, 2000, replace = T), ], AA]
...
saveRDS(Train, "./analysis/43.ModelTraining/Model1/Train3.Rds")
...
Train <- readRDS("./analysis/43.ModelTraining/Model1/Train3.Rds")
set.seed(825)
RFC <- train(Class ~ ., data = Train, 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/43.ModelTraining/Model1/RF3.Rds")



Train3.1 <- Train3[AA %in% c('T', 'N', 'R'), ]
Train3.2 <- Train3[AA %in% c('M', 'L'), ]
Train3.3 <- Train3[AA %in% c('F', 'D', 'P'), ]


set.seed(123)
Selected_Sig_TU_3.1 <- Train3.1[, .SD[sample(.N, 600, replace = T), ], AA]
Selected_Sig_TU_3.1[, AA := factor(AA, levels = c('T', 'N', 'R'))]
setkey(Selected_Sig_TU_3.1, AA)

Train <- setDF(Feature_Mat[Selected_Sig_TU_3.1[, ID], ])
...
Train$Class <- Selected_Sig_TU_3.1[, AA]
saveRDS(Train, "./analysis/43.ModelTraining/Model1/Train3.1.Rds")



set.seed(123)
Selected_Sig_TU_3.2 <- Train3.2[, .SD[sample(.N, 600, replace = T), ], AA]
Selected_Sig_TU_3.2[, AA := factor(AA, levels = c('M', 'L'))]
setkey(Selected_Sig_TU_3.2, AA)

Train <- setDF(Feature_Mat[Selected_Sig_TU_3.2[, ID], ])
Train <- Train[, c(paste0("X", sprintf("%04d", 601:1000)), paste0("P", sprintf("%03d", 1:100)), "StageSD", "CurrentSD", "Segments", "SignalCurrentPercent", "SignalCurrentWidth", "Blockade", "DwellTime")]
Train$Class <- Selected_Sig_TU_3.2[, AA]
saveRDS(Train, "./analysis/43.ModelTraining/Model1/Train3.2.Rds")



set.seed(123)
Selected_Sig_TU_3.3 <- Train3.3[, .SD[sample(.N, 600, replace = T), ], AA]
Selected_Sig_TU_3.3[, AA := factor(AA, levels = c('F', 'D', 'P'))]
setkey(Selected_Sig_TU_3.3, AA)

Train <- setDF(Feature_Mat[Selected_Sig_TU_3.3[, ID], ])
Train <- Train[, c(paste0("X", sprintf("%04d", 601:1000)), paste0("P", sprintf("%03d", 1:100)), "StageSD", "CurrentSD", "Segments", "SignalCurrentPercent", "SignalCurrentWidth", "Blockade", "DwellTime")]
Train$Class <- Selected_Sig_TU_3.3[, AA]
saveRDS(Train, "./analysis/43.ModelTraining/Model1/Train3.3.Rds")
...
Train <- readRDS("./analysis/43.ModelTraining/Model1/Train3.1.Rds")
set.seed(825)
RFC <- train(Class ~ ., data = Train, 
             # preProc = c("center", "scale", "YeoJohnson", "nzv"), 
             method = "rf", 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/43.ModelTraining/Model1/RF3.1.Rds")


Train <- readRDS("./analysis/43.ModelTraining/Model1/Train3.2.Rds")
set.seed(825)
RFC <- train(Class ~ ., data = Train, 
             # preProc = c("center", "scale", "YeoJohnson", "nzv"), 
             method = "rf", 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/43.ModelTraining/Model1/RF3.2.Rds")


Train <- readRDS("./analysis/43.ModelTraining/Model1/Train3.3.Rds")
set.seed(825)
RFC <- train(Class ~ ., data = Train, 
             # preProc = c("center", "scale", "YeoJohnson", "nzv"), 
             method = "rf", 
             ...
             metric = "Accuracy", 
             allowParallel = TRUE)
saveRDS(RFC, file = "./analysis/43.ModelTraining/Model1/RF3.3.Rds")
