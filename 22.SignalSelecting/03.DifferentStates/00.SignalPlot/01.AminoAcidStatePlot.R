...
for (aat in AABlockade$amino_acid) {
  ...
  ggplot(data = data1, mapping = aes(x = Blockade, y = DwellTime)) +
    ...
    theme_light(base_size = 15) + 
    theme(panel.grid = element_blank(), legend.position = "none") -> p2
  p <- cowplot::plot_grid(p1, p2, ncol = 1)
  ggsave(paste0("analysis/22.SignalSelecting/03.DifferentStates/00.SignalPlot/", aat, ".pdf"), p, width = 6, height = 6)
}
