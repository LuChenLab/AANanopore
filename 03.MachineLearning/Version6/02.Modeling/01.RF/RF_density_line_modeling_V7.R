...
AAInfo <- AAInfo[AllTime > 5.5 & SignalSD < 4]
...
Mat <- merge(as.data.table(BinExp, keep.rownames = "ID"), AAInfo[, .(ID, AreaRatio_L1, AreaRatio_L2, Outer, AllTime, DwellTime, SignalSD,  Blockade, amino_acid)], by = "ID")
...
save(Train, Test, up_Test, Valid, up_Valid, file = "./analysis/03.MachineLearning/01.data/Version6/Modeling_Data_7.RData")
save(Train, file = "./analysis/03.MachineLearning/01.data/Version6/Train_Data_7.RData")
...
Fit7 <- train(Class ~ ., data = Train, 
              ...
              metric = "Accuracy", 
              allowParallel = TRUE)
save(Fit7, file = "./analysis/03.MachineLearning/Version6/02.Modeling/01.RF/01.Models/RF_Fit7.RData")
stopCluster(cl)

ROC_Test <- predict(Fit7, up_Test, type = "prob")
...
ROC_Valid <- predict(Fit7, Valid, type = "prob")
...
ROC_Valid_up <- predict(Fit7, up_Valid, type = "prob")
