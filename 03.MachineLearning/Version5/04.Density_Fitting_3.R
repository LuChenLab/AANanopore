...
BinExp <- mclapply(AAInfo$ID, FUN = function(id) {
  round(density(RawSig[ID == id, pA], from = 0, to = 1, n = 512, adjust = 1)$y, 3)
}, mc.cores = 20)
...
Fit1 <- train(Class ~ ., data = Train, 
              ...
              metric = "Accuracy", 
              allowParallel = TRUE)
save(Train, Test, up_Test, Valid, up_Valid, Fit1, file = "./analysis/03.MachineLearning/01.data/Version5/DataSets_and_Model_RF_V3.RData")
stopCluster(cl)
