...
ggplot(Polypeptide1_RF[Prob > 0.7], aes(x = Blockade, y = DwellTime, colour = AA)) + 
...
```{r fig.width=15, fig.height=8}
ggplot(Polypeptide1_KNN[Prob > 0.9], aes(x = Blockade, y = DwellTime, colour = AA)) + 
  geom_text(aes(label = AA)) +
  facet_wrap( ~ Model) + 
  scale_color_manual(values = AA_Cols) + 
  theme_bw(base_size = 15) + 
...
```{r fig.width=9, fig.height=4}
ggplot(Polypeptide1_CART, aes(x = Prob, colour = Model)) + 
  geom_line(stat = "Density") + 
  theme_bw(base_size = 15) + 
  scale_color_brewer(palette = "Dark2") + 
  facet_wrap( ~ Model, nrow = 2)
...
```{r fig.width=15, fig.height=8}
ggplot(Polypeptide1_CART[Prob > 0.7], aes(x = Blockade, y = DwellTime, colour = AA)) + 
  geom_text(aes(label = AA)) +
  facet_wrap( ~ Model) + 
  scale_color_manual(values = AA_Cols) + 
  theme_bw(base_size = 15) + 
...
TargetSignal <- do.call(rbind, lapply(c("L", "S", "Y", "F"), function(a) {
  Polypeptide1_iden[N==3, .(AA = a, Percent = mean(AA %in% a) * 100), Cutoff]
}))
...
ggplot(TargetSignal, aes(x = Cutoff, y = Percent)) + 
  ...
  theme(legend.position = "none")
ggsave("./analysis/03.MachineLearning/Version7/03.PredictPolypeptide/03.DensityLineModelPrediction/Polypeptide1_Prediction_Accuracy.pdf", width = 4, height = 3)
```

```{r fig.width=15, fig.height=8}
ggplot(Polypeptide1_iden[N==3], aes(x = Blockade, y = DwellTime, colour = AA)) + 
  geom_text(aes(label = AA)) + 
  facet_wrap( ~ Cutoff) + 
  scale_color_manual(values = AA_Cols) + 
  theme_bw(base_size = 15) + 
...
ggsave("./analysis/03.MachineLearning/Version7/03.PredictPolypeptide/03.DensityLineModelPrediction/Polypeptide1_Prediction_all_Cutoff.pdf", width = 15, height = 8)
```

```{r}
for(i in 1:6) {
  ggplot(Polypeptide1_iden[Cutoff == paste0("C", i) & N == 3], aes(x = Blockade, y = DwellTime, colour = AA)) + 
    geom_text(aes(label = AA)) +
    scale_color_manual(values = AA_Cols) + 
    theme_bw(base_size = 15) + 
    theme(legend.position = "none") + 
    scale_y_sqrt() -> p
  ggsave(filename = paste0("./analysis/03.MachineLearning/Version7/03.PredictPolypeptide/03.DensityLineModelPrediction/Polypeptide1_Prediction_", i, "_Cutoff.pdf"), p, width = 5, height = 4)
}
```

```{r}
write.csv(Polypeptide1_iden, "./analysis/03.MachineLearning/Version7/03.PredictPolypeptide/03.DensityLineModelPrediction/Polypeptide1_Prediction_all_Cutoff.csv")
```
