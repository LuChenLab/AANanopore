...
ggplot(Polypeptide2_RF[Prob > 0.8], aes(x = Blockade, y = DwellTime, colour = AA)) + 
...
```{r fig.width=15, fig.height=8}
ggplot(Polypeptide2_KNN[Prob > 0.9], aes(x = Blockade, y = DwellTime, colour = AA)) + 
  geom_text(aes(label = AA)) +
  facet_wrap( ~ Model) + 
  scale_color_manual(values = AA_Cols) + 
  theme_bw(base_size = 15) + 
...
Polypeptide2_iden <- lapply(1:6, function(i) {
  ...
  Polypeptide2_RF[Model == paste0("RF", i) & ID %in% ids, .(ID, Model, DwellTime, Blockade, AA)]
})
...
```{r}
write.csv(Polypeptide2_iden, "./analysis/03.MachineLearning/Version7/03.PredictPolypeptide/02.Prediction/Polypeptide2_Prediction_all_Cutoff.csv")
```
...
ggplot(Polypeptide2_iden, aes(x = Blockade, y = DwellTime, colour = AA)) + 
  ...
  theme_bw(base_size = 15) + 
  theme(legend.position = "none")
ggsave("./analysis/03.MachineLearning/Version7/03.PredictPolypeptide/02.Prediction/Polypeptide2_Prediction_all_Cutoff.pdf", width = 15, height = 4)
```
...
for(i in 1:3) {
  ...
  ggsave(filename = paste0("./analysis/03.MachineLearning/Version7/03.PredictPolypeptide/02.Prediction/Polypeptide2_Prediction_", i, "_Cutoff.pdf"), p, width = 5, height = 4)
}
```
