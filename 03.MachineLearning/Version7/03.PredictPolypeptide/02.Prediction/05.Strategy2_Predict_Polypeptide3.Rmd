...
ggplot(Polypeptide3_RF[Prob > 0.8], aes(x = Blockade, y = DwellTime, colour = AA)) + 
...
Polypeptide3_iden <- lapply(1:6, function(i) {
  ...
  Polypeptide3_RF[Model == paste0("RF", i) & ID %in% ids, .(ID, Model, DwellTime, Blockade, AA)]
})
...
```{r}
write.csv(Polypeptide3_iden, "./analysis/03.MachineLearning/Version7/03.PredictPolypeptide/02.Prediction/Polypeptide3_Prediction_all_Cutoff.csv")
```
...
ggplot(Polypeptide3_iden, aes(x = Blockade, y = DwellTime, colour = AA)) + 
  ...
  theme(legend.position = "none") + 
  scale_y_sqrt()
ggsave("./analysis/03.MachineLearning/Version7/03.PredictPolypeptide/02.Prediction/Polypeptide3_Prediction_all_Cutoff.pdf", width = 15, height = 8)
```
...
for(i in 1:6) {
  ggplot(Polypeptide3_iden[Cutoff == paste0("C", i)], aes(x = Blockade, y = DwellTime, colour = AA)) + 
    ...
    theme(legend.position = "none") + 
    scale_y_sqrt() -> p
  ggsave(filename = paste0("./analysis/03.MachineLearning/Version7/03.PredictPolypeptide/02.Prediction/Polypeptide3_Prediction_", i, "_Cutoff.pdf"), p, width = 5, height = 4)
}
```
