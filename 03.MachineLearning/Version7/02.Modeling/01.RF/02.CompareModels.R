...
SignalsNumber <- mclapply(1:6, function(i) {
  load(Modeling_Datas[i])
  merge(Test[, .(Train = unique(table(Train$Class)), Test = .N), amino_acid], Valid[, .(Validation = .N), amino_acid], by = "amino_acid")
}, mc.cores = 6)

SignalsNumber <- data.table(Model = rep(paste0("RF", 1:6), mapply(nrow, SignalsNumber)), do.call(rbind, SignalsNumber))
...
openxlsx::write.xlsx(cutoffList, "analysis/03.MachineLearning/Version7/02.Modeling/01.RF/02.CompareModels/cutoffList.xlsx")
openxlsx::write.xlsx(SignalsNumber, "analysis/03.MachineLearning/Version7/02.Modeling/01.RF/02.CompareModels/SignalsNumber.xlsx")
saveRDS(Preds, file = "analysis/03.MachineLearning/Version7/02.Modeling/01.RF/02.CompareModels/Preds.Rds")

SignalsNumber[, Cutoff := Model]
SignalsNumber[, Cutoff := gsub("RF", "C", Cutoff)]
...
ggsave("analysis/03.MachineLearning/Version7/02.Modeling/01.RF/02.CompareModels/Cutoff_Recovery_Accuracy.pdf", width = 9, height = 6)
...
library(patchwork)
p1 / p2 / p3
ggsave("analysis/03.MachineLearning/Version7/02.Modeling/01.RF/02.CompareModels/Signal_Number_of_Train_Test_Validation.pdf", width = 5, height = 6)



library(ggpubr)
ggplot(SignalsNumber[, mean(Train), Cutoff], aes(x = Cutoff, y = V1, fill = Cutoff)) + 
  geom_col() + 
  scale_fill_brewer(palette = "Dark2") + 
  labs(y = "# training set") + 
  theme_bw(base_size = 15) + 
  theme(legend.position = "top") + 
  guides(fill = guide_legend(nrow = 1)) -> p1
lg <- as_ggplot(get_legend(p1))
p1 <- p1 + theme(legend.position = "none")

ggplot(SignalsNumber, aes(x = Cutoff, y = Test)) + 
  geom_boxplot(aes(colour = Cutoff)) + 
  scale_colour_brewer(palette = "Dark2") + 
  labs(y = "# test set") + 
  guides(colour = "none") + 
...
p1 + p2 + p3
ggsave("analysis/03.MachineLearning/Version7/02.Modeling/01.RF/02.CompareModels/Signal_Number_of_Train_Test_Validation2.pdf", width = 8, height = 3.5)
...
ggsave("analysis/03.MachineLearning/Version7/02.Modeling/01.RF/02.CompareModels/Prediction_Probability_Distribution.pdf", width = 16, height = 6)
...
ggsave("analysis/03.MachineLearning/Version7/02.Modeling/01.RF/02.CompareModels/Prediction_Probability_Distribution2.pdf", width = 16, height = 6)
