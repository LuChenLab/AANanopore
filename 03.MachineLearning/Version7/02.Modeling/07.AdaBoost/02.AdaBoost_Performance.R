...
Models <- list.files("./analysis/03.MachineLearning/Version7/02.Modeling/07.AdaBoost/01.Models", "Rds", full.names = T)
...
cutoffList <- data.table(Model = rep(paste0("AdaBoost", 1:6), mapply(nrow, cutoffList)), do.call(rbind, cutoffList))


dir.create("analysis/03.MachineLearning/Version7/02.Modeling/07.AdaBoost/02.CompareModels")
openxlsx::write.xlsx(cutoffList, "analysis/03.MachineLearning/Version7/02.Modeling/07.AdaBoost/02.CompareModels/cutoffList.xlsx")
saveRDS(Preds, file = "analysis/03.MachineLearning/Version7/02.Modeling/07.AdaBoost/02.CompareModels/Preds.Rds")

cutoffList <- as.data.table(read.xlsx("analysis/03.MachineLearning/Version7/02.Modeling/07.AdaBoost/02.CompareModels/cutoffList.xlsx"))
...
ggplot(cutoffList[Dataset %in% c("Test", "Validation")], aes(x = Recovery * 100, y = Accuracy * 100, colour = Model)) + 
  ...
  theme(legend.position = "top") + 
  geom_point(data = cutoffList[Dataset %in% c("Test", "Validation") & Cutoff %in% c(c(2)/10)]) +
  geom_text_repel(data = cutoffList[Dataset %in% c("Test", "Validation") & Cutoff %in% c(c(2)/10)], aes(label = L)) + 
  scale_color_brewer(palette = "Dark2") + 
  facet_wrap(~ Dataset) + 
  guides(colour = guide_legend(nrow = 1))
ggsave("analysis/03.MachineLearning/Version7/02.Modeling/07.AdaBoost/02.CompareModels/Cutoff_Recovery_Accuracy.pdf", width = 9, height = 6)
...
library(patchwork)
p1 / p2 / p3
ggsave("analysis/03.MachineLearning/Version7/02.Modeling/07.AdaBoost/02.CompareModels/Signal_Number_of_Train_Test_Validation.pdf", width = 5, height = 6)
...
Preds[, Model := gsub("Fit", "AdaBoost", Model)]
...
ggsave("analysis/03.MachineLearning/Version7/02.Modeling/07.AdaBoost/02.CompareModels/Prediction_Probability_Distribution.pdf", width = 16, height = 6)
...
ggsave("analysis/03.MachineLearning/Version7/02.Modeling/07.AdaBoost/02.CompareModels/Prediction_Probability_Distribution2.pdf", width = 16, height = 6)
