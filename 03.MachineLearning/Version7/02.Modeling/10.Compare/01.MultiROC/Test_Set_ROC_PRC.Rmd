...
true_label1 <- dummies::dummy(Test$amino_acid, sep = " ")
...
pred_RF0 <- predict(RF_Fit1, Test)
pred_RF1 <- predict(RF_Fit1, Test, type = "prob")
pred_RF0 <- data.table(pred = pred_RF0, Prob = apply(pred_RF1, 1, max))
pred_RF0$true <- Test$amino_acid
...
pred_NB0 <- predict(NB_Fit1, Test)
pred_NB1 <- predict(NB_Fit1, Test, type = "prob")
pred_NB0 <- data.table(pred = pred_NB0, Prob = apply(pred_NB1, 1, max))
pred_NB0$true <- Test$amino_acid
...
pred_NNet0 <- predict(NNet_Fit1, Test)
pred_NNet1 <- predict(NNet_Fit1, Test, type = "prob")
pred_NNet0 <- data.table(pred = pred_NNet0, Prob = apply(pred_NNet1, 1, max))
pred_NNet0$true <- Test$amino_acid
...
pred_KNN0 <- predict(KNN_Fit1, Test)
pred_KNN1 <- predict(KNN_Fit1, Test, type = "prob")
pred_KNN0 <- data.table(pred = pred_KNN0, Prob = apply(pred_KNN1, 1, max))
pred_KNN0$true <- Test$amino_acid
...
pred_CART0 <- predict(CART_Fit1, Test)
pred_CART1 <- predict(CART_Fit1, Test, type = "prob")
pred_CART0 <- data.table(pred = pred_CART0, Prob = apply(pred_CART1, 1, max))
pred_CART0$true <- Test$amino_acid
...
pred_AdaBoost0 <- predict(AdaBoost_Fit1, Test)
pred_AdaBoost1 <- predict(AdaBoost_Fit1, Test, type = "prob")
pred_AdaBoost0 <- data.table(pred = pred_AdaBoost0, Prob = apply(pred_AdaBoost1, 1, max))
pred_AdaBoost0$true <- Test$amino_acid
...
```{r}
saveRDS(plot_roc_List, "./analysis/03.MachineLearning/Version7/02.Modeling/10.Compare/01.MultiROC/Cutoff1_Testset_plot_roc_List.Rds")
saveRDS(plot_pr_List, "./analysis/03.MachineLearning/Version7/02.Modeling/10.Compare/01.MultiROC/Cutoff1_Testset_plot_pr_List.Rds")
```
...
saveRDS(pred0_List, "./analysis/03.MachineLearning/Version7/02.Modeling/10.Compare/01.MultiROC/Cutoff1_Testset_pred0_List.Rds")
saveRDS(pred1_List, "./analysis/03.MachineLearning/Version7/02.Modeling/10.Compare/01.MultiROC/Cutoff1_Testset_pred1_List.Rds")
```
...
write.csv(confMat, "./analysis/03.MachineLearning/Version7/02.Modeling/10.Compare/01.MultiROC/Cutoff1_Testset_confusionMatrix.csv")
```
...
ggsave("./analysis/03.MachineLearning/Version7/02.Modeling/10.Compare/01.MultiROC/Test_ROCs.pdf", width = 5.5, height = 4)
```
...
ggplot(pr_mat, aes(x = Recall, y = Precision, colour = Model)) + 
  ...
  scale_color_manual(values = MoldeCols[od]) + 
  lims(x = c(0, 1), y = c(0, 1))
ggsave("./analysis/03.MachineLearning/Version7/02.Modeling/10.Compare/01.MultiROC/Test_PRCs.pdf", width = 5.5, height = 4)
```
