...
```{r}
files <- list.files("./analysis/82.ABFProcessing/SelectedSignals/01.StandardAA/02.SelectedSignals", "signal.remove.outliers18_1.Rds", full.names = T)

lapply(files, function (fi) {
  aat <- gsub(".signal.remove.outliers18_1.Rds", "", basename(fi))
  print(aat)
  if(file.exists(paste0("./analysis/82.ABFProcessing/SelectedSignals/01.StandardAA/04.IntervalTime/", aat, "_IntervalTime.txt"))) return(NULL)
  sig <- readRDS(fi)
  ...
  SigInterTime <- lapply(sig, function(sigi) {
    ...
    L0 <- fread(list.files("./analysis/82.ABFProcessing/SelectedL0", sigi[, unique(file_id)], full.names = T))
  ...
  fwrite(SigInterTime, paste0("./analysis/82.ABFProcessing/SelectedSignals/01.StandardAA/04.IntervalTime/", aat, "_IntervalTime.txt"), sep = "\t", quote = F, row.names = F)
})
```
...
```{r}
sig <- fread("./analysis/82.ABFProcessing/SelectedSignals/01.StandardAA/03.GroupedSignals/His1.signal.txt")
sig <- split(sig, sig$File)
SigInterTime <- lapply(sig, function(sigi) {
  print(nrow(sigi))
  setkey(sigi, StartTime)
  ...
  filei <- sigi[, unique(File)]
  abf <- readRDS(paste0("./analysis/81.ABFProcessing/ABF/ABF_", filei, ".Rds"))
  L0 <- fread(list.files("./analysis/82.ABFProcessing/SelectedL0", sigi[, unique(as.character(file_id))], full.names = T))
  ssd <- abf[Sm >= L0[, min(BaseMean)] & Sm <= L0[, max(BaseMean)], sd(pA)]
  abf[, L0 := Sm >= L0[, min(BaseMean) - ssd] & Sm <= L0[, max(BaseMean) + ssd]]
...
SigInterTime <- do.call(rbind, SigInterTime)
fwrite(SigInterTime, paste0("./analysis/82.ABFProcessing/SelectedSignals/01.StandardAA/04.IntervalTime/His1_IntervalTime.txt"), sep = "\t", quote = F, row.names = F)
```

His2

```{r}
sig <- fread("./analysis/82.ABFProcessing/SelectedSignals/01.StandardAA/03.GroupedSignals/His2.signal.txt")
sig <- split(sig, sig$File)
SigInterTime <- lapply(sig, function(sigi) {
  print(nrow(sigi))
  setkey(sigi, StartTime)
...
SigInterTime <- do.call(rbind, SigInterTime)
fwrite(SigInterTime, paste0("./analysis/82.ABFProcessing/SelectedSignals/01.StandardAA/04.IntervalTime/His2_IntervalTime.txt"), sep = "\t", quote = F, row.names = F)
```
