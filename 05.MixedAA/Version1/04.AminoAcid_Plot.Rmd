...
files <- list.files("./analysis/05.MixedAA/Version1_Signal_Prediction", full.names = TRUE)
```
...
ggseqlogo(Polypeptide1_iden[N == 3 & Cutoff == "C4", AA]) + 
  labs(title = meta[gsub("AA_Signals_", "", gsub(".RData", "", basename(files[3]))), AA]) + 
...
Pred_AA_C4 <- lapply(files, function(x) {
  load(x)
  Polypeptide1_iden[N == 3 & Cutoff == "C4", AA]
})
meta[file_name == "Username_2020-07-13_008", AA := "DQ"]
names(Pred_AA_C4) <- meta[gsub("AA_Signals_", "", gsub(".RData", "", basename(files))), AA]
```
...
```{r fig.width=2, fig.height=4}
ggseqlogo(Pred_AA_C4[[1]]) + 
  labs(title = names(Pred_AA_C4)[1]) + 
  theme_minimal_grid(font_size = 15) +
  theme(legend.position = "none", 
        panel.grid = element_blank(), 
        axis.title.x = element_blank(), 
...
ggsave("./analysis/05.MixedAA/Version1_Plot/Cutoff4_All_Seqlogo.pdf", width = 14, height = 2)
```
...
Sig_Res <- lapply(seq_along(files), function(i) {
  load(files[i])
  res <- Polypeptide1_iden[N == 3 & Cutoff == "C4"]
...
ggplot(Sig_Res, aes(x = Blockade, y = DwellTime, colour = AA)) + 
  ...
  theme_bw(base_size = 15) + 
  theme(legend.position = "none")
ggsave("./analysis/05.MixedAA/Version1_Plot/Cutoff4_Scatters.pdf", width = 10, height = 10)
```
