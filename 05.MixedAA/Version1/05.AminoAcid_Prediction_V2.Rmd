...
lapply(seq_along(files), function(i) {
  ...
  Polypeptide1_iden <- lapply(1:6, function(i) {
    ...
    RF_i <- merge(Polypeptide1_RF[Model == paste0("RF", i), .(ID, DwellTime, Blockade, Prob)], Pred_i[, .(ID, AA)], by = "ID")
    KNN_i <- merge(Polypeptide1_KNN[Model == paste0("KNN", i), .(ID, DwellTime, Blockade, Prob)], Pred_i[, .(ID, AA)], by = "ID")
    CART_i <- merge(Polypeptide1_CART[Model == paste0("CART", i), .(ID, DwellTime, Blockade, Prob)], Pred_i[, .(ID, AA)], by = "ID")
    
    setnames(RF_i, "Prob", "RF")
    setnames(KNN_i, "Prob", "KNN")
    setnames(CART_i, "Prob", "CART")
    merge(merge(RF_i, KNN_i, by = c("ID", "DwellTime", "Blockade", "AA")), CART_i)
    
    res <- Reduce(function(x, y) merge(x, y, by = c("ID", "DwellTime", "Blockade", "AA")), list(RF_i, KNN_i, CART_i))
    data.table(Cutoff = paste0("C", i), res)
  })
  Polypeptide1_iden <- do.call(rbind, Polypeptide1_iden)
  
  save(Polypeptide1_iden, Polypeptide1_RF, Polypeptide1_KNN, Polypeptide1_CART, file = paste0("./analysis/05.MixedAA/Version1_Signal_Prediction/", gsub(".RData", "_V2.RData", basename(files[i]))))
})
```
