...
lapply(seq_along(xlsxfiles), function(i) {
  ...
  save(Mat, file = paste0("./analysis/05.MixedAA/Version2/01.SignalTransform/", gsub(".xlsx", ".RData", basename(xlsxfiles[i]))))
})
```
...
```{r}
files <- list.files("./analysis/05.MixedAA/Version2/01.SignalTransform", ".RData", full.names = TRUE)
```
...
lapply(seq_along(files), function(j) {
  ...
  save(Polypeptide1_iden, Polypeptide1_RF, Polypeptide1_KNN, Polypeptide1_CART, file = paste0("./analysis/05.MixedAA/Version2/02.SignalPrediction/", basename(files[j])))
})
```
