...
```{r fig.width=24, fig.height=6}
alldists <- readRDS(file.path("./analysis/87.LimitOfDetection/02.SignalsDistance/R_all_sigs_distance_CSC231026.Rds"))
sigs <- KnnSelecting(signals = dataset, dists = alldists, k = 20, noise = 3, index = AABlockade[AA == "Arg", Blockade])
```
...
ggplot(dataset[Blockade < 0.3], aes(x = Blockade, y = DwellTime)) + 
  geom_point(size = 0.4) + 
...
```{r}
fwrite(dataset[ID %in% sigs], file.path("./analysis/87.LimitOfDetection/03.SelectedSignals/R.selected.signals.CSC231026.txt"), sep = "\t", quote = F)
```
