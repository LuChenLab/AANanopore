...
saveRDS(SigInterTime, file = "./analysis/87.LimitOfDetection/03.SelectedSignals/Asp_euclidean_distance_knn_selected.signals_InterTime.Rds")
```
...
library(ggpubr)
ggplot() + 
  stat_summary(data = Asp_All_Sig_Freq, aes(x = concentration, y = Freq), fun.data = "mean_sd") + 
  ...
  stat_regline_equation(data = Asp_All_Sig_Freq[, .(Freq = mean(Freq)), concentration], 
                        ...
                        label.y = 30, size = 5) + 
  # annotate(geom = "text", 0.75, 60, label = paste0("Slope = ", Asp_All_Sig_Freq[, .(Freq = mean(Freq)), concentration][, round(coef(lm(Freq ~ concentration))[2], 2)]), size = 5) +
  theme_base()
```
