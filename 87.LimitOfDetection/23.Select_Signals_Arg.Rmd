...
dataset <- do.call(rbind, lapply(meta[amino_acid == "Arg", sig_file], fread))[Blockade < 0.35 & DwellTime > 0.3]
B_file <- paste0("./analysis/81.ABFProcessing/FeatureMatrix/FeatureMatrix_", unique(meta[amino_acid == "Arg", file_name]), ".txt")
...
```{r fig.width=24, fig.height=6}
alldists <- readRDS(file.path("./analysis/87.LimitOfDetection/02.SignalsDistance/Arg_euclidean_distance_knn.Rds"))
sigs <- KnnSelecting(signals = dataset, dists = alldists, k = 8, noise = 2, index = AABlockade[AA == "Arg", Blockade])
```
...
```{r}
sigs <- intersect(sigs, dataset[Blockade > 0.16 & Blockade < 0.188, ID])
```
...
```{r}
fwrite(dataset[ID %in% sigs], file.path("./analysis/87.LimitOfDetection/03.SelectedSignals/Arg_euclidean_distance_knn_selected.signals.txt"), sep = "\t", quote = F)
```
