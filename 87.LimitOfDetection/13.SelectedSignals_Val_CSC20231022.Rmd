...
dataset <- do.call(rbind, lapply(meta[amino_acid == "V", sig_file], fread))[Blockade < 0.35 & DwellTime > 0.3]
B_file <- paste0("./analysis/81.ABFProcessing/FeatureMatrix/FeatureMatrix_", unique(meta[amino_acid == "V", file_name]), ".txt")
...
```{r fig.width=24, fig.height=6}
alldists <- readRDS(file.path("./analysis/87.LimitOfDetection/02.SignalsDistance/V_all_sigs_distance_CSC231022.Rds"))
sigs <- KnnSelecting(signals = dataset, dists = alldists, k = 10, noise = 0, index = AABlockade[AA == "Val", Blockade])
```
...
```{r}
fwrite(dataset[ID %in% sigs], file.path("./analysis/87.LimitOfDetection/03.SelectedSignals/V.selected.signals.txt"), sep = "\t", quote = F)
```
