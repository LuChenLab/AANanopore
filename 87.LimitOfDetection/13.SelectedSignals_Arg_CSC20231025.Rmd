...
```{r fig.width=24, fig.height=6}
alldists <- readRDS(file.path("./analysis/87.LimitOfDetection/02.SignalsDistance/R_all_sigs_distance_CSC231025.Rds"))
sigs <- KnnSelecting(signals = dataset, dists = alldists, k = 10, noise = 1, index = AABlockade[AA == "Arg", Blockade])
```
...
```{r}
fwrite(dataset[ID %in% sigs], file.path("./analysis/87.LimitOfDetection/03.SelectedSignals/R.selected.signals.CSC231025.txt"), sep = "\t", quote = F)
```
