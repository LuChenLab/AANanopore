...
dataset <- do.call(rbind, lapply(meta[amino_acid == "Asp", sig_file], fread))[Blockade < 0.35 & DwellTime > 0.3]
B_file <- paste0("./analysis/81.ABFProcessing/FeatureMatrix/FeatureMatrix_", unique(meta[amino_acid == "Asp", file_name]), ".txt")
...
```{r fig.width=24, fig.height=6}
alldists <- readRDS(file.path("./analysis/87.LimitOfDetection/02.SignalsDistance/Asp_euclidean_distance_knn.Rds"))
sigs <- KnnSelecting(signals = dataset, dists = alldists, k = 10, noise = 1, index = AABlockade[AA == "Asp", Blockade])
```
...
ggplot(dataset[ID %in% sigs], aes(x = Blockade, y = DwellTime)) + 
  ...
  geom_vline(xintercept = c(0.195, 0.24))
```

```{r}
sigs <- intersect(sigs, dataset[Blockade > 0.195 & Blockade < 0.24, ID])
```
...
```{r}
sigs <- RemoveOutlier(tree = res.hc, n = 200, Keep = 0.9)
ggplot(dataset[ID %in% sigs], aes(x = Blockade, y = DwellTime, colour = ID %in% sigs)) + 
  geom_point(size = .5) + 
  scale_y_log10() + 
  scale_x_continuous(limits = c(0.1, 0.3)) + 
...
```{r}
fwrite(dataset[ID %in% sigs], file.path("./analysis/87.LimitOfDetection/03.SelectedSignals/Asp_euclidean_distance_knn_selected.signals.txt"), sep = "\t", quote = F)
```
