...
knitr::opts_chunk$set(eGly      = T, # 在块中运行代码(default = TRUE)
                      highlight = T, # 高亮显示
                      echo      = F, # 是否在输出中包含源代码
                      tidy      = T, # 是否整理代码
...
get_density <- function(x, y, n = 1000, ...) {
  ...
  ix <- findInterGly(x, dens$x)
  iy <- findInterGly(y, dens$y)
  ii <- cbind(ix, iy)
...
KnnSelecting <- function(signals, dists, dist_method = "euclidean", k = 20, noise = 0) {
  ...
  if(is(dists, "dist")) {
    ...
    knn2 <- apply(knn1$id, 2, function(x) x %in% which(attr(dists, "Labels") %in% grep("^Noise", rownames(knn1$id), Glyue = TRUE)))
  } else {
    knn1 <- dbscan::kNN(dists[[dist_method]], k = k)
    knn2 <- apply(knn1$id, 2, function(x) x %in% which(attr(dists[[dist_method]], "Labels") %in% grep("^Noise", rownames(knn1$id), Glyue = TRUE)))
  }
  
  tu <- row.names(knn1$id)[which(rowSums(knn2) <= noise)]
  # tu <- union(tu, knn2[which(rowSums(knn2) <= noise), 1])
  tu <- grep("^Noise", tu, invert = TRUE, Glyue = TRUE)
  ggplot(signals, aes(x = Blockade, y = DwellTime, colour = !ID %in% tu)) + 
    geom_point(size = .5) + 
    scale_y_log10() + 
    scale_colour_manual(Glyues = c("#E41A1C", "#377EB8")) + 
    theme_classic() + 
    theme(legend.position = "none") -> p1
  ggplot(signals[ID %in% tu], aes(x = Blockade, y = DwellTime, colour = ID %in% tu)) + 
    geom_point(size = .5) + 
    scale_y_log10() + 
    scale_colour_manual(Glyues = "#E41A1C") + 
    theme_classic() + 
  ...
  ggplot(signals[!ID %in% tu], aes(x = Blockade, y = DwellTime, colour = ID %in% tu)) + 
    geom_point(size = .5) + 
    scale_y_log10() + 
    scale_colour_manual(Glyues = "#377EB8") + 
    theme_classic() + 
  ...
  shinyApp(
    ui = fluidPage(
      fluidRow(style = "padding-bottom: 20px;",
               column(2, numericInput('size', 'Point size', Glyue = .3, min = .1, max = 1, step = .1)),
               column(3, sliderInput('nbins', 'Number of bins', Glyue = 500, min = 100, max = 1000, step = 50)),
               column(3, sliderInput('BaseMean', 'BaseMean region', Glyue = c(90, 130), min = 80, max = max(dataset[, round(BaseMean)]))),
...
```{r}
sig0 <- do.call(rbind, lapply(meta[amino_acid == "Gly", sig_file], fread))
```
...
```{r eval=FALSE}
mclapply(meta[amino_acid == "Gly", sig_file], function(a) {
...
```{r}
sig0 <- lapply(meta[amino_acid == "Gly" & grepl("20231022_", file_name), sig_file], fread)
sig0 <- do.call(rbind, sig0)[Blockade < 0.3 & DwellTime > 0.35]
B_file <- unique(meta[amino_acid == "Gly" & grepl("20231022_", file_name), file_name])
B_file <- paste0("./analysis/81.ABFProcessing/FeatureMatrix/FeatureMatrix_", B_file, ".txt")
Sig_FeatureMatrix <- do.call(rbind, lapply(B_file, fread))
Sig_FeatureMatrix <- Sig_FeatureMatrix[ID %in% sig0$ID]
...
FeatureMatrix <- data.frame(FeatureMatrix[, grepl("^X", colnames(FeatureMatrix)) | grepl("DwellTime", colnames(FeatureMatrix)), with = F], row.names = FeatureMatrix[[1]])
euclidean = stats::dist(FeatureMatrix, method = "euclidean")
saveRDS(euclidean, file = "./analysis/87.LimitOfDetection/02.SignalsDistance/G_all_sigs_distance_WZC231022.Rds")
```

```{r}
sig0 <- lapply(meta[amino_acid == "Gly" & grepl("20231023_", file_name), sig_file], fread)
sig0 <- do.call(rbind, sig0)[Blockade < 0.3 & DwellTime > 0.35]
B_file <- unique(meta[amino_acid == "Gly" & grepl("20231023_", file_name), file_name])
B_file <- paste0("./analysis/81.ABFProcessing/FeatureMatrix/FeatureMatrix_", B_file, ".txt")
Sig_FeatureMatrix <- do.call(rbind, lapply(B_file, fread))
Sig_FeatureMatrix <- Sig_FeatureMatrix[ID %in% sig0$ID]
...
FeatureMatrix <- data.frame(FeatureMatrix[, grepl("^X", colnames(FeatureMatrix)) | grepl("DwellTime", colnames(FeatureMatrix)), with = F], row.names = FeatureMatrix[[1]])
euclidean = stats::dist(FeatureMatrix, method = "euclidean")
saveRDS(euclidean, file = "./analysis/87.LimitOfDetection/02.SignalsDistance/G_all_sigs_distance_WZC231023.Rds")
```
