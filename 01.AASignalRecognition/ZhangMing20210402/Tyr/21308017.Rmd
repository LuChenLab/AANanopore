...
OutPut <- file.path("/mnt/raid61/Personal_data/tangchao/AANanopore/analysis/01.AASignalRecognition/ZhangMing20210402", AA, File)
...
```{r}
phe <- readABF::readABF(list.files("/mnt/raid61/Personal_data/tangchao/AANanopore/data/ZhangMing_20210402", File, full.names = T))
...
phe[10891:11657, plot(Time, pA, type = "s")]
phe[10891:11657, lines(Time, Sm, type = "s", col = "red")]
```

```{r eval=FALSE, fig.width=12, fig.height=6}
phe[Time > 271.48 & Time < 271.5, plot(Time, pA, type = "s")]
phe[Time > 271.48 & Time < 271.5, lines(Time, Sm, type = "s", col = "red")]
```
...
```{r}
phe[, L := NULL]
phe[round(Sm) <= max(L0) & round(Sm) >= min(L0), L := "B"] # baseline
...
BUB <- lapply(BUB, function(x) {
  ...
  lapply(start, function(x) {
    ...
    if(width(rgs)[1] > 2000) {
      start(rgs)[1] <- end(rgs)[1] - 2000
    }
    
    if(width(rgs)[steps] > 2000) {
      end(rgs)[steps] <- start(rgs)[steps] + 2000
    }
    phe[start(range(rgs)):end(range(rgs)), ]
  })
})
BUB <- do.call(c, BUB)

Wid <- mapply(BUB, FUN = function(x) x[L == "U", diff(range(Time))]) # 去除时间太长的阻塞以及太短的尖刺
BUB <- BUB[Wid < quantile(Wid, 0.99) & Wid > quantile(Wid, 0.01)]
length(BUB)
```


```{r}
# BUB2 <- lapply(BUB, function(x) {
#   y <- copy(x)
#   Sm2 <- tryCatch(KBins3(sig = y[L != "B", pA], minseglen1 = 40, pen.value = 1e-20), error = function(e) NA)
#   if(!is.na(Sm2)) {
#     y[L != "B", Sm := Sm2]
#   }
#   return(y)
...
DenSm <- density(do.call(rbind, BUB2)[Sm < 130 & Sm >= 0, Sm], adjust = 3)
...
```{r}
pAFreq <- do.call(rbind, BUB2)[, .N, by = round(Sm)]
setnames(pAFreq, "round", "pA")
pAFreq[, P := N/sum(N) * 100]
...
```{r warning=FALSE}
DenSm <- density(do.call(rbind, BUB2)[Sm < L0Min & Sm >= 40, Sm], adjust = 0.1)
...
```{r}
pAFreq <- do.call(rbind, BUB2)[Sm < L0Min & Sm >= 40, .N, by = round(Sm)]
setnames(pAFreq, "round", "pA")
pAFreq[, P := N/sum(N) * 100]
...
```{r warning=FALSE}
DenSm <- density(do.call(rbind, BUB2)[Sm < L1Min & Sm >= 30, Sm], adjust = 0.5)
...
```{r}
pAFreq <- do.call(rbind, BUB2)[Sm < L1Min & Sm >= 53, .N, by = round(Sm)]
setnames(pAFreq, "round", "pA")
pAFreq[, P := N/sum(N) * 100]
pAFreq <- pAFreq[order(P, decreasing = TRUE), ]
HFpA <- pAFreq[order(P, decreasing = TRUE), pA]
...
```{r}
BUB2 <- BUB2[!mapply(is.null, BUB2)]
BUB2 <- BUB2[mapply(function(x) any(x[, Sm] <= max(L1)), BUB2)]
...
BUB2 <- lapply(BUB2, function(x) {
  y <- copy(x)
  y[round(Sm) <= max(L0) & round(Sm) >= min(L0), S := "0"] # baseline
  y[round(Sm) <= max(L1) & round(Sm) >= min(L1), S := "1"] # first
  y[round(Sm) <= max(L2) & round(Sm) >= min(L2), S := "2"] # second
  return(y)
})
```
...
```{r}
saveRDS(object = BUB2, file = paste0(OutPut, "_BUB.Rds"))
```
...
```{r}
Lna <- mapply(function(x) x[S != 0 | is.na(S), mean(is.na(S))], BUB2)
hist(Lna, breaks = 200)
```
...
```{r fig.width=12, fig.height=6}
# Tab <- BUB2[[which(names(BUB2) == "010")[8]]]
Tab <- BUB2[[8]]
...
```{r}
hist(do.call(rbind, BUB2)[, pA], breaks = diff(range(do.call(rbind, BUB2)[, pA]))/0.1, xlab = "Current (pA)", main = "Histogram of smooth current")
```
...
```{r}
hist(L1_B1B[, (BaseMedian - Lev1Median)/BaseMedian], breaks = )
```


```{r}
mean(L1_B1B[, (BaseMedian - Lev1Median)/BaseMedian])
```

```{r}
sd(L1_B1B[, (BaseMedian - Lev1Median)/BaseMedian])
```
