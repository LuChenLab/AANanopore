...
BUB <- lapply(BUB, function(x) {
  y <- copy(x)
  Sm2 <- tryCatch(KBins3(sig = y[L != "B", pA], minseglen1 = 20, pen.value = 1e-20), error = function(e) NA)
  if(!is.na(Sm2)) {
    y[L != "B", Sm := Sm2]
  }
  return(y)
...
```{r warning=FALSE}
DenSm <- density(do.call(rbind, BUB)[Sm < L0Min & Sm >= 40, Sm], adjust = 0.5)
...
```{r}
pAFreq <- do.call(rbind, BUB)[Sm < L0Min & Sm >= 40, .N, by = round(Sm)]
setnames(pAFreq, "round", "pA")
pAFreq[, P := N/sum(N) * 100]
...
```{r warning=FALSE}
DenSm <- density(do.call(rbind, BUB)[Sm < L1Min & Sm >= 53, Sm], adjust = 0.5)
...
```{r}
pAFreq <- do.call(rbind, BUB)[Sm < L1Min & Sm >= 53, .N, by = round(Sm)]
setnames(pAFreq, "round", "pA")
pAFreq[, P := N/sum(N) * 100]
pAFreq <- pAFreq[order(P, decreasing = TRUE), ]
HFpA <- pAFreq[order(P, decreasing = TRUE), pA]
