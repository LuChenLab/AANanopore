...
```{r fig.width=12, fig.height=6}
DenSm <- density(phe[Sm < 130 & Sm >= 0, Sm], adjust = 8)
...
DenSm <- density(do.call(rbind, BUB2)[Sm < 130 & Sm >= 0, Sm], adjust = )
...
```{r warning=FALSE}
DenSm <- density(do.call(rbind, BUB2)[Sm < L0Min & Sm >= 40, Sm], adjust = 1)
...
```{r warning=FALSE}
DenSm <- density(do.call(rbind, BUB2)[Sm < L1Min & Sm >= 20, Sm], adjust = 0.5)
...
```{r fig.width=12, fig.height=6}
Tab <- BUB2[[which(names(BUB2) == "01210")[4]]]
# Tab <- BUB2[[9]]
...
```{r}
hist(L1_B1B[, (BaseMedian - Lev1Median)/Lev1Median], breaks = 100)
```


```{r}
mean(L1_B1B[, (BaseMedian - Lev1Median)/Lev1Median])
```

```{r}
sd(L1_B1B[, (BaseMedian - Lev1Median)/Lev1Median])
```
