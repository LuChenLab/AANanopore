...
phe$Sm <- KBins2(sig = phe$pA, pen.value = 1-1e-3, minseglen1 = 5, minseglen2 = 2)
```
...
```{r}
# BXB2 <- lapply(BXB, function(x) {
#   x[L != "B", Sm := KBins(sig = pA, pen.value = 1e-3, minseglen1 = 4, minseglen2 = 20)]
#   return(x)
# })
BXB2 <- BXB
```
...
Remove the level1
```{r fig.width=12, fig.height=6}
DenSm <- density(do.call(rbind, BXB2)[Sm < min(L1), Sm], adjust = 1.2)
...
Remove the level2
```{r fig.width=12, fig.height=6}
DenSm <- density(do.call(rbind, BXB2)[Sm < min(L2), Sm], adjust = 1)
...
```{r fig.width=12, fig.height=6}
DenSm <- density(do.call(rbind, BXB2)[Sm < max(L1), Sm], adjust = 0.8)
...
```{r fig.width=12, fig.height=6}
for(i in seq_along(BXB2)) {
  Tab <- BXB2[[which(names(BXB2) == "B1B")[i]]]
  ggplot() + 
  ...
  print(p)
  askYesNo(paste0("This is signal ", i, ", plot next one?"))
}
```



# Step6. Repolish of current
