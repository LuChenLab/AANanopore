...
```{r}
phe <- readABF::readABF("/mnt/raid62/Personal_data/zhangdan/AA_zhangming/0304-phe#2-3/21304018.abf")
phe <- as.data.table(as.data.frame(phe))
colnames(phe) <- c("Time", "pA", "mV")
phe <- phe[Time > 13.5 * 60 & Time < 16.5 * 60, ]
```
...
phe[Time > 810.2 & Time < 810.3, plot(Time, pA, type = "s")]
phe[Time > 810.2 & Time < 810.3, lines(Time, Sm, type = "s", col = "red")]
```
...
```{r}
saveRDS(object = BUB2, file = "/mnt/raid61/Personal_data/tangchao/AANanopore/analysis/01.AASignalRecognition/ZhangMing20210402/phe/21304018_BUB.Rds")
```

```{r}
BUB2 <- readRDS("/mnt/raid61/Personal_data/tangchao/AANanopore/analysis/01.AASignalRecognition/ZhangMing20210402/phe/21304018_BUB.Rds")
```
