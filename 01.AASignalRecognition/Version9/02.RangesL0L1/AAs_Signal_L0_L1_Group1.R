...
for(i in seq_len(nrow(meta1))) {
  ...
  if(file.exists(paste0("./analysis/01.AASignalRecognition/Version9/02.RangesL0L1/Group1/", meta1[i, file_name], ".txt"))) next()
  ...
  abf <- readRDS(paste0("./analysis/01.AASignalRecognition/Version9/01.SignalPolish/Group1/", meta1[i, file_name], ".Rds"))
  ...
  if(anyNA(c(MinL1, MaxL1))) {
    ...
    if(pd == "YES") {
      MinL1 <- abf[pA > MinL0 & pA < MaxL0, median(Sm)] * c(1 - L1max)
      MaxL1 <- abf[pA > MinL0 & pA < MaxL0, median(Sm)] * c(1 - L1min)
    } else {
  ...
  fwrite(res, paste0("./analysis/01.AASignalRecognition/Version9/02.RangesL0L1/Group1/", meta1[i, file_name], ".txt"), row.names = F, sep = "\t", quote = F)
