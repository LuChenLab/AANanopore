...
meta1 <- meta[amino_acid %in% c("His", "Pro", "Tyr")]
...
meta1 <- meta1[file_name %in% gsub(".Rds", "", list.files("./analysis/01.AASignalRecognition/Version9/01.SignalPolish/Group3/"))]

for(i in seq_len(nrow(meta1))) {
  print(paste0(i, " of ", nrow(meta1)))
  print(meta1[i, ])
  
  if(file.exists(paste0("./analysis/01.AASignalRecognition/Version9/02.RangesL0L1/Group3/", meta1[i, file_name], ".txt"))) next()
  ...
  abf <- readRDS(paste0("./analysis/01.AASignalRecognition/Version9/01.SignalPolish/Group3/", meta1[i, file_name], ".Rds"))
  ...
  fwrite(res, paste0("./analysis/01.AASignalRecognition/Version9/02.RangesL0L1/Group3/", meta1[i, file_name], ".txt"), row.names = F, sep = "\t", quote = F)
