...
meta <- do.call(rbind, lapply(list.files("./analysis/01.AASignalRecognition/Version9/02.RangesL0L1/Group3/", ".txt", full.names = TRUE), fread))
...
mclapply(seq_len(nrow(meta)), function(i) {
  print(i)
  BUB <- readRDS(paste0("./analysis/01.AASignalRecognition/Version9/03.BUBSignal/Group3/", meta[i, file_name], "_BUB.Rds"))
  ...
  fwrite(Sigs, paste0("./analysis/01.AASignalRecognition/Version9/04.FinalSignal/Group3/", meta[i, file_name], "_Sigs.txt"), sep = "\t", row.names = F, quote = F)
}, mc.cores = 1)
