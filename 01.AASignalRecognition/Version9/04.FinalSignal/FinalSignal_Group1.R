...
mclapply(seq_len(nrow(meta)), function(i) {
  BUB <- readRDS(paste0("./analysis/01.AASignalRecognition/Version9/03.BUBSignal/Group1/", meta[i, file_name], "_BUB.Rds"))
  ...
  fwrite(Sigs, paste0("./analysis/01.AASignalRecognition/Version9/04.FinalSignal/Group1/", meta[i, file_name], "_Sigs.txt"), sep = "\t", row.names = F, quote = F)
}, mc.cores = 10)

Sigs$ID <- seq_len(nrow(Sigs))

plotSig(BUB[[168]]) +
  geom_hline(yintercept = L1) + 
  geom_hline(yintercept = L2)

Sigs[AreaRatio_L1 > 0.1 & Outer == 0]
ggplot(Sigs[AreaRatio_L1 > 0.1 & Outer == 0], aes(x = Blockade, y = DwellTime)) + 
  geom_point() + 
