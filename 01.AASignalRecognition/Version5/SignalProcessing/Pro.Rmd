...
```{r}
source("/mnt/raid61/Personal_data/tangchao/AANanopore/script/01.AASignalRecognition/Version5/SignalProcessing/AdditionalFunctions.R")
```

```{r eval=FALSE}
sort(unique(openxlsx::read.xlsx("/mnt/raid61/Personal_data/tangchao/AANanopore/data/meta_info_and_base_line_20210520.xlsx")[[3]]))
```
...
OutDir <- file.path("/mnt/raid61/Personal_data/tangchao/AANanopore/analysis/01.AASignalRecognition/Version5", AA)
...
par(mfrow = c(1, 2))
hist(do.call(rbind, BUBs)$pA, probability = T, breaks = 500, xlab = "Current of BUB", main = "Histogram of BUB current")
hist(do.call(c, mclapply(BUBs, function(x) x[L != "B", 1 - pA/(x[L == "B", mean2(pA)])], mc.cores = 10)), 
     probability = T, breaks = 500, xlab = "Blockade of BUB", main = "Histogram of BUB blockade")
abline(v = c(L1Min, L1Max), col = 2)
```

## Baseline length filtering
Here, we need the minimum length of baseline are 500.     

```{r}
BUBs <- BUBs[mapply(BUBs, FUN = function(x) min(width(IRanges(x[, L == "B"])))) >= 500]
```
...
ggplot(BUB3, aes(Bs, color = File)) +
  ...
  geom_vline(xintercept = c(L1Min, L1Max), lty = 3, alpha = 0.3)
```
...
ggplot(BUB3[Bs > min(L1) & Bs < max(L1), ], aes(x = File, y = Bs, color = File)) +
  ...
  theme_classic(base_size = 15) + 
  scale_color_brewer(palette = "Dark2") + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1), legend.position = "none") -> p3
cowplot::plot_grid(p1, p2, p3, nrow = 1, rel_widths = c(2, 1, 1))
```
...
```{r fig.width=10, fig.height=4}
par(mfrow = c(1, 2))
hist(BUB_Tab[, pA], probability = TRUE, breaks = 500, xlab = "Current of BUB", main = "Histogram of BUB current")
hist(do.call(c, mclapply(BUBs2, function(x) x[L != "B", 1 - pA/(x[L == "B", mean2(pA)])], mc.cores = 10)), 
     probability = T, breaks = 500, xlab = "Blockade of BUB", main = "Histogram of BUB blockade")
```
...
```{r}
UN <- mapply(BUBs2, FUN = function(x) {
  x[L == "U", .N, Sm][, max(N)]
})
BUBs2 <- BUBs2[UN > quantile(UN, 0.01)]
```
...
mclapply(BUBs2, function(x) {
  ...
  # Blockade <- 1 - x[L == "U", median(Sm)]/x[L == "B", mean2(pA)]
  # Blockade <- 1 - x[L == "U", mean2(pA, q1 = 0.4, q2 = 0.6)]/x[L == "B", mean2(pA, q1 = 0.4, q2 = 0.6)]
  Blockade <- 1 - x[L == "U", median(Sm)]/x[L == "B", mean2(pA, q1 = 0.4, q2 = 0.6)]
...
para <- fit$estimate
hist(Summ[, Blockade], prob = TRUE, breaks = 7, xlab = "Blockade of L1", main = "Histogram of L1 blockade")
x <- BUB3$Blockade
curve(dnorm(x, para[1], para[2]), col = 2, add = TRUE)
set.seed(123)
