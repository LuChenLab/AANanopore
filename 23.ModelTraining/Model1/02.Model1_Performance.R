...
RFC <- readRDS("./analysis/23.ModelTraining/Model1/RF.Rds")


fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_20230328_0007.txt")
fm <- data.table(ID = fm[, ID], M1Pred = predict(RFC, fm), M1Prob = apply(predict(RFC, fm, type = "prob"), 1, max))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_20230328_0007.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "FYSLHE")



fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_20230411_0001.txt")
fm <- data.table(ID = fm[, ID], M1Pred = predict(RFC, fm), M1Prob = apply(predict(RFC, fm, type = "prob"), 1, max))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_20230411_0001.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "L")



fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_20230330 _0007.txt")
fm <- data.table(ID = fm[, ID], M1Pred = predict(RFC, fm), M1Prob = apply(predict(RFC, fm, type = "prob"), 1, max))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_20230330 _0007.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "LVFAGK")
...
fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_20230508_0014.txt")
fm <- data.table(ID = fm[, ID], M1Pred = predict(RFC, fm), M1Prob = apply(predict(RFC, fm, type = "prob"), 1, max))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_20230508_0014.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "GAVLIFWYNQMSTPDE")


fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_20230320_001.txt")
fm <- data.table(ID = fm[, ID], M1Pred = predict(RFC, fm), M1Prob = apply(predict(RFC, fm, type = "prob"), 1, max))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_20230320_001.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "LWQST")


fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_20230320_002.txt")
fm <- data.table(ID = fm[, ID], M1Pred = predict(RFC, fm), M1Prob = apply(predict(RFC, fm, type = "prob"), 1, max))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_20230320_002.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "LWQSTDE")
...
fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_2023_05_17_0001.txt")
fm <- data.table(ID = fm[, ID], M1Pred = predict(RFC, fm), M1Prob = apply(predict(RFC, fm, type = "prob"), 1, max))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_2023_05_17_0001.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "EKFS")


fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_2023_05_16_0003.txt")
fm <- data.table(ID = fm[, ID], M1Pred = predict(RFC, fm), M1Prob = apply(predict(RFC, fm, type = "prob"), 1, max))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_2023_05_16_0003.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "EKFS")


fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_20230512_0002.txt")
fm <- data.table(ID = fm[, ID], M1Pred = predict(RFC, fm), M1Prob = apply(predict(RFC, fm, type = "prob"), 1, max))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_20230512_0002.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "GAVLIFWYNQMSTP")


fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_20230419_0007.txt")
fm <- data.table(ID = fm[, ID], M1Pred = predict(RFC, fm), M1Prob = apply(predict(RFC, fm, type = "prob"), 1, max))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_20230419_0007.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "LWQSTIFNDE")


fm <- fread("analysis/21.ABFProcessing/03.MixtureAA/FeatureMatrix/FeatureMatrix_20230321_003.txt")
fm <- data.table(ID = fm[, ID], M1Pred = predict(RFC, fm), M1Prob = apply(predict(RFC, fm, type = "prob"), 1, max))
dataset <- fread("./analysis/21.ABFProcessing/03.MixtureAA/RawSignal/RawSignal_20230321_003.txt")
dataset <- merge(dataset, fm, by = "ID")
PointStat(dataset, target = "LWQSTDE")
