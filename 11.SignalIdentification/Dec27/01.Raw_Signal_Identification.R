...
mclapply(1:nrow(meta), function(i) {
  ...
  saveRDS(abf, file = paste0("./analysis/11.SignalIdentification/Dec27/ABF_", meta[i, file_name], ".Rds"))
  fwrite(Sigs, file = paste0("./analysis/11.SignalIdentification/Dec27/RawSignal_", meta[i, file_name], ".txt"), sep = "\t", row.names = F, quote = F)
}, mc.cores = 10)
...
mclapply(1:nrow(meta), function(i) {
  print(i)
  if(file.exists(paste0("./analysis/11.SignalIdentification/Dec27/RawSignal_", meta[i, file_name], ".txt"))) return()
  File <- meta[i, file_path]
  StartTime <- meta[i, start_time]
  EndTime <- meta[i, end_time]
