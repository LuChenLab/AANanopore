...
```{r}
meta <- data.table(openxlsx::read.xlsx("./data/ChenShanchuan/20230402_01/数据记录20230402.xlsx", sheet = 1))
meta <- meta[, c(1, 4, 2, 3)]
...
meta <- meta[!grepl("P1水解", amino_acid)]
meta$amino_acid <- mapply(function(x) tail(x, 1), strsplit(meta$amino_acid, "水解产物"))
meta[, amino_acid := gsub("）", "", amino_acid)]
meta[, amino_acid := gsub("、", "", amino_acid)]
meta[, amino_acid := gsub(" ", "", amino_acid)]

meta$file_path <- as.character(mapply(meta$file_name, FUN = function(x) list.files("./data/ChenShanchuan/20230402_01", x, full.names = T, recursive = T)))
...
mclapply(1:nrow(meta), function(i) {
  print(i)
  if(file.exists(paste0("./analysis/11.SignalIdentification/Apr02/SignalCurrent_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".Rds"))) return(NULL)
  ...
  saveRDS(abf, file = paste0("./analysis/11.SignalIdentification/Apr02/ABF_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".Rds"))
  fwrite(Sigs, file = paste0("./analysis/11.SignalIdentification/Apr02/RawSignal_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".txt"), sep = "\t", row.names = F, quote = F)
  Current <- SignalCurrent(Sigs, abf = abf, cores = 10)
  saveRDS(Current, file = paste0("./analysis/11.SignalIdentification/Apr02/SignalCurrent_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".Rds"))
}, mc.cores = 1)
```
