...
```{r}
file.path("./data/WangZichun/230314/数据记录.xlsx")

meta <- data.table(openxlsx::read.xlsx("./data/ChenShanchuan/20230315/多肽水解14-15/数据记录.xlsx", sheet = 1))
meta <- meta[, c(1, 4, 2, 3)]
...
meta$file_path <- mapply(meta$file_name, FUN = function(x) list.files("./data/ChenShanchuan/20230315", x, full.names = T, recursive = T))
...
mclapply(1:nrow(meta), function(i) {
  ...
  saveRDS(abf, file = paste0("./analysis/11.SignalIdentification/Mar27/ChenShanchuan20230315/ABF_", meta[i, paste0(amino_acid, "_", file_name)], ".Rds"))
  fwrite(Sigs, file = paste0("./analysis/11.SignalIdentification/Mar27/ChenShanchuan20230315/RawSignal_", meta[i, paste0(amino_acid, "_", file_name)], ".txt"), sep = "\t", row.names = F, quote = F)
}, mc.cores = 1)
```
...
mclapply(1:nrow(meta), function(i) {
  print(i)
  abf <- readRDS(file = paste0("./analysis/11.SignalIdentification/Mar27/ChenShanchuan20230315/ABF_", meta[i, paste0(amino_acid, "_", file_name)], ".Rds"))
  Sigs <- fread(file = paste0("./analysis/11.SignalIdentification/Mar27/ChenShanchuan20230315/RawSignal_", meta[i, paste0(amino_acid, "_", file_name)], ".txt"))
  Current <- SignalCurrent(Sigs, abf = abf, cores = 1)
  saveRDS(Current, file = paste0("./analysis/11.SignalIdentification/Mar27/ChenShanchuan20230315/SignalCurrent_", meta[i, paste0(amino_acid, "_", file_name)], ".Rds"))
}, mc.cores = 1)
```
...
mclapply(1:nrow(meta), function(i) {
  ...
  Sigs[, DwellTime := EndTime - StartTime]
  Sigs <- data.table(ID = paste(meta[i, file_name], Sigs[, seq_len(.N)], sep = "_"), Sigs)
  saveRDS(abf, file = paste0("./analysis/11.SignalIdentification/Mar27/ABF_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".Rds"))
  fwrite(Sigs, file = paste0("./analysis/11.SignalIdentification/Mar27/RawSignal_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".txt"), sep = "\t", row.names = F, quote = F)
}, mc.cores = 10)
```
...
mclapply(1:nrow(meta), function(i) {
  print(i)
  abf <- readRDS(file = paste0("./analysis/11.SignalIdentification/Mar27/ABF_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".Rds"))
  Sigs <- fread(file = paste0("./analysis/11.SignalIdentification/Mar27/RawSignal_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".txt"))
  Current <- SignalCurrent(Sigs, abf = abf, cores = 10)
  saveRDS(Current, file = paste0("./analysis/11.SignalIdentification/Mar27/SignalCurrent_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".Rds"))
}, mc.cores = 2)
```
