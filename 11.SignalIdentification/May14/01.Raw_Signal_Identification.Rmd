...
```{r}
meta <- data.table(openxlsx::read.xlsx("./data/ChenShanchuan/20230514/副本数据记录20230514.xlsx", sheet = 1))
meta <- meta[, c(1, 4, 2, 3)]
colnames(meta) <- c("file_name", "amino_acid", "start_time", "end_time")
meta <- meta[file_name %in% gsub(".abf", "", list.files("./data/ChenShanchuan/20230514", "abf$"))]
...
meta$file_path <- as.character(mapply(meta$file_name, FUN = function(x) list.files("./data/ChenShanchuan/20230514", x, full.names = T, recursive = T)))
...
mclapply(1:nrow(meta), function(i) {
  print(i)
  if(file.exists(paste0("./analysis/11.SignalIdentification/May14/SignalCurrent_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".Rds"))) return(NULL)
  ...
  saveRDS(abf, file = paste0("./analysis/11.SignalIdentification/May14/ABF_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".Rds"))
  fwrite(Sigs, file = paste0("./analysis/11.SignalIdentification/May14/RawSignal_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".txt"), sep = "\t", row.names = F, quote = F)
  Current <- SignalCurrent(Sigs, abf = abf, cores = 10)
  saveRDS(Current, file = paste0("./analysis/11.SignalIdentification/May14/SignalCurrent_", meta[i, paste(amino_acid, file_name, start_time, end_time, sep = "_")], ".Rds"))
}, mc.cores = 1)
```
