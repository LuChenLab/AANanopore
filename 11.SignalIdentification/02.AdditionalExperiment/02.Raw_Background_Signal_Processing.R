...
meta0$abf_file <- as.character(mapply(meta0[, file_name], FUN = function(x) list.files("./data/ChenShanchuan", pattern = x, full.names = TRUE, recursive = TRUE)))
meta0[file_name == "20230330 _0005", abf_file := "./data/ChenShanchuan/20230402_01/20230330_0005.abf"]

meta <- meta0[, .(start_time = min(start_time), end_time = max(end_time)), abf_file]
meta[, file_name := gsub(".abf", "", basename(abf_file))]
meta[file_name == "20230330_0005", file_name := "20230330 _0005"]
