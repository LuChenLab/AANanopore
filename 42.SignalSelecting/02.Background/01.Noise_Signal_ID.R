...
StandardAA <- function(SigFile, outdir = "analysis/42.SignalSelecting/02.Background", target = NULL) {
  ...
  shinyApp(
    ...
    server = function(input, output, session) {
      ...
      selectedData2 <- reactive({
        ...
        dataset[DwellTime > min(input$DwellTime) & DwellTime < max(input$DwellTime)]
      })
...
StandardAA(SigFile = meta[24, file], target = "noise")




noise <- do.call(rbind, lapply(list.files("analysis/42.SignalSelecting/02.Background/noise", full.names = T), fread))

ggplot(noise, aes(x = Blockade, y = DwellTime)) + 
  geom_point() + 
  scale_x_continuous(limits = c(.1, .3)) + 
  scale_y_log10()

noise[, Blockade2 := round(Blockade * 10000)]
set.seed(1)
noise2 <- noise[, .SD[sample(.N, 1)], Blockade2]
set.seed(2)
noise3 <- noise[, .SD[sample(.N, 1)], Blockade2]
set.seed(3)
noise4 <- noise[, .SD[sample(.N, 1)], Blockade2]
set.seed(4)
noise5 <- noise[, .SD[sample(.N, 1)], Blockade2]
set.seed(5)
noise6 <- noise[, .SD[sample(.N, 1)], Blockade2]
noise2 <- unique(rbind(noise2, noise3, noise4, noise5, noise6))
ggplot(noise2, aes(x = Blockade, y = DwellTime)) + 
  geom_point() + 
  # scale_x_continuous(limits = c(.1, .3)) + 
  scale_y_log10() + 
  geom_vline(xintercept = c(.1, .2), col = 2)

NoiseID <- data.table(ID = noise2[, ID], AA = 'Noise')
setkey(NoiseID, ID)
write.table(NoiseID, "./analysis/42.SignalSelecting/02.Background/Noise_Signal_ID.txt", sep = '\t', quote = F, row.names = F)
