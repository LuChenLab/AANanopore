...
mclapply(AAs, function(aat) {
  if(file.exists(paste0("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/02.SelectedSignals/", aat, ".signal.dist.after.knn10.Rds"))) return(NULL)
  ...
  alldists <- readRDS(file.path("./analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/01.SignalDistance", paste0(aat, ".signal.dist.Rds")))
  ...
  saveRDS(res.hc, file = paste0("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/02.SelectedSignals/", aat, ".signal.dist.after.knn10.Rds"))
}, mc.cores = 21)
...
mclapply(AAs, function(aat) {
  if(file.exists(paste0("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/02.SelectedSignals/", aat, ".signal.dist.after.knn15.Rds"))) return(NULL)
  bgsignal <- fread(paste0("./analysis/81.ABFProcessing/SelectedSignals/", aat, "_background.txt"))
  bgsignal[, file_id := as.character(file_id)]
  bgsignal[, File := gsub("\\.[0-9]", "", file_id)]
  bgsignal[, ID := paste0("Noise_", ID)]
  ...
  res.ds <- stats::dist(subtab)
  res.hc <- stats::hclust(res.ds, method = "single")
  saveRDS(res.hc, file = paste0("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/02.SelectedSignals/", aat, ".signal.dist.after.knn15.Rds"))
}, mc.cores = 21)
...
mclapply(AAs, function(aat) {
  if(file.exists(paste0("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/02.SelectedSignals/", aat, ".signal.dist.after.knn18_1.Rds"))) return(NULL)
  dataset <- fread(paste0("./analysis/81.ABFProcessing/SelectedSignals/", aat, "_signal.txt"))
  dataset[, file_id := as.character(file_id)]
  dataset[, File := gsub("\\.[0-9]", "", file_id)]
  dataset[, ID := paste0(aat, "_", ID)]
  ...
  res.ds <- stats::dist(subtab)
  res.hc <- stats::hclust(res.ds, method = "single")
  saveRDS(res.hc, file = paste0("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/02.SelectedSignals/", aat, ".signal.dist.after.knn18_1.Rds"))
}, mc.cores = 21)
...
mclapply(AAs, function(aat) {
  if(file.exists(paste0("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/02.SelectedSignals/", aat, ".signal.dist.after.knn20.Rds"))) return(NULL)
  bgsignal <- fread(paste0("./analysis/81.ABFProcessing/SelectedSignals/", aat, "_background.txt"))
  bgsignal[, file_id := as.character(file_id)]
  bgsignal[, File := gsub("\\.[0-9]", "", file_id)]
  bgsignal[, ID := paste0("Noise_", ID)]
  ...
  res.ds <- stats::dist(subtab)
  res.hc <- stats::hclust(res.ds, method = "single")
  saveRDS(res.hc, file = paste0("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/02.SelectedSignals/", aat, ".signal.dist.after.knn20.Rds"))
}, mc.cores = 21)
...
mclapply(AAs, function(aat) {
  if(file.exists(paste0("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/02.SelectedSignals/", aat, ".signal.remove.outliers18_1.Rds"))) return(NULL)
  dataset <- fread(paste0("./analysis/81.ABFProcessing/SelectedSignals/", aat, "_signal.txt"))
  dataset[, file_id := as.character(file_id)]
  dataset[, File := gsub("\\.[0-9]", "", file_id)]
  dataset[, ID := paste0(aat, "_", ID)]
  
  res.hc <- readRDS(paste0("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/02.SelectedSignals/", aat, ".signal.dist.after.knn18_1.Rds"))
  sigs <- RemoveOutlier(tree = res.hc, n = 50, Keep = 0.95)
  saveRDS(dataset[ID %in% sigs], file = paste0("analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/02.SelectedSignals/", aat, ".signal.remove.outliers18_1.Rds"))
}, mc.cores = 21)
