...
bgfile <- list.files("./analysis/81.ABFProcessing/SelectedSignals", "_background.txt", full.names = TRUE)
...
Big_FeatureMatrixs <- lapply(seq_len(nrow(B_file)), FUN = function(i) {
  ...
  Big_FeatureMatrix <- paste0("./analysis/81.ABFProcessing/FeatureMatrix/FeatureMatrix_", FMfs, ".txt")
...
saveRDS(Big_FeatureMatrixs, "./analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/01.SignalDistance/StandardAA_Big_FeatureMatrixs.Rds")



mclapply(c(AMINO_ACID_CODE[1:20], "CbC"), FUN = function(aat){
  print(aat)
  outfile <- file.path("./analysis/81.ABFProcessing/SelectedSignals/01.StandardAA/01.SignalDistance", paste0(aat, ".signal.dist.Rds"))
  ...
  Sig_FeatureMatrix <- paste0("./analysis/81.ABFProcessing/FeatureMatrix/FeatureMatrix_", FMfs, ".txt")
